...
static cl::opt<unsigned>
    inc_threshold("machine-combiner-inc-threshold", cl::Hidden, cl::desc("Incremental depth computation will be used for basic "
...
static cl::opt<bool> VerifyPatternOrder(
    "machine-combiner-verify-pattern-order", cl::Hidden, cl::desc("Verify that the generated patterns are ordered by increasing latency"), cl::init(true));
#else
static cl::opt<bool> VerifyPatternOrder(
    "machine-combiner-verify-pattern-order", cl::Hidden, cl::desc("Verify that the generated patterns are ordered by increasing latency"), cl::init(false));
#endif
...
private:
    ...
    unsigned getLatency(MachineInstr* Root, MachineInstr* NewRoot, MachineTraceMetrics::Trace BlockTrace);
    bool improvesCriticalPathLen(MachineBasicBlock* MBB, MachineInstr* Root, MachineTraceMetrics::Trace BlockTrace, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg, unsigned Pattern, bool SlackIsAccurate);
    bool reduceRegisterPressure(MachineInstr& Root, MachineBasicBlock* MBB, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, unsigned Pattern);
    ...
    void verifyPatternOrder(MachineBasicBlock* MBB, MachineInstr& Root, SmallVector<unsigned, 16>& Patterns);
...
INITIALIZE_PASS_BEGIN(MachineCombiner, DEBUG_TYPE, "Machine InstCombiner", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(MachineTraceMetricsWrapperPass)
INITIALIZE_PASS_END(MachineCombiner, DEBUG_TYPE, "Machine InstCombiner", false, false)
...
MachineCombiner::getDepth(SmallVectorImpl<MachineInstr*>& InsInstrs,
    ...
    for (auto* InstrPtr : InsInstrs) { // for each Use
        ...
        for (const MachineOperand& MO : InstrPtr->all_uses()) {
            ...
            DenseMap<unsigned, unsigned>::iterator II = InstrIdxForVirtReg.find(MO.getReg());
            if (II != InstrIdxForVirtReg.end()) {
                ...
                assert(DefInstr && "There must be a definition for a new virtual register");
                DepthOp = InstrDepth[II->second];
                int DefIdx = DefInstr->findRegisterDefOperandIdx(MO.getReg(), /*TRI=*/nullptr);
                int UseIdx = InstrPtr->findRegisterUseOperandIdx(MO.getReg(), /*TRI=*/nullptr);
                LatencyOp = TSchedModel.computeOperandLatency(DefInstr, DefIdx,
                    InstrPtr, UseIdx);
            } else {
                MachineInstr* DefInstr = getOperandDef(MO);
                if (DefInstr && (TII->getMachineCombinerTraceStrategy() != MachineTraceStrategy::TS_Local || DefInstr->getParent() == &MBB)) {
...
unsigned MachineCombiner::getLatency(MachineInstr* Root, MachineInstr* NewRoot, MachineTraceMetrics::Trace BlockTrace) {
...
std::pair<unsigned, unsigned> MachineCombiner::getLatenciesForInstrSequences(
    MachineInstr& MI, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, MachineTraceMetrics::Trace BlockTrace) {
...
bool MachineCombiner::reduceRegisterPressure(
    MachineInstr& Root, MachineBasicBlock* MBB, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, unsigned Pattern) {
...
bool MachineCombiner::improvesCriticalPathLen(
    MachineBasicBlock* MBB, MachineInstr* Root, MachineTraceMetrics::Trace BlockTrace, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg, unsigned Pattern, bool SlackIsAccurate) {
    // Get depth and latency of NewRoot and Root.
    unsigned NewRootDepth = getDepth(InsInstrs, InstrIdxForVirtReg, BlockTrace, *MBB);
    ...
    if (TII->accumulateInstrSeqToRootLatency(*Root)) {
        std::tie(NewRootLatency, RootLatency) = getLatenciesForInstrSequences(*Root, InsInstrs, DelInstrs, BlockTrace);
    } else {
    ...
    unsigned OldCycleCount = RootDepth + RootLatency + (SlackIsAccurate ? RootSlack : 0);
...
bool MachineCombiner::preservesResourceLen(
    MachineBasicBlock* MBB, MachineTraceMetrics::Trace BlockTrace, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs) {
    ...
    unsigned ResLenAfterCombine = BlockTrace.getResourceLength(MBBarr, MSCInsArr, MSCDelArr);

    LLVM_DEBUG(dbgs() << "\t\tResource length before replacement: "
                      << ResLenBeforeCombine
                      << " and after: " << ResLenAfterCombine << "\n");
    LLVM_DEBUG(
        ResLenAfterCombine <= ResLenBeforeCombine + TII->getExtendResourceLenLimit()
            ? dbgs() << "\t\t  As result it IMPROVES/PRESERVES Resource Length\n"
            : dbgs() << "\t\t  As result it DOES NOT improve/preserve Resource "
                        "Length\n");

    return ResLenAfterCombine <= ResLenBeforeCombine + TII->getExtendResourceLenLimit();
}
...
insertDeleteInstructions(MachineBasicBlock* MBB, MachineInstr& MI, SmallVectorImpl<MachineInstr*>& InsInstrs, 
    ...
    data.mf_size = MBB->getParent()->size();
    ...
    if (Pattern < 4U) {
        ...
        data.pattern = nico::to_string(static_cast<nico::MachineCombinerPattern2>(Pattern));
        ...
        data.pattern = nico::to_string(static_cast<nico::AArch64MachineCombinerPattern2>(Pattern)); //std::to_string(static_cast<int>(static_cast<nico::AArch64MachineCombinerPattern2>(Pattern)));
    } else {
...
void MachineCombiner::verifyPatternOrder(MachineBasicBlock* MBB,
    ...
    for (auto P : Patterns) {
        ...
        assert(CurrentLatencyDiff <= PrevLatencyDiff && "Current pattern is better than previous pattern.");
...
bool MachineCombiner::combineInstructions(MachineBasicBlock* MBB) {
    ...
    bool DoRegPressureReduce = TII->shouldReduceRegisterPressure(MBB, &RegClassInfo);
    ...
    while (BlockIter != MBB->end()) {
        ...
        for (const auto P : Patterns) {
            ...
            SmallVector<MachineInstr*, 16> DelInstrs;
            DenseMap<unsigned, unsigned> InstrIdxForVirtReg;
            TII->genAlternativeCodeSequence(MI, P, InsInstrs, DelInstrs, InstrIdxForVirtReg);
            ...
            if (DoRegPressureReduce && getCombinerObjective(P) == CombinerObjective::MustReduceRegisterPressure) {
                ...
                if (reduceRegisterPressure(MI, MBB, InsInstrs, DelInstrs, P)) {
                    ...
                    insertDeleteInstructions(MBB, MI, InsInstrs, DelInstrs, TraceEnsemble, RegUnits, TII, P, IncrementalUpdate);
                    Changed |= true;
...
bool MachineCombiner::runOnMachineFunction(MachineFunction& MF) {
    ...
    MBFI = (PSI && PSI->hasProfileSummary()) ? &getAnalysis<LazyMachineBlockFrequencyInfoPass>().getBFI() : nullptr;
