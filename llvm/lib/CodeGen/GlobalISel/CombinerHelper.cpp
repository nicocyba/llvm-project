...
// addressing.
static cl::opt<bool>
    ForceLegalIndexing("force-legal-indexing", cl::Hidden, cl::init(false), cl::desc("Force all indexed operations to be "
...
bool CombinerHelper::matchCombineExtendingLoads(
    ...
    unsigned PreferredOpcode = isa<GLoad>(&MI)
...
bool CombinerHelper::matchSextTruncSextLoad(MachineInstr& MI) const {
    ...
    Register TruncSrc;
    if (mi_match_wrapper3(SrcReg, MRI, m_GTrunc(m_Reg(TruncSrc)))) {
        LoadUser = TruncSrc;
    }
...
static cl::opt<unsigned> PostIndexUseThreshold(
    "post-index-use-threshold", cl::Hidden, cl::init(32), cl::desc("Number of uses of a base pointer to check before it is no longer "
...
bool CombinerHelper::findPostIndexCandidate(GLoadStore& LdSt, Register& Addr, Register& Base, Register& Offset, bool& RematOffset) const {
    ...
    for (auto& Use : MRI.use_nodbg_instructions(Ptr)) {
        ...
        if (!ForceLegalIndexing && !TLI.isIndexingLegal(LdSt, PtrAdd->getBaseReg(), Offset,
                /*IsPre*/ false, MRI)) {
            continue;
        }
...
bool CombinerHelper::findPreIndexCandidate(GLoadStore& LdSt, Register& Addr, Register& Base, Register& Offset) const {
    ...
    if (!mi_match_wrapper3(Addr, MRI, m_GPtrAdd(m_Reg(Base), m_Reg(Offset))) || MRI.hasOneNonDBGUse(Addr)) {
        return false;
    }
...
bool CombinerHelper::matchShiftImmedChain(MachineInstr& MI,
    ...
    unsigned Opcode = MI.getOpcode();
    assert((Opcode == TargetOpcode::G_SHL || Opcode == TargetOpcode::G_ASHR || Opcode == TargetOpcode::G_LSHR || Opcode == TargetOpcode::G_SSHLSAT || Opcode == TargetOpcode::G_USHLSAT) && "Expected G_SHL, G_ASHR, G_LSHR, G_SSHLSAT or G_USHLSAT");
...
bool CombinerHelper::matchShiftOfShiftedLogic(
    ...
    assert((ShiftOpcode == TargetOpcode::G_SHL || ShiftOpcode == TargetOpcode::G_ASHR || ShiftOpcode == TargetOpcode::G_LSHR || ShiftOpcode == TargetOpcode::G_USHLSAT || ShiftOpcode == TargetOpcode::G_SSHLSAT) && "Expected G_SHL, G_ASHR, G_LSHR, G_USHLSAT and G_SSHLSAT");
...
void CombinerHelper::applyShiftOfShiftedLogic(
    MachineInstr& MI, ShiftOfShiftedLogic& MatchInfo) const {
    unsigned Opcode = MI.getOpcode();
    assert((Opcode == TargetOpcode::G_SHL || Opcode == TargetOpcode::G_ASHR || Opcode == TargetOpcode::G_LSHR || Opcode == TargetOpcode::G_USHLSAT || Opcode == TargetOpcode::G_SSHLSAT) && "Expected G_SHL, G_ASHR, G_LSHR, G_USHLSAT and G_SSHLSAT");
...
bool CombinerHelper::matchCommuteShift(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(SrcReg, MRI,
    ...
    APInt C1Val, C2Val;
    if (!mi_match_wrapper3(C1, MRI, m_ICstOrSplat(C1Val)) || !mi_match_wrapper3(ShiftReg, MRI, m_ICstOrSplat(C2Val))) {
        return false;
    }
...
bool CombinerHelper::matchCombineShlOfExtend(MachineInstr& MI,
    ...
    Register ExtSrc;
    if (!mi_match_wrapper3(LHS, MRI, m_GAnyExt(m_Reg(ExtSrc))) && !mi_match_wrapper3(LHS, MRI, m_GZExt(m_Reg(ExtSrc))) && !mi_match_wrapper3(LHS, MRI, m_GSExt(m_Reg(ExtSrc)))) {
        return false;
    }
...
static Register peekThroughBitcast(Register Reg,
    const MachineRegisterInfo& MRI) {
    while (mi_match_wrapper3(Reg, MRI, m_GBitcast(m_Reg(Reg))))
        ;
...
bool CombinerHelper::matchCombineUnmergeZExtToZExt(MachineInstr& MI) const {
    ...
    Register ZExtSrcReg;
    if (!mi_match_wrapper3(SrcReg, MRI, m_GZExt(m_Reg(ZExtSrcReg)))) {
        return false;
    }
...
bool CombinerHelper::matchCombineI2PToP2I(MachineInstr& MI,
    ...
    Register SrcReg = MI.getOperand(1).getReg();
    if (mi_match_wrapper3(SrcReg, MRI, m_GPtrToInt(m_all_of(m_SpecificType(DstTy), m_Reg(Reg))))) {
        outs() << "\t\t\t\t\t" << getFunctionName(__PRETTY_FUNCTION__) << "\n";
        return true;
    }
    return false;
...
bool CombinerHelper::matchCombineAddP2IToPtrAdd(
    ...
    for (Register SrcReg : {LHS, RHS}) {
        if (mi_match_wrapper3(SrcReg, MRI, m_GPtrToInt(m_Reg(PtrReg.first)))) {
...
bool CombinerHelper::matchCombineConstPtrAddToI2P(MachineInstr& MI,
    ...
    if (auto RHSCst = getIConstantVRegVal(RHS, MRI)) {
        APInt Cst;
        if (mi_match_wrapper3(LHS, MRI, m_GIntToPtr(m_ICst(Cst)))) {
...
bool CombinerHelper::matchCombineAnyExtTrunc(MachineInstr& MI,
    ...
    LLT DstTy = MRI.getType(DstReg);
    if (mi_match_wrapper3(SrcReg, MRI, m_GTrunc(m_all_of(m_Reg(Reg), m_SpecificType(DstTy))))) {
        outs() << "\t\t\t\t\t" << getFunctionName(__PRETTY_FUNCTION__) << "\n";
        return true;
    }
    return false;
...
bool CombinerHelper::matchConstantFPOp(const MachineOperand& MOP,
    ...
    if (!mi_match_wrapper3(MOP.getReg(), MRI, m_GFCstOrSplat(MaybeCst))) {
        return false;
    }
...
bool CombinerHelper::matchSimplifyAddToSub(
    ...
    auto CheckFold = [&](Register& MaybeSub, Register& MaybeNewLHS) {
        if (!mi_match_wrapper3(MaybeSub, MRI, m_Neg(m_Reg(NewRHS)))) {
            return false;
        }
        NewLHS = MaybeNewLHS;
        return true;
...
bool CombinerHelper::matchAshrShlToSextInreg(
    ...
    Register Src;
    if (!mi_match_wrapper3(MI.getOperand(0).getReg(), MRI,
...
bool CombinerHelper::matchNotCmp(
    ...
    if (Ty.isVector()) {
        ...
        if (!mi_match_wrapper3(CstReg, MRI, m_ICst(Cst))) {
            return false;
        }
...
bool CombinerHelper::matchXorOfAndWithSameReg(
    ...
    if (!mi_match_wrapper3(AndReg, MRI, m_GAnd(m_Reg(X), m_Reg(Y)))) {
...
matchLoadAndBytePosition(Register Reg, unsigned MemSizeInBits, const MachineRegisterInfo& MRI) {
    assert(MRI.hasOneNonDBGUse(Reg) && "Expected Reg to only have one non-debug use?");
    Register MaybeLoad;
    int64_t Shift;
    if (!mi_match_wrapper3(Reg, MRI,
...
CombinerHelper::findLoadOffsetsForLoadOrCombine(
    ...
    for (auto Reg : RegsToVisit) {
        ...
        Register LoadPtr;
        int64_t Idx;
        if (!mi_match_wrapper3(Load->getOperand(1).getReg(), MRI,
...
bool CombinerHelper::matchOrShiftToFunnelShift(MachineInstr& MI,
    ...
    if (mi_match_wrapper3(ShlAmt, MRI, m_ICstOrSplat(CstShlAmt)) && mi_match_wrapper3(LShrAmt, MRI, m_ICstOrSplat(CstLShrAmt)) && CstShlAmt + CstLShrAmt == BitWidth) {
        FshOpc = TargetOpcode::G_FSHR;
        Amt = LShrAmt;

    } else if (mi_match_wrapper3(LShrAmt, MRI,
                   m_GSub(m_SpecificICstOrSplat(BitWidth), m_Reg(Amt)))
        && ShlAmt == Amt) {
        // (or (shl x, amt), (lshr y, (sub bw, amt))) -> (fshl x, y, amt)
        FshOpc = TargetOpcode::G_FSHL;

    } else if (mi_match_wrapper3(ShlAmt, MRI,
                   m_GSub(m_SpecificICstOrSplat(BitWidth), m_Reg(Amt)))
        && LShrAmt == Amt) {
...
bool CombinerHelper::matchICmpToLHSKnownBits(
    ...
    if (!mi_match_wrapper3(MI.getOperand(3).getReg(), MRI, m_SpecificICst(OneOrZero))) {
        return false;
    }
...
bool CombinerHelper::matchBitfieldExtractFromAnd(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(And->getReg(0), MRI,
...
bool CombinerHelper::matchReassocConstantInnerLHS(GPtrAdd& MI,
    ...
    if (!mi_match_wrapper3(MI.getBaseReg(), MRI,
...
bool CombinerHelper::matchReassocFoldConstantsInSubTree(
    GPtrAdd& MI, MachineInstr* LHS, MachineInstr* RHS, BuildFnTy& MatchInfo) const {
...
bool CombinerHelper::matchMulOBy2(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(MI.getOperand(3).getReg(), MRI, m_SpecificICstOrSplat(2))) {
        return false;
    }
...
bool CombinerHelper::matchMulOBy0(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(MI.getOperand(3).getReg(), MRI, m_SpecificICstOrSplat(0))) {
        return false;
    }
    Register Dst = MI.getOperand(0).getReg();
...
bool CombinerHelper::matchAddEToAddO(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(MI.getOperand(4).getReg(), MRI, m_SpecificICstOrSplat(0))) {
        return false;
    }
    MatchInfo = [&](MachineIRBuilder& B) {
        unsigned NewOpcode;
...
bool CombinerHelper::matchSubAddSameReg(MachineInstr& MI,
    ...
    // (x + y) - z -> y (if x == z)
    Register X, Y, Z;
    if (mi_match_wrapper3(Dst, MRI, m_GSub(m_GAdd(m_Reg(X), m_Reg(Y)), m_Reg(Z)))) {
        Register ReplaceReg;
        int64_t CstX, CstY;
        if (Y == Z || (mi_match_wrapper3(Y, MRI, m_ICstOrSplat(CstY)) && mi_match_wrapper3(Z, MRI, m_SpecificICstOrSplat(CstY)))) {
            ReplaceReg = X;
        } else if (X == Z || (mi_match_wrapper3(X, MRI, m_ICstOrSplat(CstX)) && mi_match_wrapper3(Z, MRI, m_SpecificICstOrSplat(CstX)))) {
            ReplaceReg = Y;
        }
        if (ReplaceReg) {
    ...
    // x - (y + z) -> 0 - y (if x == z)
    // x - (y + z) -> 0 - z (if x == y)
    if (mi_match_wrapper3(Dst, MRI, m_GSub(m_Reg(X), m_GAdd(m_Reg(Y), m_Reg(Z))))) {
        Register ReplaceReg;
        int64_t CstX;
        if (X == Z || (mi_match_wrapper3(X, MRI, m_ICstOrSplat(CstX)) && mi_match_wrapper3(Z, MRI, m_SpecificICstOrSplat(CstX)))) {
            ReplaceReg = Y;
        } else if (X == Y || (mi_match_wrapper3(X, MRI, m_ICstOrSplat(CstX)) && mi_match_wrapper3(Y, MRI, m_SpecificICstOrSplat(CstX)))) {
            ReplaceReg = Z;
        }
        if (ReplaceReg) {
            MatchInfo = [=](MachineIRBuilder& B) {
...
bool CombinerHelper::matchRedundantNegOperands(MachineInstr& MI,
    ...
    if (mi_match_wrapper3(Dst, MRI, m_GFAdd(m_Reg(X), m_GFNeg(m_Reg(Y)))) && isLegalOrBeforeLegalizer({TargetOpcode::G_FSUB, {Type}})) {
        Opc = TargetOpcode::G_FSUB;
    }
    /// fold (fsub x, fneg(y)) -> (fadd x, y)
    else if (mi_match_wrapper3(Dst, MRI, m_GFSub(m_Reg(X), m_GFNeg(m_Reg(Y)))) && isLegalOrBeforeLegalizer({TargetOpcode::G_FADD, {Type}})) {
    ...
    else if ((Opc == TargetOpcode::G_FMUL || Opc == TargetOpcode::G_FDIV || Opc == TargetOpcode::G_FMAD || Opc == TargetOpcode::G_FMA) && mi_match_wrapper3(X, MRI, m_GFNeg(m_Reg(X))) && mi_match_wrapper3(Y, MRI, m_GFNeg(m_Reg(Y)))) {
        // no opcode change
    } else {
        return false;
    }
...
bool CombinerHelper::matchCombineFAddFpExtFMulToFMadOrFMA(
    ...
    if (mi_match_wrapper3(LHS.Reg, MRI, m_GFPExt(m_MInstr(FpExtSrc))) && isContractableFMul(*FpExtSrc, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstType, MRI.getType(FpExtSrc->getOperand(1).getReg()))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
    ...
    if (mi_match_wrapper3(RHS.Reg, MRI, m_GFPExt(m_MInstr(FpExtSrc))) && isContractableFMul(*FpExtSrc, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstType, MRI.getType(FpExtSrc->getOperand(1).getReg()))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
            auto FpExtX = B.buildFPExt(DstType, FpExtSrc->getOperand(1).getReg());
            auto FpExtY = B.buildFPExt(DstType, FpExtSrc->getOperand(2).getReg());
            B.buildInstr(PreferredFusedOpcode, {MI.getOperand(0).getReg()},
...
bool CombinerHelper::matchCombineFAddFMAFMulToFMadOrFMA(
    ...
    if (LHS.MI->getOpcode() == PreferredFusedOpcode && (MRI.getVRegDef(LHS.MI->getOperand(3).getReg())->getOpcode() == TargetOpcode::G_FMUL) && MRI.hasOneNonDBGUse(LHS.MI->getOperand(0).getReg()) && MRI.hasOneNonDBGUse(LHS.MI->getOperand(3).getReg())) {
        FMA = LHS.MI;
        Z = RHS.Reg;
    }
    // fold (fadd z, (fma x, y, (fmul u, v))) -> (fma x, y, (fma u, v, z))
    else if (RHS.MI->getOpcode() == PreferredFusedOpcode && (MRI.getVRegDef(RHS.MI->getOperand(3).getReg())->getOpcode() == TargetOpcode::G_FMUL) && MRI.hasOneNonDBGUse(RHS.MI->getOperand(0).getReg()) && MRI.hasOneNonDBGUse(RHS.MI->getOperand(3).getReg())) {
        Z = LHS.Reg;
        FMA = RHS.MI;
    }
...
bool CombinerHelper::matchCombineFAddFpExtFMulToFMadOrFMAAggressive(
    ...
    auto buildMatchInfo = [=, &MI](Register U, Register V, Register Z, Register X,
        ...
        Register InnerFMA = B.buildInstr(PreferredFusedOpcode, {DstType}, {FpExtU, FpExtV, Z})
                                .getReg(0);
        B.buildInstr(PreferredFusedOpcode, {MI.getOperand(0).getReg()},
            {X, Y, InnerFMA});
    };
    ...
    if (LHS.MI->getOpcode() == PreferredFusedOpcode && mi_match_wrapper3(LHS.MI->getOperand(3).getReg(), MRI, m_GFPExt(m_MInstr(FMulMI))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstType, MRI.getType(FMulMI->getOperand(0).getReg()))) {
        MatchInfo = [=](MachineIRBuilder& B) {
    ...
    if (mi_match_wrapper3(LHS.Reg, MRI, m_GFPExt(m_MInstr(FMAMI))) && FMAMI->getOpcode() == PreferredFusedOpcode) {
    ...
    //   -> (fma x, y, (fma (fpext u), (fpext v), z))
    if (RHS.MI->getOpcode() == PreferredFusedOpcode && mi_match_wrapper3(RHS.MI->getOperand(3).getReg(), MRI, m_GFPExt(m_MInstr(FMulMI))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstType, MRI.getType(FMulMI->getOperand(0).getReg()))) {
        MatchInfo = [=](MachineIRBuilder& B) {
            buildMatchInfo(FMulMI->getOperand(1).getReg(),
    ...
    // operation into two double-precision operations, which might not be
    // interesting for all targets, especially GPUs.
    if (mi_match_wrapper3(RHS.Reg, MRI, m_GFPExt(m_MInstr(FMAMI))) && FMAMI->getOpcode() == PreferredFusedOpcode) {
        MachineInstr* FMulMI = MRI.getVRegDef(FMAMI->getOperand(3).getReg());
        if (isContractableFMul(*FMulMI, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstType, MRI.getType(FMAMI->getOperand(0).getReg()))) {
            MatchInfo = [=](MachineIRBuilder& B) {
                Register X = FMAMI->getOperand(1).getReg();
...
bool CombinerHelper::matchCombineFSubFNegFMulToFMadOrFMA(
    ...
    if (mi_match_wrapper3(LHSReg, MRI, m_GFNeg(m_MInstr(FMulMI))) && (Aggressive || (MRI.hasOneNonDBGUse(LHSReg) && MRI.hasOneNonDBGUse(FMulMI->getOperand(0).getReg()))) && isContractableFMul(*FMulMI, AllowFusionGlobally)) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
    ...
    if (mi_match_wrapper3(RHSReg, MRI, m_GFNeg(m_MInstr(FMulMI))) && (Aggressive || (MRI.hasOneNonDBGUse(RHSReg) && MRI.hasOneNonDBGUse(FMulMI->getOperand(0).getReg()))) && isContractableFMul(*FMulMI, AllowFusionGlobally)) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
            B.buildInstr(PreferredFusedOpcode, {MI.getOperand(0).getReg()},
...
bool CombinerHelper::matchCombineFSubFpExtFMulToFMadOrFMA(
    ...
    if (mi_match_wrapper3(LHSReg, MRI, m_GFPExt(m_MInstr(FMulMI))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && (Aggressive || MRI.hasOneNonDBGUse(LHSReg))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
    ...
    if (mi_match_wrapper3(RHSReg, MRI, m_GFPExt(m_MInstr(FMulMI))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && (Aggressive || MRI.hasOneNonDBGUse(RHSReg))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
...
bool CombinerHelper::matchCombineFSubFpExtFNegFMulToFMadOrFMA(
    ...
    //      (fneg (fma (fpext x), (fpext y), z))
    if ((mi_match_wrapper3(LHSReg, MRI, m_GFPExt(m_GFNeg(m_MInstr(FMulMI)))) || mi_match_wrapper3(LHSReg, MRI, m_GFNeg(m_GFPExt(m_MInstr(FMulMI))))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstTy, MRI.getType(FMulMI->getOperand(0).getReg()))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
    ...
    if ((mi_match_wrapper3(RHSReg, MRI, m_GFPExt(m_GFNeg(m_MInstr(FMulMI)))) || mi_match_wrapper3(RHSReg, MRI, m_GFNeg(m_GFPExt(m_MInstr(FMulMI))))) && isContractableFMul(*FMulMI, AllowFusionGlobally) && TLI.isFPExtFoldable(MI, PreferredFusedOpcode, DstTy, MRI.getType(FMulMI->getOperand(0).getReg()))) {
        MatchInfo = [=, &MI](MachineIRBuilder& B) {
...
bool CombinerHelper::matchAddSubSameReg(MachineInstr& MI, Register& Src) const {
    ...
    auto CheckFold = [&](Register MaybeSub, Register MaybeSameReg) {
        Register Reg;
        return mi_match_wrapper3(MaybeSub, MRI, m_GSub(m_Reg(Src), m_Reg(Reg))) && Reg == MaybeSameReg;
    };
    if (CheckFold(LHS, RHS) || CheckFold(RHS, LHS)) {
        outs() << "\t\t\t\t\t" << getFunctionName(__PRETTY_FUNCTION__) << "\n";
        return true;
...
bool CombinerHelper::matchBuildVectorIdentityFold(MachineInstr& MI,
    ...
    if (mi_match_wrapper3(MI, MRI, m_GBuildVector(m_GTrunc(m_GBitcast(m_Reg(Lo))), m_GImplicitDef()))) {
        MatchInfo = Lo;
...
bool CombinerHelper::matchTruncBuildVectorFold(MachineInstr& MI,
    ...
    if (!mi_match_wrapper3(MI.getOperand(1).getReg(), MRI,
...
unsigned CombinerHelper::getFPMinMaxOpcForSelect(
    CmpInst::Predicate Pred, LLT DstTy, SelectPatternNaNBehaviour VsNaNRetVal) const {
...
CombinerHelper::computeRetValAgainstNaN(Register LHS, Register RHS, bool IsOrderedComparison) const {
...
bool CombinerHelper::matchSimplifySelectToMinMax(MachineInstr& MI,
    ...
    Register MaybeTrunc;
    if (mi_match_wrapper3(Cond, MRI, m_OneNonDBGUse(m_GTrunc(m_Reg(MaybeTrunc))))) {
        Cond = MaybeTrunc;
    }
    Register Dst = MI.getOperand(0).getReg();
...
bool CombinerHelper::matchRedundantBinOpInEquality(MachineInstr& MI,
    ...
    bool MatchedSub = mi_match_wrapper3(
        Dst, MRI,
...
getMinUselessShift(KnownBits ValueKB, unsigned Opcode, std::optional<int64_t>& Result) {
...
bool CombinerHelper::matchCommuteFPConstantToRHS(MachineInstr& MI) const {
    ...
    if (!mi_match_wrapper3(LHS, MRI, m_GFCstOrSplat(ValAndVReg))) {
        return false;
    }
    if (!mi_match_wrapper3(RHS, MRI, m_GFCstOrSplat(ValAndVReg))) {
        outs() << "\t\t\t\t\t" << getFunctionName(__PRETTY_FUNCTION__) << "\n";
        return true;
    }
    return false;
...
bool CombinerHelper::matchSimplifyNegMinMax(MachineInstr& MI,
    ...
    if (mi_match_wrapper3(DestReg, MRI,
...
bool CombinerHelper::tryFoldLogicOfFCmps(GLogicalBinOp* Logic,
    ...
    if (!isLegalOrBeforeLegalizer(
        ...
        || !MRI.hasOneNonDBGUse(Logic->getReg(0)) || !MRI.hasOneNonDBGUse(Cmp1->getReg(0)) || !MRI.hasOneNonDBGUse(Cmp2->getReg(0)) || MRI.getType(Cmp1->getLHSReg()) != MRI.getType(Cmp2->getLHSReg())) {
        return false;
    }
