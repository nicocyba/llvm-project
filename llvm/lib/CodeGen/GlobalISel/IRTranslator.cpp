...
INITIALIZE_PASS_BEGIN(IRTranslator, DEBUG_TYPE, "IRTranslator LLVM IR -> MI", false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(GISelCSEAnalysisWrapperPass)
INITIALIZE_PASS_DEPENDENCY(BlockFrequencyInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(StackProtector)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(IRTranslator, DEBUG_TYPE, "IRTranslator LLVM IR -> MI", false, false)
...
public:
    ...
    void createdInstr(MachineInstr& MI) override {
        ...
        // debug experience.
        assert((CurrInst->getDebugLoc() == MI.getDebugLoc() || (MI.getParent()->isEntryBlock() && !MI.getDebugLoc()) || (MI.isDebugInstr())) && "Line info was not transferred to all instructions");
    }
};
} // namespace
#endif // ifndef NDEBUG
...
int IRTranslator::getOrCreateFrameIndex(const AllocaInst& AI) {
    ...
    uint64_t Size = ElementSize * cast<ConstantInt>(AI.getArraySize())->getZExtValue();
...
static bool containsBF16Type(const User& U) {
    ...
    // supports bfloat.
    return U.getType()->getScalarType()->isBFloatTy() || any_of(U.operands(), [](Value* V) {
        return V->getType()->getScalarType()->isBFloatTy();
    });
}

bool IRTranslator::translateBinaryOp(unsigned Opcode, const User& U, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::translateUnaryOp(unsigned Opcode, const User& U, MachineIRBuilder& MIRBuilder) {
    if (containsBF16Type(U)) {
        return false;
    }
...
void IRTranslator::emitBranchForMergedCondition(
    const Value* Cond, MachineBasicBlock* TBB, MachineBasicBlock* FBB, MachineBasicBlock* CurBB, MachineBasicBlock* SwitchBB, BranchProbability TProb, BranchProbability FProb, bool InvertCond) {
...
void IRTranslator::findMergedConditions(
    const Value* Cond, MachineBasicBlock* TBB, MachineBasicBlock* FBB, MachineBasicBlock* CurBB, MachineBasicBlock* SwitchBB, Instruction::BinaryOps Opc, BranchProbability TProb, BranchProbability FProb, bool InvertCond) {
    using namespace PatternMatch;
    assert((Opc == Instruction::And || Opc == Instruction::Or) && "Expected Opc to be AND/OR");
    // Skip over not part of the tree and remember to invert op and operands at
    // next level.
    Value* NotCond;
    if (match(Cond, m_OneUse(m_Not(m_Value(NotCond)))) && isValInBlock(NotCond, CurBB->getBasicBlock())) {
    ...
    if (!BOpIsInOrAndTree || BOp->getParent() != CurBB->getBasicBlock() || !isValInBlock(BOpOp0, CurBB->getBasicBlock()) || !isValInBlock(BOpOp1, CurBB->getBasicBlock())) {
...
bool IRTranslator::shouldEmitAsBranches(
    ...
    // Handle: (X == null) & (Y == null) --> (X|Y) == 0
    if (Cases[0].CmpRHS == Cases[1].CmpRHS && Cases[0].PredInfo.Pred == Cases[1].PredInfo.Pred && isa<Constant>(Cases[0].CmpRHS) && cast<Constant>(Cases[0].CmpRHS)->isNullValue()) {
        if (Cases[0].PredInfo.Pred == CmpInst::ICMP_EQ && Cases[0].TrueBB == Cases[1].ThisBB) {
            return false;
        }
        if (Cases[0].PredInfo.Pred == CmpInst::ICMP_NE && Cases[0].FalseBB == Cases[1].ThisBB) {
            return false;
        }
    }
...
bool IRTranslator::translateBr(const User& U, MachineIRBuilder& MIRBuilder) {
    ...
    if (!TLI->isJumpExpensive() && CondI && CondI->hasOneUse() && !BrInst.hasMetadata(LLVMContext::MD_unpredictable)) {
        ...
        if (Opcode && !(match(BOp0, m_ExtractElt(m_Value(Vec), m_Value())) && match(BOp1, m_ExtractElt(m_Specific(Vec), m_Value())))) {
...
bool IRTranslator::translateSwitch(const User& U, MachineIRBuilder& MIB) {
    ...
    for (const auto& I : SI.cases()) {
        ...
        BranchProbability Prob = BPI ? BPI->getEdgeProbability(SI.getParent(), I.getSuccessorIndex())
    ...
    while (!WorkList.empty()) {
        ...
        if (NumClusters > 3 && MF->getTarget().getOptLevel() != CodeGenOptLevel::None && !DefaultMBB->getParent()->getFunction().hasMinSize()) {
...
void IRTranslator::splitWorkItem(SwitchCG::SwitchWorkList& WorkList,
    ...
    using namespace SwitchCG;
    assert(W.FirstCluster->Low->getValue().slt(W.LastCluster->Low->getValue()) && "Clusters not sorted?");
    assert(W.LastCluster - W.FirstCluster + 1 >= 2 && "Too small to split!");

    auto [LastLeft, FirstRight, LeftProb, RightProb] = SL->computeSplitWorkItemInfo(W);
    ...
    if (FirstLeft == LastLeft && FirstLeft->Kind == CC_Range && FirstLeft->Low == W.GE && (FirstLeft->High->getValue() + 1LL) == Pivot->getValue()) {
    ...
    if (FirstRight == LastRight && FirstRight->Kind == CC_Range && W.LT && (FirstRight->High->getValue() + 1ULL) == W.LT->getValue()) {
    ...
    if (!CB.CmpMHS) {
        ...
        if (MRI->getType(CondLHS).getSizeInBits() == 1 && CI && CI->isOne() && CB.PredInfo.Pred == CmpInst::ICMP_EQ) {
            ...
            if (CmpInst::isFPPredicate(CB.PredInfo.Pred)) {
                Cond = MIB.buildFCmp(CB.PredInfo.Pred, i1Ty, CondLHS, CondRHS).getReg(0);
            } else {
                Cond = MIB.buildICmp(CB.PredInfo.Pred, i1Ty, CondLHS, CondRHS).getReg(0);
            }
        }
    } else {
        assert(CB.PredInfo.Pred == CmpInst::ICMP_SLE && "Can only handle SLE ranges");
        ...
        if (cast<ConstantInt>(CB.CmpLHS)->isMinValue(true)) {
            Register CondRHS = getOrCreateVReg(*CB.CmpRHS);
            Cond = MIB.buildICmp(CmpInst::ICMP_SLE, i1Ty, CmpOpReg, CondRHS).getReg(0);
        } else {
...
void IRTranslator::emitBitTestHeader(SwitchCG::BitTestBlock& B,
    ...
    if (MaskTy.getSizeInBits() > PtrTy.getSizeInBits() || !llvm::has_single_bit<uint32_t>(MaskTy.getSizeInBits())) {
...
void IRTranslator::emitBitTestCase(SwitchCG::BitTestBlock& BB,
    ...
    if (PopCount == 1) {
        ...
        auto MaskTrailingZeros = MIB.buildConstant(SwitchTy, llvm::countr_zero(B.Mask));
        Cmp = MIB.buildICmp(ICmpInst::ICMP_EQ, LLT::scalar(1), Reg, MaskTrailingZeros)
                  .getReg(0);
    } else if (PopCount == BB.Range) {
        // There is only one zero bit in the range, test for it directly.
        auto MaskTrailingOnes = MIB.buildConstant(SwitchTy, llvm::countr_one(B.Mask));
...
bool IRTranslator::translateLoad(const User& U, MachineIRBuilder& MIRBuilder) {
    ...
    if (CLI->supportSwiftError() && isSwiftError(Ptr)) {
        ...
        Register VReg = SwiftError.getOrCreateVRegUseAt(&LI, &MIRBuilder.getMBB(), Ptr);
        MIRBuilder.buildCopy(Regs[0], VReg);
        return true;
    }

    MachineMemOperand::Flags Flags = TLI->getLoadMemOperandFlags(LI, *DL, AC, LibInfo);
    ...
    const MDNode* Ranges = Regs.size() == 1 ? LI.getMetadata(LLVMContext::MD_range) : nullptr;
...
static uint64_t getOffsetFromIndices(const User& U, const DataLayout& DL) {
    ...
    return 8 * static_cast<uint64_t>(DL.getIndexedOffsetInType(Src->getType(), Indices));
}
...
bool IRTranslator::translateCopy(const User& U, const Value& V, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::translateBitCast(const User& U,
    ...
    if (getLLTForType(*U.getOperand(0)->getType(), *DL) == getLLTForType(*U.getType(), *DL)) {
...
bool IRTranslator::translateCast(unsigned Opcode, const User& U, MachineIRBuilder& MIRBuilder) {
    if (containsBF16Type(U)) {
        return false;
    }
    ...
    for (gep_type_iterator GTI = gep_type_begin(&U), E = gep_type_end(&U);
        ...
        if (StructType* StTy = GTI.getStructTypeOrNull()) {
            ...
            if (ElementSize != 1) {
                ...
                GepOffsetReg = MIRBuilder.buildMul(OffsetTy, IdxReg, ElementSizeMIB).getReg(0);
            } else {
                GepOffsetReg = IdxReg;
            }
...
bool IRTranslator::translateMemFunc(const CallInst& CI,
    ...
    Align DstAlign;
    Align SrcAlign;
    unsigned IsVol = cast<ConstantInt>(CI.getArgOperand(CI.arg_size() - 1))->getZExtValue();
    ...
    if (AA && CopySize && AA->pointsToConstantMemory(MemoryLocation(SrcPtr, LocationSize::precise(CopySize->getZExtValue()), AAInfo))) {
...
bool IRTranslator::translateTrap(const CallInst& CI,
    MachineIRBuilder& MIRBuilder,
    unsigned Opcode) {
    StringRef TrapFuncName = CI.getAttributes().getFnAttr("trap-func-name").getValueAsString();
...
bool IRTranslator::translateVectorInterleave2Intrinsic(
    ...
    assert(CI.getIntrinsicID() == Intrinsic::vector_interleave2 && "This function can only be called on the interleave2 intrinsic!");
...
bool IRTranslator::translateVectorDeinterleave2Intrinsic(
    const CallInst& CI, MachineIRBuilder& MIRBuilder) {
    assert(CI.getIntrinsicID() == Intrinsic::vector_deinterleave2 && "This function can only be called on the deinterleave2 intrinsic!");
...
void IRTranslator::getStackGuard(Register DstReg,
    ...
    auto MIB = MIRBuilder.buildInstr(TargetOpcode::LOAD_STACK_GUARD, {DstReg}, {});
    ...
    auto Flags = MachineMemOperand::MOLoad | MachineMemOperand::MOInvariant | MachineMemOperand::MODereferenceable;
    MachineMemOperand* MemRef = MF->getMachineMemOperand(
        MPInfo, Flags, PtrTy, DL->getPointerABIAlignment(AddrSpace));
    MIB.setMemRefs({MemRef});
}

bool IRTranslator::translateOverflowIntrinsic(const CallInst& CI, unsigned Op, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::translateFixedPointIntrinsic(unsigned Op, const CallInst& CI, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::translateIfEntryValueArgument(bool isDeclare, Value* Val, const DILocalVariable* Var, const DIExpression* Expr, const DebugLoc& DL, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::translateConvergenceControlIntrinsic(
    ...
    if (ID == Intrinsic::experimental_convergence_loop) {
        ...
        Register InputReg = getOrCreateConvergenceTokenVReg(*Bundle->Inputs[0].get());
        MIB.addUse(InputReg);
    }
...
bool IRTranslator::translateKnownIntrinsic(const CallInst& CI, Intrinsic::ID ID, MachineIRBuilder& MIRBuilder) {
    ...
    switch (ID) {
        ...
        case Intrinsic::lifetime_end: {
            ...
            if (MF->getTarget().getOptLevel() == CodeGenOptLevel::None || MF->getFunction().hasOptNone()) {
                return true;
            }
        ...
        case Intrinsic::fmuladd: {
            ...
            if (TM.Options.AllowFPOpFusion != FPOpFusion::Strict && TLI->isFMAFasterThanFMulAndFAdd(*MF, TLI->getValueType(*DL, CI.getType()))) {
        ...
        case Intrinsic::localescape: {
            ...
            for (unsigned Idx = 0, E = CI.arg_size(); Idx < E; ++Idx) {
                ...
                MCSymbol* FrameAllocSym = MF->getContext().getOrCreateFrameAllocSymbol(EscapedName, Idx);

                // This should be inserted at the start of the entry block.
                auto LocalEscape = MIRBuilder.buildInstrNoInsert(TargetOpcode::LOCAL_ESCAPE)
        ...
        case Intrinsic::fptrunc_round: {
            ...
            std::optional<RoundingMode> RoundMode = convertStrToRoundingMode(cast<MDString>(MD)->getString());
...
bool IRTranslator::translateCallBase(const CallBase& CB,
    ...
    for (const auto& Arg : CB.args()) {
        if (CLI->supportSwiftError() && isSwiftError(Arg)) {
            ...
            MIRBuilder.buildCopy(SwiftInVReg, SwiftError.getOrCreateVRegUseAt(&CB, &MIRBuilder.getMBB(), Arg));
            Args.emplace_back(ArrayRef(SwiftInVReg));
            SwiftErrorVReg = SwiftError.getOrCreateVRegDefAt(&CB, &MIRBuilder.getMBB(), Arg);
            continue;
        }
    ...
    if (auto Bundle = CB.getOperandBundle(LLVMContext::OB_ptrauth)) {
        ...
        if (!CalleeCPA || !isa<Function>(CalleeCPA->getPointer()) || !CalleeCPA->isKnownCompatibleWith(Key, Discriminator, *DL)) {
...
bool IRTranslator::translateCall(const User& U, MachineIRBuilder& MIRBuilder) {
    ...
    // weak symbols.
    if (F && (F->hasDLLImportStorageClass() || (MF->getTarget().getTargetTriple().isOSWindows() && F->hasExternalWeakLinkage()))) {
        return false;
    }
    ...
    for (const auto& Arg : enumerate(CI.args())) {
        ...
        if (CI.paramHasAttr(Arg.index(), Attribute::ImmArg)) {
            if (ConstantInt* CI = dyn_cast<ConstantInt>(Arg.value())) {
                ...
                assert(CI->getBitWidth() <= 64 && "large intrinsic immediates not handled");
...
bool IRTranslator::translateInvoke(const User& U,
    ...
    // FIXME: support Windows dllimport function calls and calls through
    // weak symbols.
    if (Fn && (Fn->hasDLLImportStorageClass() || (MF->getTarget().getTargetTriple().isOSWindows() && Fn->hasExternalWeakLinkage()))) {
        return false;
    }
    ...
    BranchProbability EHPadBBProb = BPI ? BPI->getEdgeProbability(InvokeMBB->getBasicBlock(), EHPadBB)
...
bool IRTranslator::translateLandingPad(const User& U,
    ...
    if (TLI->getExceptionPointerRegister(PersonalityFn) == 0 && TLI->getExceptionSelectorRegister(PersonalityFn) == 0) {
        return true;
    }
...
bool IRTranslator::translateAlloca(const User& U,
    ...
    Register TySize = getOrCreateVReg(*ConstantInt::get(IntPtrIRTy, DL->getTypeAllocSize(Ty)));
    ...
    auto AlignCst = MIRBuilder.buildConstant(IntPtrTy, ~(uint64_t)(StackAlign.value() - 1));
...
bool IRTranslator::translateExtractElement(const User& U,
    ...
    // If it is a <1 x Ty> vector, use the scalar as it is
    // not a legal vector type in LLT.
    if (const FixedVectorType* FVT = dyn_cast<FixedVectorType>(U.getOperand(0)->getType())) {
...
bool IRTranslator::translateFreeze(const User& U,
    ...
    assert(DstRegs.size() == SrcRegs.size() && "Freeze with different source and destination type?");
...
void IRTranslator::translateDbgValueRecord(Value* V, bool HasArgList, const DILocalVariable* Variable, const DIExpression* Expression, const DebugLoc& DL, MachineIRBuilder& MIRBuilder) {
    assert(Variable->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
    ...
    if (auto* AI = dyn_cast<AllocaInst>(V);
        ...
        auto* ExprDerefRemoved = DIExpression::get(AI->getContext(), ExprOperands.drop_front());
...
void IRTranslator::translateDbgDeclareRecord(Value* Address, bool HasArgList, const DILocalVariable* Variable, const DIExpression* Expression, const DebugLoc& DL, MachineIRBuilder& MIRBuilder) {
...
bool IRTranslator::finalizeBasicBlock(const BasicBlock& BB,
    ...
    if (SP.shouldEmitSDCheck(BB)) {
        bool FunctionBasedInstrumentation = TLI->getSSPStackGuardCheck(*MF->getFunction().getParent());
...
bool IRTranslator::emitSPDescriptorParent(StackProtectorDescriptor& SPD,
    ...
    if (TLI->useLoadStackGuardNode(*ParentBB->getBasicBlock()->getModule())) {
        Guard = MRI->createGenericVirtualRegister(LLT::scalar(PtrTy.getSizeInBits()));
    ...
    auto Cmp = CurBuilder->buildICmp(CmpInst::ICMP_NE, LLT::scalar(1), Guard, GuardVal);
...
bool IRTranslator::runOnMachineFunction(MachineFunction& CurMF) {
    current_stage = IRTRANSLATOR;
    outs() << "Running IRTranslator on function: " << CurMF.getFunction().getName() << "\n";
    ...
    // be maximal.
    assert(EntryBB->succ_size() == 1 && "Custom BB used for lowering should have only one successor");
    // Get the successor of the current entry block.
    MachineBasicBlock& NewEntryBB = **EntryBB->succ_begin();
    assert(NewEntryBB.pred_size() == 1 && "LLVM-IR entry block has a predecessor!?");
    ...
    assert(&MF->front() == &NewEntryBB && "New entry wasn't next in the list of basic block!");
