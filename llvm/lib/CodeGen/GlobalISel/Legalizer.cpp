...
static cl::opt<bool> AllowGInsertAsArtifact("allow-ginsert-as-artifact",
...
#ifndef NDEBUG
static cl::opt<DebugLocVerifyLevel> VerifyDebugLocs("verify-legalizer-debug-locs",
    cl::desc("Verify that debug locations are handled"),
    cl::values(clEnumValN(DebugLocVerifyLevel::None, "none", "No verification"),
        clEnumValN(DebugLocVerifyLevel::Legalizations, "legalizations", "Verify legalizations"),
        clEnumValN(DebugLocVerifyLevel::LegalizationsAndArtifactCombiners, "legalizations+artifactcombiners", "Verify legalizations and artifact combines")),
...
char Legalizer::ID = 0;
INITIALIZE_PASS_BEGIN(Legalizer, DEBUG_TYPE, "Legalize the Machine IR a function's Machine IR", false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(GISelCSEAnalysisWrapperPass)
INITIALIZE_PASS_DEPENDENCY(GISelKnownBitsAnalysis)
INITIALIZE_PASS_END(Legalizer, DEBUG_TYPE, "Legalize the Machine IR a function's Machine IR", false, false)
...
public:
    LegalizerWorkListManager(InstListTy& Insts, ArtifactListTy& Arts) : InstList(Insts), ArtifactList(Arts) {}
    ...
    void changingInstr(MachineInstr& MI) override { LLVM_DEBUG(dbgs() << ".. .. Changing MI: " << MI); }
...
Legalizer::MFResult Legalizer::legalizeMachineFunction(
    MachineFunction& MF, const LegalizerInfo& LI, ArrayRef<GISelChangeObserver*> AuxObservers, LostDebugLocObserver& LocObserver, MachineIRBuilder& MIRBuilder, GISelKnownBits* KB) {
    MIRBuilder.setMF(MF);
    ...
    do {
        ...
        while (!ArtifactList.empty()) {
            ...
            if (ArtCombiner.tryCombineInstruction(MI, DeadInstructions, WrapperObserver)) {
                WorkListObserver.printNewInstrs();
                eraseInstrs(DeadInstructions, MRI, &LocObserver);
                LocObserver.checkpoint(VerifyDebugLocs == DebugLocVerifyLevel::LegalizationsAndArtifactCombiners);
                Changed = true;
                continue;
            }
...
bool Legalizer::runOnMachineFunction(MachineFunction& MF) {
    ...
    bool EnableCSE = EnableCSEInLegalizer.getNumOccurrences() ? EnableCSEInLegalizer : TPC.isGISelCSEEnabled();
    if (EnableCSE) {
    ...
    MFResult Result = legalizeMachineFunction(MF, LI, AuxObservers, LocObserver, *MIRBuilder, KB);

    if (Result.FailedOn) {
        reportGISelFailure(MF, TPC, MORE, "gisel-legalize", "unable to legalize instruction", *Result.FailedOn);
        return false;
    }

    if (LocObserver.getNumLostDebugLocs()) {
        MachineOptimizationRemarkMissed R("gisel-legalize", "LostDebugLoc", MF.getFunction().getSubprogram(),
            /*MBB=*/&*MF.begin());
        R << "lost " << ore::NV("NumLostDebugLocs", LocObserver.getNumLostDebugLocs()) << " debug locations during pass";
