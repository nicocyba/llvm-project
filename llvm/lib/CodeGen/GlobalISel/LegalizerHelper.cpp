...
getNarrowTypeBreakDown(LLT OrigTy, LLT NarrowTy, LLT& LeftoverTy) {
    ...
    if (NarrowTy.isVector()) {
        ...
        LeftoverTy = LLT::scalarOrVector(ElementCount::getFixed(LeftoverSize / EltSize),
...
LegalizerHelper::LegalizerHelper(MachineFunction& MF, const LegalizerInfo& LI, GISelChangeObserver& Observer, MachineIRBuilder& B, GISelKnownBits* KB)
...
LegalizerHelper::LegalizeResult LegalizerHelper::legalizeInstrStep(MachineInstr& MI,
    ...
    if (isa<GIntrinsic>(MI)) {
        ...
        data.event = std::format("{} - {}", __func__, actionToStr);
        ...
        switch (Step.Action) {
            ...
            case Libcall:
                ...
                Result = libcall(MI, LocObserver);
                break;
            case NarrowScalar:
                LLVM_DEBUG(dbgs() << ".. Narrow scalar\n");
                Result = narrowScalar(MI, Step.TypeIdx, Step.NewType);
                break;
            case WidenScalar:
                LLVM_DEBUG(dbgs() << ".. Widen scalar\n");
                Result = widenScalar(MI, Step.TypeIdx, Step.NewType);
                break;
            case Bitcast:
                LLVM_DEBUG(dbgs() << ".. Bitcast type\n");
                Result = bitcast(MI, Step.TypeIdx, Step.NewType);
                break;
            case Lower:
                LLVM_DEBUG(dbgs() << ".. Lower\n");
                Result = lower(MI, Step.TypeIdx, Step.NewType);
                break;
            case FewerElements:
                LLVM_DEBUG(dbgs() << ".. Reduce number of elements\n");
                Result = fewerElementsVector(MI, Step.TypeIdx, Step.NewType);
                break;
            case MoreElements:
                LLVM_DEBUG(dbgs() << ".. Increase number of elements\n");
                Result = moreElementsVector(MI, Step.TypeIdx, Step.NewType);
                break;
            case Custom:
                LLVM_DEBUG(dbgs() << ".. Custom legalization\n");
                Result = LI.legalizeCustom(*this, MI, LocObserver) ? Legalized
                                                                   : UnableToLegalize;
                break;
            default:
...
LLT LegalizerHelper::extractGCDType(SmallVectorImpl<Register>& Parts, LLT DstTy, LLT NarrowTy, Register SrcReg) {
    LLT SrcTy = MRI.getType(SrcReg);
...
LLT LegalizerHelper::buildLCMMergePieces(LLT DstTy, LLT NarrowTy, LLT GCDTy, SmallVectorImpl<Register>& VRegs, unsigned PadStrategy) {
    ...
    if (NumOrigSrc < NumParts * NumSubParts) {
        if (PadStrategy == TargetOpcode::G_ZEXT) {
            ...
            auto ShiftAmt = MIRBuilder.buildConstant(LLT::scalar(64), GCDTy.getSizeInBits() - 1);
...
void LegalizerHelper::buildWidenedRemergeToDst(Register DstReg, LLT LCMTy, ArrayRef<Register> RemergeRegs) {
...
static bool isLibCallInTailPosition(const CallLowering::ArgInfo& Result,
    ...
    if (CallerAttrs.hasRetAttr(Attribute::ZExt) || CallerAttrs.hasRetAttr(Attribute::SExt)) {
        return false;
    }
...
llvm::createLibcall(MachineIRBuilder& MIRBuilder, const char* Name, const CallLowering::ArgInfo& Result, ArrayRef<CallLowering::ArgInfo> Args, const CallingConv::ID CC, LostDebugLocObserver& LocObserver, MachineInstr* MI) {
    ...
    Info.OrigRet = Result;
    if (MI) {
        Info.IsTailCall = (Result.Ty->isVoidTy() || Result.Ty == MIRBuilder.getMF().getFunction().getReturnType()) && isLibCallInTailPosition(Result, *MI, MIRBuilder.getTII(), *MIRBuilder.getMRI());
    }
    ...
    if (MI && Info.LoweredTailCall) {
        ...
        do {
            ...
            assert(Next && (Next->isCopy() || Next->isReturn() || Next->isDebugInstr()) && "Expected instr following MI to be return or debug inst?");
...
LegalizerHelper::LegalizeResult
llvm::createLibcall(MachineIRBuilder& MIRBuilder, RTLIB::Libcall Libcall, const CallLowering::ArgInfo& Result, ArrayRef<CallLowering::ArgInfo> Args, LostDebugLocObserver& LocObserver, MachineInstr* MI) {
...
simpleLibcall(MachineInstr& MI, MachineIRBuilder& MIRBuilder, unsigned Size, Type* OpType, LostDebugLocObserver& LocObserver) {
...
LegalizerHelper::LegalizeResult
llvm::createMemLibcall(MachineIRBuilder& MIRBuilder, MachineRegisterInfo& MRI, MachineInstr& MI, LostDebugLocObserver& LocObserver) {
    ...
    Info.IsTailCall = MI.getOperand(MI.getNumOperands() - 1).getImm() && isLibCallInTailPosition(Info.OrigRet, MI, MIRBuilder.getTII(), MRI);
...
createAtomicLibcall(MachineIRBuilder& MIRBuilder, MachineInstr& MI) {
    ...
    switch (Opc) {
        ...
        case TargetOpcode::G_ATOMIC_CMPXCHG_WITH_SUCCESS: {
            Register Success;
            LLT SuccessLLT;
            auto [Ret, RetLLT, Mem, MemLLT, Cmp, CmpLLT, New, NewLLT] = MI.getFirst4RegLLTs();
        ...
        case TargetOpcode::G_ATOMICRMW_XOR: {
            ...
            if (Opc == TargetOpcode::G_ATOMICRMW_AND) {
                Val = MIRBuilder.buildXor(ValLLT, MIRBuilder.buildConstant(ValLLT, -1), Val)
                          .getReg(0);
            } else if (Opc == TargetOpcode::G_ATOMICRMW_SUB) {
                Val = MIRBuilder.buildSub(ValLLT, MIRBuilder.buildConstant(ValLLT, 0), Val)
                          .getReg(0);
            }
...
static LegalizerHelper::LegalizeResult
conversionLibcall(MachineInstr& MI, MachineIRBuilder& MIRBuilder, Type* ToType, Type* FromType, LostDebugLocObserver& LocObserver, const TargetLowering& TLI, bool IsSigned = false) {
...
LegalizerHelper::createGetStateLibcall(MachineIRBuilder& MIRBuilder,
    ...
    auto Res = createLibcall(MIRBuilder, RTLibcall,
...
LegalizerHelper::createFCMPLibcall(MachineIRBuilder& MIRBuilder,
    ...
    if ((Size != 32 && Size != 64 && Size != 128) || OpLLT != MRI.getType(Cmp->getRHSReg())) {
        return UnableToLegalize;
    }
    ...
    if (const auto [Libcall, ICmpPred] = getFCMPLibcallDesc(Cond, Size);
        Libcall != RTLIB::UNKNOWN_LIBCALL && ICmpPred != CmpInst::BAD_ICMP_PREDICATE) {
    ...
    switch (Cond) {
        case CmpInst::FCMP_UEQ: {
            ...
            const auto [OeqLibcall, OeqPred] = getFCMPLibcallDesc(CmpInst::FCMP_OEQ, Size);
            const auto Oeq = BuildLibcall(OeqLibcall, OeqPred, DstTy);

            const auto [UnoLibcall, UnoPred] = getFCMPLibcallDesc(CmpInst::FCMP_UNO, Size);
        ...
        case CmpInst::FCMP_ONE: {
            ...
            const auto [OeqLibcall, OeqPred] = getFCMPLibcallDesc(CmpInst::FCMP_OEQ, Size);
            const auto NotOeq = BuildLibcall(OeqLibcall, CmpInst::getInversePredicate(OeqPred), DstTy);

            const auto [UnoLibcall, UnoPred] = getFCMPLibcallDesc(CmpInst::FCMP_UNO, Size);
            const auto NotUno = BuildLibcall(UnoLibcall, CmpInst::getInversePredicate(UnoPred), DstTy);
        ...
        case CmpInst::FCMP_ORD: {
            ...
            //                            Op1, Op2));
            const auto [InversedLibcall, InversedPred] = getFCMPLibcallDesc(CmpInst::getInversePredicate(Cond), Size);
...
LegalizerHelper::libcall(MachineInstr& MI, LostDebugLocObserver& LocObserver) {
    ...
    switch (MI.getOpcode()) {
        ...
        case TargetOpcode::G_INTRINSIC_LLRINT: {
            ...
            auto Libcall = getRTLibDesc(MI.getOpcode(), Size);
            LegalizeResult Status = createLibcall(MIRBuilder, Libcall, {MI.getOperand(0).getReg(), ITy, 0},
        ...
        case TargetOpcode::G_FLDEXP: {
            ...
            LegalizeResult Status = createLibcall(MIRBuilder, Libcall, {MI.getOperand(0).getReg(), HLTy, 0},
                Args, LocObserver, &MI);
            if (Status != Legalized) {
                return Status;
            }
        ...
        case TargetOpcode::G_FPTRUNC: {
            ...
            LegalizeResult Status = conversionLibcall(MI, MIRBuilder, ToTy, FromTy, LocObserver, TLI);
            if (Status != Legalized) {
                return Status;
            }
            break;
        ...
        case TargetOpcode::G_UITOFP: {
            ...
            LegalizeResult Status = conversionLibcall(MI, MIRBuilder, ToTy, Type::getIntNTy(Ctx, FromSize),
        ...
        case TargetOpcode::G_MEMMOVE:
        case TargetOpcode::G_MEMSET: {
            LegalizeResult Result = createMemLibcall(MIRBuilder, *MIRBuilder.getMRI(), MI, LocObserver);
            if (Result != Legalized) {
                return Result;
            }
            MI.eraseFromParent();
        ...
        case TargetOpcode::G_RESET_FPENV:
        case TargetOpcode::G_RESET_FPMODE: {
            LegalizeResult Result = createResetStateLibcall(MIRBuilder, MI, LocObserver);
            if (Result != Legalized) {
                return Result;
            }
            break;
    ...
    switch (MI.getOpcode()) {
        ...
        case TargetOpcode::G_PHI: {
            ...
            for (unsigned i = 0; i < NumParts; ++i) {
                ...
                MachineInstrBuilder MIB = MIRBuilder.buildInstr(TargetOpcode::G_PHI).addDef(DstRegs[i]);
        ...
        case TargetOpcode::G_SEXT_INREG: {
            ...
            Register AshrCstReg = MIRBuilder.buildConstant(NarrowTy, NarrowTy.getScalarSizeInBits() - 1)
                                      .getReg(0);
            ...
            for (int i = 0; i < NumParts; ++i) {
                if ((i + 1) * NarrowTy.getScalarSizeInBits() <= SizeInBits) {
                    ...
                    assert(PartialExtensionReg && "Expected to visit partial extension before full");
                    if (FullExtensionReg) {
...
void LegalizerHelper::widenScalarSrc(MachineInstr& MI, LLT WideTy, unsigned OpIdx, unsigned ExtOpcode) {
    MachineOperand& MO = MI.getOperand(OpIdx);
    auto ExtB = MIRBuilder.buildInstr(ExtOpcode, {WideTy}, {MO});
    MO.setReg(ExtB.getReg(0));
}

void LegalizerHelper::narrowScalarSrc(MachineInstr& MI, LLT NarrowTy, unsigned OpIdx) {
    MachineOperand& MO = MI.getOperand(OpIdx);
    auto ExtB = MIRBuilder.buildTrunc(NarrowTy, MO);
    MO.setReg(ExtB.getReg(0));
}

void LegalizerHelper::widenScalarDst(MachineInstr& MI, LLT WideTy, unsigned OpIdx, unsigned TruncOpcode) {
    MachineOperand& MO = MI.getOperand(OpIdx);
...
void LegalizerHelper::narrowScalarDst(MachineInstr& MI, LLT NarrowTy, unsigned OpIdx, unsigned ExtOpcode) {
    MachineOperand& MO = MI.getOperand(OpIdx);
...
void LegalizerHelper::moreElementsVectorDst(MachineInstr& MI, LLT WideTy, unsigned OpIdx) {
    MachineOperand& MO = MI.getOperand(OpIdx);
    MIRBuilder.setInsertPt(MIRBuilder.getMBB(), ++MIRBuilder.getInsertPt());
...
void LegalizerHelper::moreElementsVectorSrc(MachineInstr& MI, LLT MoreTy, unsigned OpIdx) {
    MachineOperand& MO = MI.getOperand(OpIdx);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarMergeValues(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
    ...
    if (WideSize >= DstSize) {
        ...
        for (unsigned I = 2; I != NumOps; ++I) {
            ...
            Register NextResult = I + 1 == NumOps && WideTy == DstTy ? DstReg : MRI.createGenericVirtualRegister(WideTy);
    ...
    for (int I = 0; I != NumMerge; ++I, Slicer = Slicer.drop_front(PartsPerGCD)) {
        auto Merge = MIRBuilder.buildMergeLikeInstr(WideTy, Slicer.take_front(PartsPerGCD));
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarUnmergeValues(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarExtract(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    ...
    MI.getOperand(2).setImm((WideTy.getSizeInBits() / SrcTy.getSizeInBits()) * Offset);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarInsert(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarAddSubOverflow(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    unsigned Opcode;
    unsigned ExtOpcode;
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarAddSubShlSat(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    bool IsSigned = MI.getOpcode() == TargetOpcode::G_SADDSAT || MI.getOpcode() == TargetOpcode::G_SSUBSAT || MI.getOpcode() == TargetOpcode::G_SSHLSAT;
    bool IsShift = MI.getOpcode() == TargetOpcode::G_SSHLSAT || MI.getOpcode() == TargetOpcode::G_USHLSAT;
...
LegalizerHelper::LegalizeResult
LegalizerHelper::widenScalarMulo(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    if (TypeIdx == 1) {
        Observer.changingInstr(MI);
        widenScalarDst(MI, WideTy, 1);
        Observer.changedInstr(MI);
    ...
    unsigned MulOpc = WideMulCanOverflow ? MI.getOpcode() : (unsigned)TargetOpcode::G_MUL;
    ...
    if (WideMulCanOverflow) {
        auto Overflow = MIRBuilder.buildICmp(CmpInst::ICMP_NE, OverflowTy, Mul, ExtResult);
...
LegalizerHelper::widenScalar(MachineInstr& MI, unsigned TypeIdx, LLT WideTy) {
    ...
    switch (Opcode) {
        ...
        case TargetOpcode::G_ATOMIC_CMPXCHG_WITH_SUCCESS:
            ...
            assert(TypeIdx == 1 && "G_ATOMIC_CMPXCHG_WITH_SUCCESS with third scalar type");
            Observer.changingInstr(MI);
            widenScalarDst(MI, WideTy, 1);
            Observer.changedInstr(MI);
            return Legalized;
        ...
        case TargetOpcode::G_CTPOP: {
            ...
            unsigned ExtOpc = Opcode == TargetOpcode::G_CTTZ || Opcode == TargetOpcode::G_CTTZ_ZERO_UNDEF
            ...
            if (NewOpc == TargetOpcode::G_CTTZ) {
                ...
                auto TopBit = APInt::getOneBitSet(WideTy.getSizeInBits(), CurTy.getSizeInBits());
        ...
        case TargetOpcode::G_UMAX: {
            ...
            auto& Ctx = MIRBuilder.getMF().getFunction().getContext();
            unsigned ExtOpc = TLI.isSExtCheaperThanZExt(getApproximateEVTForLLT(Ty, Ctx),
        ...
        case TargetOpcode::G_FPTOUI_SAT:
            ...
            if (TypeIdx == 0) {
                ...
                if (Opcode == TargetOpcode::G_FPTOSI_SAT) {
                    ...
                    Register MidReg = MIRBuilder.buildSMin(WideTy, ExtReg, MaxVal).getReg(0);
        ...
        case TargetOpcode::G_STORE: {
            ...
            unsigned ExtType = Ty.getScalarSizeInBits() == 1 ? TargetOpcode::G_ZEXT : TargetOpcode::G_ANYEXT;
        ...
        case TargetOpcode::G_CONSTANT: {
            ...
            assert((ExtOpc == TargetOpcode::G_ZEXT || ExtOpc == TargetOpcode::G_SEXT || ExtOpc == TargetOpcode::G_ANYEXT) && "Illegal Extend");
        ...
        case TargetOpcode::G_ICMP:
            ...
            if (TypeIdx == 0) {
                ...
                auto& Ctx = MIRBuilder.getMF().getFunction().getContext();
                unsigned ExtOpcode = (CmpInst::isSigned(Pred) || TLI.isSExtCheaperThanZExt(getApproximateEVTForLLT(SrcTy, Ctx), getApproximateEVTForLLT(WideTy, Ctx)))
                    ? TargetOpcode::G_SEXT
                    : TargetOpcode::G_ZEXT;
...
static void emitLoadFromConstantPool(Register DstReg, const Constant* ConstVal, MachineIRBuilder& MIRBuilder) {
    ...
    MachineMemOperand* MMO = MF.getMachineMemOperand(MachinePointerInfo::getConstantPool(MF),
...
static Register getBitcastWiderVectorElementOffset(MachineIRBuilder& B,
    ...
    return B.buildShl(IdxTy, OffsetIdx,
                B.buildConstant(IdxTy, Log2_32(OldEltSize)))
        .getReg(0);
}
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastExtractVectorElt(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
    ...
    if (NewNumElts > OldNumElts) {
        ...
        LLT MidTy = LLT::scalarOrVector(ElementCount::getFixed(NewEltsPerOldElt), NewEltTy);
...
static Register buildBitFieldInsert(MachineIRBuilder& B,
    ...
    auto EltMask = B.buildConstant(
        TargetTy, APInt::getLowBitsSet(TargetTy.getSizeInBits(), InsertTy.getSizeInBits()));
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastInsertVectorElt(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }

    auto [Dst, DstTy, SrcVec, SrcVecTy, Val, ValTy, Idx, IdxTy] = MI.getFirst4RegLLTs();
    LLT VecTy = DstTy;
    ...
    if (NewNumElts < OldNumElts) {
        ...
        if (CastTy.isVector()) {
            InsertedElt = MIRBuilder.buildInsertVectorElement(
                                        CastTy, CastVec, InsertedElt, ScaledIdx)
                              .getReg(0);
        }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastConcatVector(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    ...
    Register BuildReg = MIRBuilder.buildBuildVector(CastTy, BitcastRegs).getReg(0);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastShuffleVector(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    ...
    if (TypeIdx != 0 || CastTy.getScalarSizeInBits() != DstTy.getScalarSizeInBits() || CastTy.getElementCount() != DstTy.getElementCount()) {
        return UnableToLegalize;
    }
    ...
    auto Shuf = MIRBuilder.buildShuffleVector(CastTy, Inp1, Inp2, ShuffleMI->getMask());
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastExtractSubvector(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    ...
    if (Idx % AdjustAmt != 0 || DstTyMinElts % AdjustAmt != 0 || SrcTyMinElts % AdjustAmt != 0) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::bitcastInsertSubvector(MachineInstr& MI, unsigned TypeIdx, LLT CastTy) {
    ...
    auto AdjustAmt = CastEltSize / DstEltSize;
    if (Idx % AdjustAmt != 0 || DstTyMinElts % AdjustAmt != 0 || BigVecTyMinElts % AdjustAmt != 0 || SubVecTyMinElts % AdjustAmt != 0) {
        return UnableToLegalize;
    }
    ...
    auto PromotedIS = MIRBuilder.buildInsertSubvector(CastTy, CastBigVec, CastSubVec, Idx);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerLoad(GAnyLoad& LoadMI) {
    ...
    if (MemSizeInBits != MemStoreSizeInBits) {
        ...
        MachineMemOperand* NewMMO = MF.getMachineMemOperand(&MMO, MMO.getPointerInfo(), WideMemTy);
    ...
    MachineMemOperand* LargeMMO = MF.getMachineMemOperand(&MMO, 0, LargeSplitSize / 8);
    MachineMemOperand* SmallMMO = MF.getMachineMemOperand(&MMO, LargeSplitSize / 8, SmallSplitSize / 8);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerStore(GStore& StoreMI) {
    ...
    if (StoreWidth != StoreSizeInBits && !SrcTy.isVector()) {
        ...
        MachineMemOperand* NewMMO = MF.getMachineMemOperand(&MMO, MMO.getPointerInfo(), WideTy);
...
LegalizerHelper::lower(MachineInstr& MI, unsigned TypeIdx, LLT LowerHintTy) {
    ...
    switch (MI.getOpcode()) {
        ...
        case TargetOpcode::G_UREM: {
            LLT Ty = MRI.getType(MI.getOperand(0).getReg());
            auto Quot = MIRBuilder.buildInstr(MI.getOpcode() == G_SREM ? G_SDIV : G_UDIV, {Ty},
        ...
        case G_USUBE: {
            ...
            auto Zero = MIRBuilder.buildConstant(Ty, 0);
            auto TmpResEqZero = MIRBuilder.buildICmp(CmpInst::ICMP_EQ, CondTy, TmpRes, Zero);
...
MachineInstrBuilder
LegalizerHelper::createStackTemporary(TypeSize Bytes, Align Alignment, MachinePointerInfo& PtrInfo) {
    MachineFunction& MF = MIRBuilder.getMF();
    const DataLayout& DL = MIRBuilder.getDataLayout();
...
MachineInstrBuilder LegalizerHelper::createStackStoreLoad(const DstOp& Res,
    ...
    Align StackTypeAlign = std::max(getStackTemporaryAlignment(SrcTy),
        getStackTemporaryAlignment(Res.getLLTTy(MRI)));
    MachinePointerInfo PtrInfo;
    auto StackTemp = createStackTemporary(SrcTy.getSizeInBytes(), StackTypeAlign, PtrInfo);
...
Register LegalizerHelper::getVectorElementPointer(Register VecPtr, LLT VecTy, Register Index) {
    LLT EltTy = VecTy.getElementType();

    // Calculate the element offset and add it to the pointer.
    unsigned EltSize = EltTy.getSizeInBits() / 8; // FIXME: should be ABI size.
    assert(EltSize * 8 == EltTy.getSizeInBits() && "Converting bits to bytes lost precision");
...
static bool hasSameNumEltsOnAllVectorOperands(
    GenericMachineInstr& MI, MachineRegisterInfo& MRI, std::initializer_list<unsigned> NonVecOpIndices) {
...
static void makeDstOps(SmallVectorImpl<DstOp>& DstOps, LLT Ty, unsigned NumElts) {
    ...
    std::tie(NumParts, NumLeftover) = getNarrowTypeBreakDown(Ty, NarrowTy, LeftoverTy);
...
LegalizerHelper::fewerElementsVectorMultiEltType(
    ...
    assert(hasSameNumEltsOnAllVectorOperands(MI, MRI, NonVecOpIndices) && "Non-compatible opcode or not specified non-vector operands");
...
LegalizerHelper::fewerElementsVectorUnmergeValues(MachineInstr& MI,
    ...
    if ((SrcTy.getSizeInBits() % NarrowTy.getSizeInBits() != 0) || (NarrowTy.getSizeInBits() % DstTy.getSizeInBits() != 0)) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::fewerElementsVectorMerge(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    auto [DstReg, DstTy, SrcReg, SrcTy] = MI.getFirst2RegLLTs();
    ...
    if (TypeIdx == 1) {
        ...
        assert((SrcTy.getScalarType() == NarrowTy.getScalarType()) && "bad type");
        if ((DstTy.getSizeInBits() % NarrowTy.getSizeInBits() != 0) || (NarrowTy.getNumElements() >= SrcTy.getNumElements())) {
            return UnableToLegalize;
        }
    ...
    if ((NarrowTy.getSizeInBits() % SrcTy.getSizeInBits() != 0) || (DstTy.getSizeInBits() % NarrowTy.getSizeInBits() != 0)) {
        return UnableToLegalize;
    }
...
LegalizerHelper::fewerElementsVectorExtractInsertVectorElt(MachineInstr& MI,
    ...
    if (MaybeCst) {
        ...
        auto NewIdx = MIRBuilder.buildConstant(IdxTy, IdxVal - NewNumElts * PartIdx);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::reduceLoadStoreWidth(GLoadStore& LdStMI, unsigned TypeIdx, LLT NarrowTy) {
    ...
    auto splitTypePieces = [=](LLT PartTy, SmallVectorImpl<Register>& ValRegs,
        ...
        for (unsigned Idx = 0, E = NumParts; Idx != E && Offset < TotalSize;
            ...
            MachineMemOperand* NewMMO = MF.getMachineMemOperand(&MMO, ByteOffset, PartTy);
    ...
    unsigned HandledOffset = splitTypePieces(NarrowTy, NarrowRegs, NumParts, Offset);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::fewerElementsVector(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    using namespace TargetOpcode;
...
LegalizerHelper::LegalizeResult
LegalizerHelper::fewerElementsBitcast(MachineInstr& MI, unsigned int TypeIdx, LLT NarrowTy) {
    assert(MI.getOpcode() == TargetOpcode::G_BITCAST && "Not a bitcast operation");
...
LegalizerHelper::LegalizeResult LegalizerHelper::fewerElementsVectorShuffle(
    ...
    auto [DstReg, DstTy, Src1Reg, Src1Ty, Src2Reg, Src2Ty] = MI.getFirst3RegLLTs();
    ...
    NarrowTy = DstTy.changeElementCount(DstTy.getElementCount().divideCoefficientBy(2));
...
LegalizerHelper::LegalizeResult LegalizerHelper::fewerElementsVectorReductions(
    ...
    if (NarrowTy.isVector() && (SrcTy.getNumElements() % NarrowTy.getNumElements() != 0)) {
        return UnableToLegalize;
    }
    ...
    const unsigned NumParts = NarrowTy.isVector() ? SrcTy.getNumElements() / NarrowTy.getNumElements()
    ...
    if (isPowerOf2_32(SrcTy.getNumElements()) && isPowerOf2_32(NarrowTy.getNumElements())) {
...
LegalizerHelper::fewerElementsVectorSeqReductions(MachineInstr& MI,
    ...
    LLT NarrowTy) {
    auto [DstReg, DstTy, ScalarReg, ScalarTy, SrcReg, SrcTy] = MI.getFirst3RegLLTs();
    if (!NarrowTy.isScalar() || TypeIdx != 2 || DstTy != ScalarTy || DstTy != NarrowTy) {
        return UnableToLegalize;
    }

    assert((MI.getOpcode() == TargetOpcode::G_VECREDUCE_SEQ_FADD || MI.getOpcode() == TargetOpcode::G_VECREDUCE_SEQ_FMUL) && "Unexpected vecreduce opcode");
...
LegalizerHelper::tryNarrowPow2Reduction(MachineInstr& MI, Register SrcReg, LLT SrcTy, LLT NarrowTy, unsigned ScalarOpc) {
    ...
    while (SplitSrcs.size() > 1) {
        ...
        for (unsigned Idx = 0; Idx < SplitSrcs.size() - 1; Idx += 2) {
            ...
            Register Res = MIRBuilder.buildInstr(ScalarOpc, {NarrowTy}, {LHS, RHS}).getReg(0);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarShiftByConstant(MachineInstr& MI, const APInt& Amt, const LLT HalfTy, const LLT AmtTy) {
    ...
    if (MI.getOpcode() == TargetOpcode::G_SHL) {
        if (Amt.ugt(VTBits)) {
            ...
            auto OrLHS = MIRBuilder.buildShl(NVT, InH, MIRBuilder.buildConstant(AmtTy, Amt));
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarShift(MachineInstr& MI, unsigned TypeIdx, LLT RequestedTy) {
    if (TypeIdx == 1) {
        Observer.changingInstr(MI);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::moreElementsVectorPhi(MachineInstr& MI, unsigned TypeIdx, LLT MoreTy) {
    assert(TypeIdx == 0 && "Expecting only Idx 0");
...
LegalizerHelper::LegalizeResult
LegalizerHelper::moreElementsVector(MachineInstr& MI, unsigned TypeIdx, LLT MoreTy) {
    unsigned Opc = MI.getOpcode();
    switch (Opc) {
        case TargetOpcode::G_IMPLICIT_DEF:
        case TargetOpcode::G_LOAD: {
        ...
        case TargetOpcode::G_SELECT: {
            ...
            if (TypeIdx == 1) {
                if (!CondTy.isScalar() || DstTy.getElementCount() != MoreTy.getElementCount()) {
                    return UnableToLegalize;
                }
...
LegalizerHelper::equalizeVectorShuffleLengths(MachineInstr& MI) {
    ...
    if (MaskNumElts != PaddedMaskNumElts) {
        auto Shuffle = MIRBuilder.buildShuffleVector(PaddedTy, Src1, Src2, MappedOps);

        SmallVector<Register, 16> Elts(MaskNumElts);
        for (unsigned I = 0; I < MaskNumElts; ++I) {
            Elts[I] = MIRBuilder.buildExtractVectorElementConstant(DestEltTy, Shuffle, I)
                          .getReg(0);
        }
...
LegalizerHelper::moreElementsVectorShuffle(MachineInstr& MI,
    ...
    if (DstTy.isVector() && Src1Ty.isVector() && DstTy.getNumElements() != Src1Ty.getNumElements()) {
...
void LegalizerHelper::multiplyRegisters(SmallVectorImpl<Register>& DstRegs,
    ...
    Register FactorSum = B.buildMul(NarrowTy, Src1Regs[DstIdx], Src2Regs[DstIdx]).getReg(0);
    ...
    for (DstIdx = 1; DstIdx < DstParts; DstIdx++) {
        ...
        for (unsigned i = DstIdx + 1 < SrcParts ? 0 : DstIdx - SrcParts + 1;
            ...
            MachineInstrBuilder Mul = B.buildMul(NarrowTy, Src1Regs[DstIdx - i], Src2Regs[i]);
        ...
        for (unsigned i = DstIdx < SrcParts ? 0 : DstIdx - SrcParts;
            ...
            MachineInstrBuilder Umulh = B.buildUMulH(NarrowTy, Src1Regs[DstIdx - 1 - i], Src2Regs[i]);
        ...
        if (DstIdx != DstParts - 1) {
            MachineInstrBuilder Uaddo = B.buildUAddo(NarrowTy, LLT::scalar(1), Factors[0], Factors[1]);
            FactorSum = Uaddo.getReg(0);
            CarrySum = B.buildZExt(NarrowTy, Uaddo.getReg(1)).getReg(0);
            for (unsigned i = 2; i < Factors.size(); ++i) {
                MachineInstrBuilder Uaddo = B.buildUAddo(NarrowTy, LLT::scalar(1), FactorSum, Factors[i]);
                FactorSum = Uaddo.getReg(0);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarAddSub(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
    ...
    for (int i = 0, e = Src1Regs.size(); i != e; ++i) {
        Register DstReg = MRI.createGenericVirtualRegister(MRI.getType(Src1Regs[i]));
        Register CarryOut;
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarFPTOI(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
    ...
    if (SrcTy.getScalarType() != LLT::scalar(16) || NarrowTy.getScalarSizeInBits() < (IsSigned ? 17u : 16u)) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarExtract(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarInsert(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    // FIXME: Don't know how to handle secondary types yet.
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
    ...
    for (int I = 0, E = SrcRegs.size(); I != E; ++I) {
        ...
        if (OpStart < DstStart) {
            ...
            ExtractOffset = 0;
            SegSize = std::min(NarrowSize - InsertOffset, OpStart + OpSize - DstStart);
        }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarBasic(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    Register DstReg = MI.getOperand(0).getReg();
    LLT DstTy = MRI.getType(DstReg);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarExt(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarSelect(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 0) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarCTLZ(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
    ...
    if (SrcTy.isScalar() && SrcTy.getSizeInBits() == 2 * NarrowSize) {
        ...
        auto LoCTLZ = IsUndef ? B.buildCTLZ_ZERO_UNDEF(DstTy, UnmergeSrc.getReg(0)) : B.buildCTLZ(DstTy, UnmergeSrc.getReg(0));
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarCTTZ(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
    ...
    if (SrcTy.isScalar() && SrcTy.getSizeInBits() == 2 * NarrowSize) {
        ...
        auto HiCTTZ = IsUndef ? B.buildCTTZ_ZERO_UNDEF(DstTy, UnmergeSrc.getReg(1)) : B.buildCTTZ(DstTy, UnmergeSrc.getReg(1));
        auto C_NarrowSize = B.buildConstant(DstTy, NarrowSize);
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarCTPOP(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult
LegalizerHelper::narrowScalarFLDEXP(MachineInstr& MI, unsigned TypeIdx, LLT NarrowTy) {
    if (TypeIdx != 1) {
        return UnableToLegalize;
    }
...
LegalizerHelper::lowerBitCount(MachineInstr& MI) {
    ...
    switch (Opc) {
        ...
        case TargetOpcode::G_CTTZ: {
            ...
            if (!isSupported({TargetOpcode::G_CTPOP, {SrcTy, SrcTy}}) && isSupported({TargetOpcode::G_CTLZ, {SrcTy, SrcTy}})) {
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerEXT(MachineInstr& MI) {
    ...
    if (!isPowerOf2_32(DstTySize) || !isPowerOf2_32(DstTyScalarSize) || !isPowerOf2_32(SrcTyScalarSize)) {
        return UnableToLegalize;
    }
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerTRUNC(MachineInstr& MI) {
    ...
    if (DstTy.isVector() && isPowerOf2_32(DstTy.getNumElements()) && isPowerOf2_32(DstTy.getScalarSizeInBits()) && isPowerOf2_32(SrcTy.getNumElements()) && isPowerOf2_32(SrcTy.getScalarSizeInBits())) {
        // Split input type.
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerRotate(MachineInstr& MI) {
    ...
    unsigned RevRot = IsLeft ? TargetOpcode::G_ROTR : TargetOpcode::G_ROTL;
    if (LI.isLegalOrCustom({RevRot, {DstTy, SrcTy}}) && isPowerOf2_32(EltSizeInBits)) {
    ...
    if ((IsFShLegal = LI.isLegalOrCustom({FShOpc, {DstTy, AmtTy}})) || LI.isLegalOrCustom({RevFsh, {DstTy, AmtTy}})) {
    ...
    if (isPowerOf2_32(EltSizeInBits)) {
        ...
        RevShiftVal = MIRBuilder.buildInstr(RevShiftOpc, {DstTy}, {Src, RevAmt}).getReg(0);
    } else {
        ...
        RevShiftVal = MIRBuilder.buildInstr(RevShiftOpc, {DstTy}, {Inner, RevAmt}).getReg(0);
    }
...
LegalizerHelper::lowerU64ToF32WithSITOFP(MachineInstr& MI) {
    ...
    auto IsLarge = MIRBuilder.buildICmp(CmpInst::Predicate::ICMP_SLT, S1, Src, Zero);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerFPTOUI(MachineInstr& MI) {
    ...
    MachineInstrBuilder FCMP = MIRBuilder.buildFCmp(CmpInst::FCMP_ULT, S1, Src, Threshold);
...
LegalizerHelper::lowerFPTOINT_SAT(MachineInstr& MI) {
    ...
    APFloat::opStatus MinStatus = MinFloat.convertFromAPInt(MinInt, IsSigned, APFloat::rmTowardZero);
    APFloat::opStatus MaxStatus = MaxFloat.convertFromAPInt(MaxInt, IsSigned, APFloat::rmTowardZero);
    bool AreExactFloatBounds = !(MinStatus & APFloat::opStatus::opInexact) && !(MaxStatus & APFloat::opStatus::opInexact);
    ...
    if (AreExactFloatBounds) {
        ...
        auto MinP = MIRBuilder.buildFCmp(CmpInst::FCMP_OLT, SrcTy.changeElementSize(1), Max,
            MinC, MachineInstr::FmNoNans);
        auto Min = MIRBuilder.buildSelect(SrcTy, MinP, Max, MinC, MachineInstr::FmNoNans);
    ...
    auto ULT = MIRBuilder.buildFCmp(CmpInst::FCMP_ULT, SrcTy.changeElementSize(1), Src,
    ...
    auto OGT = MIRBuilder.buildFCmp(CmpInst::FCMP_OGT, SrcTy.changeElementSize(1), Src,
...
LegalizerHelper::lowerFPTRUNC_F64_TO_F16(MachineInstr& MI) {
    ...
    auto [Dst, Src] = MI.getFirst2Regs();
    assert(MRI.getType(Dst).getScalarType() == LLT::scalar(16) && MRI.getType(Src).getScalarType() == LLT::scalar(64));
...
LegalizerHelper::lowerThreewayCompare(MachineInstr& MI) {
    ...
    if (TLI.shouldExpandCmpUsingSelects(getApproximateEVTForLLT(SrcTy, Ctx)) || BC == TargetLowering::UndefinedBooleanContent) {
        ...
        // them.
        unsigned BoolExtOp = MIRBuilder.getBoolExtOp(DstTy.isVector(), /*isFP=*/false);
...
LegalizerHelper::lowerFMinNumMaxNum(MachineInstr& MI) {
    unsigned NewOp = MI.getOpcode() == TargetOpcode::G_FMINNUM ? TargetOpcode::G_FMINNUM_IEEE : TargetOpcode::G_FMAXNUM_IEEE;
...
LegalizerHelper::lowerIntrinsicRound(MachineInstr& MI) {
    ...
    auto Cmp = MIRBuilder.buildFCmp(CmpInst::FCMP_OGE, CondTy, AbsDiff, Half, Flags);
...
LegalizerHelper::lowerShuffleVector(MachineInstr& MI) {
    auto [DstReg, DstTy, Src0Reg, Src0Ty, Src1Reg, Src1Ty] = MI.getFirst3RegLLTs();
...
LegalizerHelper::lowerVECTOR_COMPRESS(llvm::MachineInstr& MI) {
    ...
    Align VecAlign = getStackTemporaryAlignment(VecTy);
    MachinePointerInfo PtrInfo;
    Register StackPtr = createStackTemporary(TypeSize::getFixed(VecTy.getSizeInBytes()), VecAlign,
        PtrInfo)
                            .getReg(0);
    ...
    bool HasPassthru = MRI.getVRegDef(Passthru)->getOpcode() != TargetOpcode::G_IMPLICIT_DEF;
    ...
    std::optional<APInt> PassthruSplatVal = isConstantOrConstantSplatVector(*MRI.getVRegDef(Passthru), MRI);
    ...
    if (PassthruSplatVal.has_value()) {
        ...
        Register LastElmtPtr = getVectorElementPointer(StackPtr, VecTy, Popcount.getReg(0));
        LastWriteVal = MIRBuilder.buildLoad(ValTy, LastElmtPtr, ValPtrInfo, ValAlign)
                           .getReg(0);
    }
    ...
    for (unsigned I = 0; I < NumElmts; ++I) {
        ...
        Register ElmtPtr = getVectorElementPointer(StackPtr, VecTy, OutPos.getReg(0));
        ...
        if (HasPassthru && I == NumElmts - 1) {
            ...
            LastWriteVal = MIRBuilder.buildSelect(ValTy, AllLanesSelected, Val, LastWriteVal)
                               .getReg(0);
...
LegalizerHelper::lowerDynStackAlloc(MachineInstr& MI) {
    ...
    Register SPTmp = getDynStackAllocTargetPtr(SPReg, AllocSize, Alignment, PtrTy);
...
LegalizerHelper::lowerExtract(MachineInstr& MI) {
    ...
    if (SrcTy.isVector()) {
        ...
        if ((Offset % SrcEltSize == 0) && (DstSize % SrcEltSize == 0) && (Offset + DstSize <= SrcTy.getSizeInBits())) {
    ...
    if (DstTy.isScalar() && (SrcTy.isScalar() || (SrcTy.isVector() && DstTy == SrcTy.getElementType()))) {
        LLT SrcIntTy = SrcTy;
        if (!SrcTy.isScalar()) {
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerInsert(MachineInstr& MI) {
    ...
    if (DstTy.isVector() && !InsertTy.isPointer()) {
        ...
        if ((Offset % EltSize == 0) && (InsertSize % EltSize == 0) && (Offset + InsertSize <= DstTy.getSizeInBits())) {
    ...
    if (InsertTy.isVector() || (DstTy.isVector() && DstTy.getElementType() != InsertTy)) {
        return UnableToLegalize;
    }

    const DataLayout& DL = MIRBuilder.getDataLayout();
    if ((DstTy.isPointer() && DL.isNonIntegralAddressSpace(DstTy.getAddressSpace())) || (InsertTy.isPointer() && DL.isNonIntegralAddressSpace(InsertTy.getAddressSpace()))) {
...
LegalizerHelper::lowerSADDO_SSUBO(MachineInstr& MI) {
    auto [Dst0, Dst0Ty, Dst1, Dst1Ty, LHS, LHSTy, RHS, RHSTy] = MI.getFirst4RegLLTs();
    ...
    auto ResultLowerThanLHS = MIRBuilder.buildICmp(CmpInst::ICMP_SLT, BoolTy, NewDst0, LHS);
...
LegalizerHelper::lowerAddSubSatToMinMax(MachineInstr& MI) {
    ...
    if (IsSigned) {
        ...
        auto RHSClamped = MIRBuilder.buildSMin(Ty, MIRBuilder.buildSMax(Ty, Lo, RHS), Hi);
...
LegalizerHelper::lowerAddSubSatToAddoSubo(MachineInstr& MI) {
    ...
    auto OverflowRes = MIRBuilder.buildInstr(OverflowOp, {Ty, BoolTy}, {LHS, RHS});
...
LegalizerHelper::lowerShlSat(MachineInstr& MI) {
    assert((MI.getOpcode() == TargetOpcode::G_SSHLSAT || MI.getOpcode() == TargetOpcode::G_USHLSAT) && "Expected shlsat opcode!");
...
LegalizerHelper::lowerBitreverse(MachineInstr& MI) {
    ...
    if (Size >= 8) {
        MachineInstrBuilder BSWAP = MIRBuilder.buildInstr(TargetOpcode::G_BSWAP, {Ty}, {Src});
        ...
        MachineInstrBuilder Swap4 = SwapN(4, Ty, MIRBuilder, BSWAP, APInt::getSplat(Size, APInt(8, 0xF0)));
        ...
        MachineInstrBuilder Swap2 = SwapN(2, Ty, MIRBuilder, Swap4, APInt::getSplat(Size, APInt(8, 0xCC)));
...
LegalizerHelper::lowerISFPCLASS(MachineInstr& MI) {
    ...
    APInt QNaNBitMask = APInt::getOneBitSet(BitSize, AllOneMantissa.getActiveBits() - 1);
    ...
    auto Sign = MIRBuilder.buildICmp(CmpInst::Predicate::ICMP_NE, DstTy, AsInt, Abs);
    ...
    if (FPClassTest PartialCheck = Mask & fcSubnormal) {
        ...
        auto SubnormalRes = MIRBuilder.buildICmp(CmpInst::Predicate::ICMP_ULT, DstTy, VMinusOne,
    ...
    if (FPClassTest PartialCheck = Mask & fcNan) {
        ...
        if (PartialCheck == fcNan) {
            ...
            auto IsNan = MIRBuilder.buildICmp(CmpInst::Predicate::ICMP_UGT, DstTy, Abs, InfC);
    ...
    if (FPClassTest PartialCheck = Mask & fcNormal) {
        ...
        auto NormalRes = MIRBuilder.buildICmp(CmpInst::Predicate::ICMP_ULT, DstTy, ExpMinusOne,
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerSelect(MachineInstr& MI) {
    ...
    auto [DstReg, DstTy, MaskReg, MaskTy, Op1Reg, Op1Ty, Op2Reg, Op2Ty] = MI.getFirst4RegLLTs();
    ...
    if (MaskTy.isScalar()) {
        ...
        MaskElt = MIRBuilder.buildSExtOrTrunc(DstTy.getScalarType(), MaskElt).getReg(0);
...
LegalizerHelper::lowerAbsToAddXor(MachineInstr& MI) {
    ...
    auto ShiftAmt = MIRBuilder.buildConstant(DstTy, DstTy.getScalarSizeInBits() - 1);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerVAArg(MachineInstr& MI) {
    ...
    if (A > TLI.getMinStackArgumentAlignment()) {
        Register AlignAmt = MIRBuilder.buildConstant(PtrTyAsScalarTy, A.value() - 1).getReg(0);
    ...
    auto IncAmt = MIRBuilder.buildConstant(PtrTyAsScalarTy, DL.getTypeAllocSize(Ty));
...
static bool findGISelOptimalMemOpLowering(std::vector<LLT>& MemOps,
    ...
    if (Ty == LLT()) {
        ...
        if (Op.isFixedDstAlign()) {
            while (Op.getDstAlign() < Ty.getSizeInBytes() && !TLI.allowsMisalignedMemoryAccesses(Ty, DstAS, Op.getDstAlign())) {
    ...
    while (Size) {
        ...
        while (TySize > Size) {
            ...
            if (NumMemOps && Op.allowOverlap() && NewTySize < Size && TLI.allowsMisalignedMemoryAccesses(VT, DstAS, Op.isFixedDstAlign() ? Op.getDstAlign() : Align(1), MachineMemOperand::MONone, &Fast) && Fast) {
                TySize = Size;
            } else {
                Ty = NewTy;
                TySize = NewTySize;
...
LegalizerHelper::LegalizeResult
LegalizerHelper::lowerMemset(MachineInstr& MI, Register Dst, Register Val, uint64_t KnownLen, Align Alignment, bool IsVolatile) {
    ...
    for (unsigned I = 0; I < MemOps.size(); I++) {
        ...
        if (Ty.getSizeInBits() < LargestTy.getSizeInBits()) {
            ...
            if (!LargestTy.isVector() && !Ty.isVector() && TLI.isTruncateFree(LargestVT, VT)) {
        ...
        Register Ptr = Dst;
        if (DstOff != 0) {
            auto Offset = MIB.buildConstant(LLT::scalar(PtrTy.getSizeInBits()), DstOff);
...
LegalizerHelper::lowerMemcpyInline(MachineInstr& MI) {
    ...
    assert(LenVRegAndVal && "inline memcpy with dynamic size is not yet supported");
...
LegalizerHelper::lowerMemcpy(MachineInstr& MI, Register Dst, Register Src, uint64_t KnownLen, uint64_t Limit, Align DstAlign, Align SrcAlign, bool IsVolatile) {
    ...
    for (auto CopyTy : MemOps) {
        ...
        auto* LoadMMO = MF.getMachineMemOperand(&SrcMMO, CurrOffset, CopyTy.getSizeInBytes());
        auto* StoreMMO = MF.getMachineMemOperand(&DstMMO, CurrOffset, CopyTy.getSizeInBytes());
...
LegalizerHelper::LegalizeResult
LegalizerHelper::lowerMemmove(MachineInstr& MI, Register Dst, Register Src, uint64_t KnownLen, Align DstAlign, Align SrcAlign, bool IsVolatile) {
    auto& MF = *MI.getParent()->getParent();
    const auto& TLI = *MF.getSubtarget().getTargetLowering();
    auto& DL = MF.getDataLayout();
    LLVMContext& C = MF.getFunction().getContext();
    ...
    for (auto CopyTy : MemOps) {
        ...
        if (CurrOffset != 0) {
            LLT SrcTy = MRI.getType(Src);
            auto Offset = MIB.buildConstant(LLT::scalar(SrcTy.getSizeInBits()), CurrOffset);
            LoadPtr = MIB.buildPtrAdd(SrcTy, Src, Offset).getReg(0);
        }
    ...
    for (unsigned I = 0; I < MemOps.size(); ++I) {
        ...
        if (CurrOffset != 0) {
            LLT DstTy = MRI.getType(Dst);
            auto Offset = MIB.buildConstant(LLT::scalar(DstTy.getSizeInBits()), CurrOffset);
            StorePtr = MIB.buildPtrAdd(DstTy, Dst, Offset).getReg(0);
        }
...
LegalizerHelper::lowerMemCpyFamily(MachineInstr& MI, unsigned MaxLen) {
    ...
    // matcher function.
    assert((Opc == TargetOpcode::G_MEMCPY || Opc == TargetOpcode::G_MEMMOVE || Opc == TargetOpcode::G_MEMSET) && "Expected memcpy like instruction");
