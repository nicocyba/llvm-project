...
bool CombinerHelper::matchExtOfExt(const MachineInstr& FirstMI, const MachineInstr& SecondMI, BuildFnTy& MatchInfo) const {
    ...
    if (First->getOpcode() == Second->getOpcode() && isLegalOrBeforeLegalizer({Second->getOpcode(), {DstTy, SrcTy}})) {
    ...
    if (First->getOpcode() == TargetOpcode::G_ANYEXT && isLegalOrBeforeLegalizer({Second->getOpcode(), {DstTy, SrcTy}})) {
        if (Second->getOpcode() == TargetOpcode::G_ZEXT) {
            MachineInstr::MIFlag Flag = MachineInstr::MIFlag::NoFlags;
            if (Second->getFlag(MachineInstr::MIFlag::NonNeg)) {
                Flag = MachineInstr::MIFlag::NonNeg;
    ...
    if (Second->getOpcode() == TargetOpcode::G_ANYEXT && isLegalOrBeforeLegalizer({First->getOpcode(), {DstTy, SrcTy}})) {
...
bool CombinerHelper::matchCastOfBuildVector(const MachineInstr& CastMI, const MachineInstr& BVMI, BuildFnTy& MatchInfo) const {
    ...
    MatchInfo = [=](MachineIRBuilder& B) {
        ...
        for (unsigned I = 0; I < Elements; ++I) {
            auto CastI = B.buildInstr(Cast->getOpcode(), {ElemTy}, {BV->getSourceReg(I)});
