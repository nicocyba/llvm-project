...
bool CombinerHelper::matchExtractVectorElement(MachineInstr& MI, BuildFnTy& MatchInfo) const {
    ...
    std::optional<ValueAndVReg> MaybeIndex = getIConstantVRegValWithLookThrough(Index, MRI);
    ...
    if (IndexC && VectorTy.isFixedVector() && IndexC->uge(VectorTy.getNumElements()) && isLegalOrBeforeLegalizer({TargetOpcode::G_IMPLICIT_DEF, {DstTy}})) {
...
bool CombinerHelper::matchExtractVectorElementWithDifferentIndices(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    GInsertVectorElement* Insert = getOpcodeDef<GInsertVectorElement>(Vector, MRI);
    if (!Insert) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }
    ...
    std::optional<ValueAndVReg> MaybeInsertIndex = getIConstantVRegValWithLookThrough(Insert->getIndexReg(), MRI);
    ...
    if (MaybeInsertIndex && MaybeInsertIndex->Value != *IndexC) {
        ...
        MatchInfo = [=](MachineIRBuilder& B) { B.buildExtractVectorElement(Dst, Insert->getVectorReg(), Index); };
        NICO_MARKER_LOGGING_APPEND_TRUE;
        return true;
    }
    NICO_MARKER_LOGGING_APPEND_FALSE;
...
bool CombinerHelper::matchExtractVectorElementWithBuildVector(const MachineInstr& MI, const MachineInstr& MI2, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    if (!MRI.hasOneNonDBGUse(Build->getReg(0)) || !getTargetLowering().aggressivelyPreferBuildVectorSources(Ty)) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }
    ...
    MatchInfo = [=](MachineIRBuilder& B) { B.buildCopy(Dst, Build->getSourceReg(Index.getZExtValue())); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}

bool CombinerHelper::matchExtractVectorElementWithBuildVectorTrunc(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    MachineInstr* Root = getDefIgnoringCopies(MO.getReg(), MRI);
    GExtractVectorElement* Extract = cast<GExtractVectorElement>(Root);
    ...
    MatchInfo = [=](MachineIRBuilder& B) { B.buildTrunc(Dst, Build->getSourceReg(MaybeIndex->Value.getZExtValue())); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}

bool CombinerHelper::matchExtractVectorElementWithShuffleVector(const MachineInstr& MI, const MachineInstr& MI2, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    const GExtractVectorElement* Extract = cast<GExtractVectorElement>(&MI);
    ...
    if (SrcIdx < 0 && isLegalOrBeforeLegalizer({TargetOpcode::G_IMPLICIT_DEF, {DstTy}})) {
        MatchInfo = [=](MachineIRBuilder& B) { B.buildUndef(Dst); };
        NICO_MARKER_LOGGING_APPEND_TRUE;
        return true;
    }
    ...
    if (!isLegalOrBeforeLegalizer({TargetOpcode::G_EXTRACT_VECTOR_ELT, {DstTy, NewVectorTy, IdxTy}}) || !isConstantLegalOrBeforeLegalizer({IdxTy})) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }
...
bool CombinerHelper::matchInsertVectorElementOOB(MachineInstr& MI, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    if (MaybeIndex && MaybeIndex->Value.uge(DstTy.getNumElements()) && isLegalOrBeforeLegalizer({TargetOpcode::G_IMPLICIT_DEF, {DstTy}})) {
        MatchInfo = [=](MachineIRBuilder& B) { B.buildUndef(Dst); };
        NICO_MARKER_LOGGING_APPEND_TRUE;
        return true;
    }
...
bool CombinerHelper::matchAddOfVScale(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    if (!MRI.hasOneNonDBGUse(LHSVScale->getReg(0)) || !MRI.hasOneNonDBGUse(RHSVScale->getReg(0))) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }

    MatchInfo = [=](MachineIRBuilder& B) { B.buildVScale(Dst, LHSVScale->getSrc() + RHSVScale->getSrc()); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}

bool CombinerHelper::matchMulOfVScale(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    MatchInfo = [=](MachineIRBuilder& B) { B.buildVScale(Dst, LHSVScale->getSrc() * *MaybeRHS); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}

bool CombinerHelper::matchSubOfVScale(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    if (!MRI.hasOneNonDBGUse(RHSVScale->getReg(0)) || !isLegalOrBeforeLegalizer({TargetOpcode::G_ADD, DstTy})) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }
...
bool CombinerHelper::matchShlOfVScale(const MachineOperand& MO, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    ...
    if (!MRI.hasOneNonDBGUse(LHSVScale->getReg(0)) || !isLegalOrBeforeLegalizer({TargetOpcode::G_VSCALE, DstTy})) {
        NICO_MARKER_LOGGING_APPEND_FALSE;
        return false;
    }

    MatchInfo = [=](MachineIRBuilder& B) { B.buildVScale(Dst, LHSVScale->getSrc().shl(*MaybeRHS)); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}
