...
bool CombinerHelper::matchMergeXAndUndef(const MachineInstr& MI, BuildFnTy& MatchInfo) const {
    ...
    MatchInfo = [=](MachineIRBuilder& B) { B.buildAnyExt(Dst, Merge->getSourceReg(0)); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}

bool CombinerHelper::matchMergeXAndZero(const MachineInstr& MI, BuildFnTy& MatchInfo) const {
    NICO_MARKER_LOGGING_START;
    const GMerge* Merge = cast<GMerge>(&MI);
    ...
    MatchInfo = [=](MachineIRBuilder& B) { B.buildZExt(Dst, Merge->getSourceReg(0)); };
    NICO_MARKER_LOGGING_APPEND_TRUE;
    return true;
}
