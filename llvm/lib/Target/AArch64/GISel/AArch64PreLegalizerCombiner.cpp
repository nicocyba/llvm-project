...
void applyICmpRedundantTrunc(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& Builder, GISelChangeObserver& Observer, Register& WideReg) {
...
void applyFoldGlobalOffset(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer, std::pair<uint64_t, uint64_t>& MatchInfo) {
    // Change:
    //
    //  %g = G_GLOBAL_VALUE @x
    //  %ptr1 = G_PTR_ADD %g, cst1
...
bool matchExtAddvToUdotAddv(MachineInstr& MI, MachineRegisterInfo& MRI, const AArch64Subtarget& STI, std::tuple<Register, Register, bool>& MatchInfo) {
    ...
    if (I1Opc == TargetOpcode::G_MUL) {
        ...
        MachineInstr* ExtMI1 = getDefIgnoringCopies(I1->getOperand(1).getReg(), MRI);
        MachineInstr* ExtMI2 = getDefIgnoringCopies(I1->getOperand(2).getReg(), MRI);
...
void applyExtAddvToUdotAddv(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& Builder, GISelChangeObserver& Observer, const AArch64Subtarget& STI, std::tuple<Register, Register, bool>& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_VECREDUCE_ADD && "Expected a G_VECREDUCE_ADD instruction");
    assert(STI.hasDotProd() && "Target should have Dot Product feature");

    // Initialise the variables
    unsigned DotOpcode = std::get<2>(MatchInfo) ? AArch64::G_SDOT : AArch64::G_UDOT;
    ...
    if (NumOfDotMI == 1) {
        ...
        // Merge the output
        auto ConcatMI = Builder.buildConcatVectors(LLT::fixed_vector(NumElements, 32), DotReg);
...
void applyExtUaddvToUaddlv(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer, std::pair<Register, bool>& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_VECREDUCE_ADD && "Expected G_VECREDUCE_ADD Opcode");
    ...
    if ((SrcScalSize == 8 && SrcNumElem > 16) || (SrcScalSize == 16 && SrcNumElem > 8) || (SrcScalSize == 32 && SrcNumElem > 4)) {
    ...
    for (unsigned I = 0; I < WorkingRegisters.size(); I++) {
        ...
        if ((WorkingRegTy.getScalarSizeInBits() == 8) && (WorkingRegTy.getNumElements() == 4)) {
            WorkingRegisters[I] = B.buildInstr(std::get<1>(MatchInfo) ? TargetOpcode::G_SEXT
                                                                      : TargetOpcode::G_ZEXT,
        ...
        Register addlvReg = B.buildInstr(Opc, {addlvTy}, {WorkingRegisters[I]}).getReg(0);
        ...
        if (MidScalarSize == 32 || MidScalarSize == 64) {
            ...
            Register extractReg = B.buildInstr(AArch64::G_EXTRACT_VECTOR_ELT,
                                      ...
                                      .getReg(0);
            WorkingRegisters[I] = B.buildTrunc({MidScalarLLT}, {extractReg}).getReg(0);
        }
    }
...
void applyPushAddSubExt(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, bool isSExt, Register DstReg, Register SrcReg1, Register SrcReg2) {
    ...
    Register AddReg = B.buildInstr(MI.getOpcode(), {MidTy}, {Ext1Reg, Ext2Reg}).getReg(0);
...
bool tryToSimplifyUADDO(MachineInstr& MI, MachineIRBuilder& B, const CombinerHelper& Helper, GISelChangeObserver& Observer) {
    ...
    Register Op0Wide;
    Register Op1Wide;
    if (!mi_match(DefOp0->getParent(), MRI, m_GTrunc(m_Reg(Op0Wide))) || !mi_match(DefOp1->getParent(), MRI, m_GTrunc(m_Reg(Op1Wide)))) {
        return false;
    }
    ...
    if (Op0WideDef->getOpcode() != TargetOpcode::G_ASSERT_ZEXT || Op1WideDef->getOpcode() != TargetOpcode::G_ASSERT_ZEXT || OpTySize != Op0WideDef->getOperand(2).getImm() || OpTySize != Op1WideDef->getOperand(2).getImm()) {
        return false;
    }

    // Only scalar UADDO with either 8 or 16 bit operands are handled.
    if (!WideTy0.isScalar() || !WideTy1.isScalar() || WideTy0 != WideTy1 || OpTySize >= WideTy0.getScalarSizeInBits() || (OpTySize != 8 && OpTySize != 16)) {
        return false;
    }
    ...
    if (any_of(MRI.use_nodbg_instructions(ResVal),
            [&MI, FailMBB, CurrentMBB](MachineInstr& I) {
                return &MI != &I && (I.getParent() == FailMBB || I.getParent() == CurrentMBB);
            })) {
        return false;
    }
...
public:
    AArch64PreLegalizerCombinerImpl(
        MachineFunction& MF, CombinerInfo& CInfo, const TargetPassConfig* TPC, GISelKnownBits& KB, GISelCSEInfo* CSEInfo, const AArch64PreLegalizerCombinerImplRuleConfig& RuleConfig, const AArch64Subtarget& STI, MachineDominatorTree* MDT, const LegalizerInfo* LI);
...
bool AArch64PreLegalizerCombinerImpl::tryCombineAll(MachineInstr& MI) const {
    outs() << "\t\t\t" << nico::getFunctionName(__PRETTY_FUNCTION__) << " - " << "MI: " << MI2String(MI) << "\n";
    nico::total_data.back().logs.push_back("\t\t\t" + nico::nico::getFunctionName(__PRETTY_FUNCTION__));
...
INITIALIZE_PASS_BEGIN(AArch64PreLegalizerCombiner, DEBUG_TYPE, "Combine AArch64 machine instrs before legalization", false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(GISelKnownBitsAnalysis)
INITIALIZE_PASS_DEPENDENCY(GISelCSEAnalysisWrapperPass)
INITIALIZE_PASS_END(AArch64PreLegalizerCombiner, DEBUG_TYPE, "Combine AArch64 machine instrs before legalization", false, false)
