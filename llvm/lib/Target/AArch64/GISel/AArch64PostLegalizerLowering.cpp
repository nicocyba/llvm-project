...
bool matchREV(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
...
bool matchTRN(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
...
bool matchUZP(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
    unsigned WhichResult;
    ArrayRef<int> ShuffleMask = MI.getOperand(3).getShuffleMask();
    Register Dst = MI.getOperand(0).getReg();
...
bool matchZip(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
    unsigned WhichResult;
    ArrayRef<int> ShuffleMask = MI.getOperand(3).getShuffleMask();
    Register Dst = MI.getOperand(0).getReg();
...
bool matchDupFromInsertVectorElt(int Lane, MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    if (Lane != 0) {
        return false;
    }
...
/// Helper function for matchDup.
bool matchDupFromBuildVector(int Lane, MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
...
bool matchDup(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
...
bool matchEXT(MachineInstr& MI, MachineRegisterInfo& MRI, ShuffleVectorPseudo& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
    Register Dst = MI.getOperand(0).getReg();
    ...
    if (!ExtInfo) {
        if (!getOpcodeDef<GImplicitDef>(V2, MRI) || !isSingletonExtMask(Mask, DstTy)) {
            return false;
        }
...
void applyEXT(MachineInstr& MI, ShuffleVectorPseudo& MatchInfo) {
    ...
    if (MatchInfo.SrcOps[2].getImm() == 0) {
        ...
        auto Cst = MIRBuilder.buildConstant(LLT::scalar(32), MatchInfo.SrcOps[2].getImm());
...
void applyFullRev(MachineInstr& MI, MachineRegisterInfo& MRI) {
    ...
    Register Src = MI.getOperand(1).getReg();
    LLT DstTy = MRI.getType(Dst);
    assert(DstTy.getSizeInBits() == 128 && "Expected 128bit vector in applyFullRev");
    MachineIRBuilder MIRBuilder(MI);
...
void applyNonConstInsert(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& Builder) {
    ...
    assert(isPowerOf2_64(VecTy.getNumElements()) && "Expected a power-2 vector size");
    ...
    Register EltPtr = Builder.buildPtrAdd(MRI.getType(StackTemp.getReg(0)), StackTemp, Mul)
                          .getReg(0);
...
///
bool matchINS(MachineInstr& MI, MachineRegisterInfo& MRI, std::tuple<Register, int, Register, int>& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
    ArrayRef<int> ShuffleMask = MI.getOperand(3).getShuffleMask();
    Register Dst = MI.getOperand(0).getReg();
...
void applyINS(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& Builder, std::tuple<Register, int, Register, int>& MatchInfo) {
...
bool isVShiftRImm(Register Reg, MachineRegisterInfo& MRI, LLT Ty, int64_t& Cnt) {
...
bool matchVAshrLshrImm(MachineInstr& MI, MachineRegisterInfo& MRI, int64_t& Imm) {
    assert(MI.getOpcode() == TargetOpcode::G_ASHR || MI.getOpcode() == TargetOpcode::G_LSHR);
...
void applyVAshrLshrImm(MachineInstr& MI, MachineRegisterInfo& MRI, int64_t& Imm) {
    unsigned Opc = MI.getOpcode();
    assert(Opc == TargetOpcode::G_ASHR || Opc == TargetOpcode::G_LSHR);
    unsigned NewOpc = Opc == TargetOpcode::G_ASHR ? AArch64::G_VASHR : AArch64::G_VLSHR;
...
tryAdjustICmpImmAndPred(Register RHS, CmpInst::Predicate P, const MachineRegisterInfo& MRI) {
    ...
    switch (P) {
        ...
        case CmpInst::ICMP_SGE:
            ...
            if ((Size == 64 && static_cast<int64_t>(C) == INT64_MIN) || (Size == 32 && static_cast<int32_t>(C) == INT32_MIN)) {
                return std::nullopt;
            }
        ...
        case CmpInst::ICMP_SGT:
            ...
            if ((Size == 32 && static_cast<int32_t>(C) == INT32_MAX) || (Size == 64 && static_cast<int64_t>(C) == INT64_MAX)) {
                return std::nullopt;
            }
        ...
        case CmpInst::ICMP_UGT:
            ...
            if ((Size == 32 && static_cast<uint32_t>(C) == UINT32_MAX) || (Size == 64 && C == UINT64_MAX)) {
                return std::nullopt;
            }
    ...
    if (!IsMaterializableInSingleInstruction(OriginalC) && IsMaterializableInSingleInstruction(C)) {
        return {{C, P}};
    }
...
bool matchAdjustICmpImmAndPred(
    MachineInstr& MI, const MachineRegisterInfo& MRI, std::pair<uint64_t, CmpInst::Predicate>& MatchInfo) {
...
void applyAdjustICmpImmAndPred(
    MachineInstr& MI, std::pair<uint64_t, CmpInst::Predicate>& MatchInfo, MachineIRBuilder& MIB, GISelChangeObserver& Observer) {
...
void applyDupLane(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, std::pair<unsigned, int>& MatchInfo) {
    assert(MI.getOpcode() == TargetOpcode::G_SHUFFLE_VECTOR);
    Register Src1Reg = MI.getOperand(1).getReg();
    const LLT SrcTy = MRI.getType(Src1Reg);
...
void applyScalarizeVectorUnmerge(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B) {
    auto& Unmerge = cast<GUnmerge>(MI);
    Register Src1Reg = Unmerge.getReg(Unmerge.getNumOperands() - 1);
    const LLT SrcTy = MRI.getType(Src1Reg);
    assert((SrcTy.isVector() && !SrcTy.isScalable()) && "Expected a fixed length vector");
...
void applyBuildVectorToDup(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B) {
    B.setInstrAndDebugLoc(MI);
...
unsigned getCmpOperandFoldingProfit(Register CmpOp, MachineRegisterInfo& MRI) {
    ...
    auto IsSupportedExtend = [&](const MachineInstr& MI) {
        ...
        auto ValAndVReg = getIConstantVRegValWithLookThrough(MI.getOperand(2).getReg(), MRI);
        if (!ValAndVReg) {
            return false;
        }
    ...
    if (Opc != TargetOpcode::G_SHL && Opc != TargetOpcode::G_ASHR && Opc != TargetOpcode::G_LSHR) {
        return 0;
    }

    auto MaybeShiftAmt = getIConstantVRegValWithLookThrough(Def->getOperand(2).getReg(), MRI);
    if (!MaybeShiftAmt) {
        return 0;
    }
    uint64_t ShiftAmt = MaybeShiftAmt->Value.getZExtValue();
    MachineInstr* ShiftLHS = getDefIgnoringCopies(Def->getOperand(1).getReg(), MRI);
    ...
    if ((ShiftSize == 32 && ShiftAmt <= 31) || (ShiftSize == 64 && ShiftAmt <= 63)) {
        return 1;
    }
    return 0;
}
...
bool trySwapICmpOperands(MachineInstr& MI, MachineRegisterInfo& MRI) {
    ...
    // LHS and RHS.
    return (getCmpOperandFoldingProfit(TheLHS, MRI) > getCmpOperandFoldingProfit(TheRHS, MRI));
}
...
getVectorFCMP(AArch64CC::CondCode CC, Register LHS, Register RHS, bool IsZero, bool NoNans, MachineRegisterInfo& MRI) {
...
bool matchLowerVectorFCMP(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& MIB) {
...
/// Try to lower a vector G_FCMP \p MI into an AArch64-specific pseudo.
void applyLowerVectorFCMP(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& MIB) {
    assert(MI.getOpcode() == TargetOpcode::G_FCMP);
    const auto& ST = MI.getMF()->getSubtarget<AArch64Subtarget>();
    ...
    if ((Pred == CmpInst::Predicate::FCMP_ORD || Pred == CmpInst::Predicate::FCMP_UNO) && IsZero) {
    ...
    const bool NoNans = ST.getTargetLowering()->getTargetMachine().Options.NoNaNsFPMath;
...
void applyLowerBuildToInsertVecElt(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B) {
    auto* GBuildVec = cast<GBuildVector>(&MI);
    ...
    for (unsigned I = 0; I < GBuildVec->getNumSources(); ++I) {
        ...
        DstReg = B.buildInsertVectorElement(DstTy, DstReg, SrcReg, IdxReg).getReg(0);
    }
    B.buildCopy(GBuildVec->getReg(0), DstReg);
    GBuildVec->eraseFromParent();
}

bool matchFormTruncstore(MachineInstr& MI, MachineRegisterInfo& MRI, Register& SrcReg) {
...
void applyFormTruncstore(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer, Register& SrcReg) {
    assert(MI.getOpcode() == TargetOpcode::G_STORE);
    Observer.changingInstr(MI);
...
void applyVectorSextInReg(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer) {
    assert(MI.getOpcode() == TargetOpcode::G_SEXT_INREG);
    B.setInstrAndDebugLoc(MI);
...
bool matchUnmergeExtToUnmerge(MachineInstr& MI, MachineRegisterInfo& MRI, Register& MatchInfo) {
    auto& Unmerge = cast<GUnmerge>(MI);
    ...
    auto LowestVal = getIConstantVRegValWithLookThrough(Ext->getOperand(3).getReg(), MRI);
...
void applyUnmergeExtToUnmerge(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer, Register& SrcReg) {
    Observer.changingInstr(MI);
    // Swap dst registers.
...
void applyExtMulToMULL(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer) {
    assert(MI.getOpcode() == TargetOpcode::G_MUL && "Expected a G_MUL instruction");
...
public:
    AArch64PostLegalizerLoweringImpl(
        MachineFunction& MF, CombinerInfo& CInfo, const TargetPassConfig* TPC, GISelCSEInfo* CSEInfo, const AArch64PostLegalizerLoweringImplRuleConfig& RuleConfig, const AArch64Subtarget& STI);
...
AArch64PostLegalizerLowering::AArch64PostLegalizerLowering() : MachineFunctionPass(ID) {
...
INITIALIZE_PASS_BEGIN(AArch64PostLegalizerLowering, DEBUG_TYPE, "Lower AArch64 MachineInstrs after legalization", false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_END(AArch64PostLegalizerLowering, DEBUG_TYPE, "Lower AArch64 MachineInstrs after legalization", false, false)
