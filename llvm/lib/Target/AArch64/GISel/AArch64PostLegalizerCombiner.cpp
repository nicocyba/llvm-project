...
bool matchExtractVecEltPairwiseAdd(MachineInstr& MI, MachineRegisterInfo& MRI, std::tuple<unsigned, LLT, Register>& MatchInfo) {
    ...
    MachineInstr* Shuffle = getOpcodeDef(TargetOpcode::G_SHUFFLE_VECTOR, Src1Op2, MRI);
    ...
    if (Shuffle && Shuffle->getOperand(3).getShuffleMask()[0] == 1 && Other == MRI.getVRegDef(Shuffle->getOperand(1).getReg())) {
...
bool matchAArch64MulConstCombine(MachineInstr& MI, MachineRegisterInfo& MRI, std::function<void(MachineIRBuilder& B, Register DstReg)>& ApplyFn) {
    ...
    ApplyFn = [=](MachineIRBuilder& B, Register DstReg) {
        ...
        assert(!(NegateResult && TrailingZeroes) && "NegateResult and TrailingZeroes cannot both be true for now.");
        // Negate the result.
        if (NegateResult) {
...
void applyMutateAnyExtToZExt(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer) {
    Observer.changingInstr(MI);
    MI.setDesc(B.getTII().get(TargetOpcode::G_ZEXT));
    Observer.changedInstr(MI);
}
...
void applySplitStoreZero128(MachineInstr& MI, MachineRegisterInfo& MRI, MachineIRBuilder& B, GISelChangeObserver& Observer) {
    B.setInstrAndDebugLoc(MI);
    GStore& Store = cast<GStore>(MI);
    assert(MRI.getType(Store.getValueReg()).isVector() && "Expected a vector store value");
...
bool AArch64PostLegalizerCombiner::optimizeConsecutiveMemOpAddressing(MachineFunction& MF, CSEMIRBuilder& MIB) {
    ...
    auto storeIsValid = [&](StoreInfo& Last, StoreInfo New) {
        ...
        if (Last.Ptr->getBaseReg() != New.Ptr->getBaseReg() || (Last.Offset + static_cast<int64_t>(Last.StoredType.getSizeInBytes()) != New.Offset) || Last.StoredType != New.StoredType) {
            return false;
        }
...
INITIALIZE_PASS_BEGIN(AArch64PostLegalizerCombiner, DEBUG_TYPE, "Combine AArch64 MachineInstrs after legalization", false, false)
