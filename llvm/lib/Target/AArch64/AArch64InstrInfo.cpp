...
static cl::opt<unsigned> TBZDisplacementBits(
    "aarch64-tbz-offset-bits", cl::Hidden, cl::init(14), cl::desc("Restrict range of TB[N]Z instructions (DEBUG)"));

static cl::opt<unsigned> CBZDisplacementBits(
    "aarch64-cbz-offset-bits", cl::Hidden, cl::init(19), cl::desc("Restrict range of CB[N]Z instructions (DEBUG)"));

static cl::opt<unsigned>
    BCCDisplacementBits("aarch64-bcc-offset-bits", cl::Hidden, cl::init(19), cl::desc("Restrict range of Bcc instructions (DEBUG)"));

static cl::opt<unsigned>
    BDisplacementBits("aarch64-b-offset-bits", cl::Hidden, cl::init(26), cl::desc("Restrict range of B instructions (DEBUG)"));

AArch64InstrInfo::AArch64InstrInfo(const AArch64Subtarget& STI)
    : AArch64GenInstrInfo(AArch64::ADJCALLSTACKDOWN, AArch64::ADJCALLSTACKUP, AArch64::CATCHRET),
...
unsigned AArch64InstrInfo::getInstSizeInBytes(const MachineInstr& MI) const {
    ...
    switch (Desc.getOpcode()) {
        ...
        case TargetOpcode::PATCHABLE_FUNCTION_ENTER:
            ...
            NumBytes = F.getFnAttributeAsParsedInteger("patchable-function-entry", 9) * 4;
            break;
...
void AArch64InstrInfo::insertIndirectBranch(MachineBasicBlock& MBB,
    ...
    assert(RestoreBB.empty() && "restore block should be inserted for restoring clobbered registers");
    ...
    if (Scavenged != AArch64::NoRegister && MBB.getSectionID() == MBBSectionID::ColdSectionID) {
...
bool AArch64InstrInfo::analyzeBranch(MachineBasicBlock& MBB,
    ...
    if (AllowModify && isUncondBranchOpcode(LastOpc) && MBB.isLayoutSuccessor(getBranchDestBlock(*LastInst))) {
        LastInst->eraseFromParent();
        LastInst = SecondLastInst;
        LastOpc = LastInst->getOpcode();
        if (I == MBB.begin() || !isUnpredicatedTerminator(*--I)) {
            assert(!isUncondBranchOpcode(LastOpc) && "unreachable unconditional branches removed above");
...
unsigned AArch64InstrInfo::removeBranch(MachineBasicBlock& MBB,
    ...
    if (!isUncondBranchOpcode(I->getOpcode()) && !isCondBranchOpcode(I->getOpcode())) {
        return 0;
    }
...
void AArch64InstrInfo::instantiateCondBranch(
    MachineBasicBlock& MBB, const DebugLoc& DL, MachineBasicBlock* TBB, ArrayRef<MachineOperand> Cond) const {
    if (Cond[0].getImm() != -1) {
        // Regular Bcc
        ...
        const MachineInstrBuilder MIB = BuildMI(&MBB, DL, get(Cond[1].getImm())).add(Cond[2]);
        if (Cond.size() > 3) {
...
unsigned AArch64InstrInfo::insertBranch(
    MachineBasicBlock& MBB, MachineBasicBlock* TBB, MachineBasicBlock* FBB, ArrayRef<MachineOperand> Cond, const DebugLoc& DL, int* BytesAdded) const {
...
static unsigned canFoldIntoCSel(const MachineRegisterInfo& MRI, unsigned VReg, unsigned* NewVReg = nullptr) {
    ...
    switch (DefMI->getOpcode()) {
        ...
        case AArch64::ADDWri:
            // add x, 1 -> csinc.
            if (!DefMI->getOperand(2).isImm() || DefMI->getOperand(2).getImm() != 1 || DefMI->getOperand(3).getImm() != 0) {
                return 0;
            }
            SrcOpNum = 1;
...
bool AArch64InstrInfo::canInsertSelect(const MachineBasicBlock& MBB,
    ...
    if (AArch64::GPR64allRegClass.hasSubClassEq(RC) || AArch64::GPR32allRegClass.hasSubClassEq(RC)) {
    ...
    if (AArch64::FPR64RegClass.hasSubClassEq(RC) || AArch64::FPR32RegClass.hasSubClassEq(RC)) {
...
bool AArch64InstrInfo::isFalkorShiftExtFast(const MachineInstr& MI) {
    switch (MI.getOpcode()) {
        ...
        case AArch64::SUBSWrs: {
            unsigned Imm = MI.getOperand(3).getImm();
            unsigned ShiftVal = AArch64_AM::getShiftValue(Imm);
            return ShiftVal == 0 || (AArch64_AM::getShiftType(Imm) == AArch64_AM::ASR && ShiftVal == 31);
        }
        ...
        case AArch64::SUBSXrs: {
            unsigned Imm = MI.getOperand(3).getImm();
            unsigned ShiftVal = AArch64_AM::getShiftValue(Imm);
            return ShiftVal == 0 || (AArch64_AM::getShiftType(Imm) == AArch64_AM::ASR && ShiftVal == 63);
        }
...
bool AArch64InstrInfo::areMemAccessesTriviallyDisjoint(
    ...
    if (MIa.hasUnmodeledSideEffects() || MIb.hasUnmodeledSideEffects() || MIa.hasOrderedMemoryRef() || MIb.hasOrderedMemoryRef()) {
        return false;
    }
    ...
    if (getMemOperandWithOffsetWidth(MIa, BaseOpA, OffsetA, OffsetAIsScalable,
        ...
        && getMemOperandWithOffsetWidth(MIb, BaseOpB, OffsetB, OffsetBIsScalable,
            WidthB, TRI)) {
        if (BaseOpA->isIdenticalTo(*BaseOpB) && OffsetAIsScalable == OffsetBIsScalable) {
            int LowOffset = OffsetA < OffsetB ? OffsetA : OffsetB;
            int HighOffset = OffsetA < OffsetB ? OffsetB : OffsetA;
            TypeSize LowWidth = (LowOffset == OffsetA) ? WidthA : WidthB;
            if (LowWidth.isScalable() == OffsetAIsScalable && LowOffset + (int)LowWidth.getKnownMinValue() <= HighOffset) {
                return true;
            }
        }
    }
...
static bool UpdateOperandRegClass(MachineInstr& Instr) {
    ...
    for (unsigned OpIdx = 0, EndIdx = Instr.getNumOperands(); OpIdx < EndIdx;
        ...
        const TargetRegisterClass* OpRegCstraints = Instr.getRegClassConstraint(OpIdx, TII, TRI);
        ...
        assert(MO.isReg() && "Operand has register constraints without being a register!");
        ...
        } else if (!OpRegCstraints->hasSubClassEq(MRI->getRegClass(Reg)) && !MRI->constrainRegClass(Reg, OpRegCstraints)) {
            return false;
        }
    }
...
static unsigned convertToNonFlagSettingOpc(const MachineInstr& MI) {
    ...
    if (MI.definesRegister(AArch64::WZR, /*TRI=*/nullptr) || MI.definesRegister(AArch64::XZR, /*TRI=*/nullptr)) {
...
static bool areCFlagsAccessedBetweenInstrs(
    MachineBasicBlock::iterator From, MachineBasicBlock::iterator To, const TargetRegisterInfo* TRI, const AccessKind AccessToCheck = AK_All) {
    ...
    for (const MachineInstr& Instr :
        ...
        if (((AccessToCheck & AK_Write) && Instr.modifiesRegister(AArch64::NZCV, TRI)) || ((AccessToCheck & AK_Read) && Instr.readsRegister(AArch64::NZCV, TRI))) {
            return true;
        }
    }
    return false;
...
AArch64InstrInfo::canRemovePTestInstr(MachineInstr* PTest, MachineInstr* Mask, MachineInstr* Pred, const MachineRegisterInfo* MRI) const {
    ...
    if (PredIsWhileLike) {
        ...
        // mask.
        if (isPTrueOpcode(MaskOpcode) && Mask->getOperand(1).getImm() == 31 && getElementSizeForOpcode(MaskOpcode) == getElementSizeForOpcode(PredOpcode)) {
            return PredOpcode;
        }
    ...
    if (PredIsPTestLike) {
        ...
        if (Mask == PTestLikeMask && (PredElementSize == AArch64::ElementSizeB || PTest->getOpcode() == AArch64::PTEST_PP_ANY)) {
            return PredOpcode;
        }
    ...
    switch (PredOpcode) {
        ...
        case AArch64::BRKN_PPzP: {
            ...
            if ((MaskOpcode != AArch64::PTRUE_B) || (Mask->getOperand(1).getImm() != 31)) {
                return {};
            }
            break;
        }
...
bool AArch64InstrInfo::optimizePTestInstr(
    MachineInstr* PTest, unsigned MaskReg, unsigned PredReg, const MachineRegisterInfo* MRI) const {
...
bool AArch64InstrInfo::optimizeCompareInstr(
    MachineInstr& CmpInstr, Register SrcReg, Register SrcReg2, int64_t CmpMask, int64_t CmpValue, const MachineRegisterInfo* MRI) const {
    assert(CmpInstr.getParent());
    assert(MRI);

    // Replace SUBSWrr with SUBWrr if NZCV is not used.
    int DeadNZCVIdx = CmpInstr.findRegisterDefOperandIdx(AArch64::NZCV, /*TRI=*/nullptr, true);
    if (DeadNZCVIdx != -1) {
        if (CmpInstr.definesRegister(AArch64::WZR, /*TRI=*/nullptr) || CmpInstr.definesRegister(AArch64::XZR, /*TRI=*/nullptr)) {
    ...
    if (CmpInstr.getOpcode() == AArch64::PTEST_PP || CmpInstr.getOpcode() == AArch64::PTEST_PP_ANY) {
    ...
    return (CmpValue == 0 || CmpValue == 1) && removeCmpToZeroOrOne(CmpInstr, SrcReg, CmpValue, *MRI);
}
...
llvm::examineCFlagsUse(MachineInstr& MI, MachineInstr& CmpInstr, const TargetRegisterInfo& TRI, SmallVectorImpl<MachineInstr*>* CCUseInstrs) {
...
static bool canInstrSubstituteCmpInstr(MachineInstr& MI, MachineInstr& CmpInstr, const TargetRegisterInfo& TRI) {
    ...
    assert((CmpInstr.getOperand(2).isImm() && CmpInstr.getOperand(2).getImm() == 0) && "Caller guarantees that CmpInstr compares with constant 0");
...
bool AArch64InstrInfo::substituteCmpToZero(
    MachineInstr& CmpInstr, unsigned SrcReg, const MachineRegisterInfo& MRI) const {
...
static bool canCmpInstrBeRemoved(MachineInstr& MI, MachineInstr& CmpInstr, int CmpValue, const TargetRegisterInfo& TRI, SmallVectorImpl<MachineInstr*>& CCUseInstrs, bool& IsInvertCC) {
    assert((CmpValue == 0 || CmpValue == 1) && "Only comparisons to 0 or 1 considered for removal!");

    // MI is 'CSINCWr %vreg, wzr, wzr, <cc>' or 'CSINCXr %vreg, xzr, xzr, <cc>'
    unsigned MIOpc = MI.getOpcode();
    if (MIOpc == AArch64::CSINCWr) {
        if (MI.getOperand(1).getReg() != AArch64::WZR || MI.getOperand(2).getReg() != AArch64::WZR) {
            return false;
        }
    } else if (MIOpc == AArch64::CSINCXr) {
        if (MI.getOperand(1).getReg() != AArch64::XZR || MI.getOperand(2).getReg() != AArch64::XZR) {
            return false;
        }
    } else {
        return false;
    ...
    std::optional<UsedNZCV> NZCVUsedAfterCmp = examineCFlagsUse(MI, CmpInstr, TRI, &CCUseInstrs);
    ...
    if ((MIUsedNZCV.Z && NZCVUsedAfterCmp->N) || (MIUsedNZCV.N && NZCVUsedAfterCmp->Z)) {
        return false;
    }
    ...
    IsInvertCC = (CmpValue && (MICC == AArch64CC::EQ || MICC == AArch64CC::PL)) || (!CmpValue && MICC == AArch64CC::NE);
    return true;
}
...
bool AArch64InstrInfo::removeCmpToZeroOrOne(
    MachineInstr& CmpInstr, unsigned SrcReg, int CmpValue, const MachineRegisterInfo& MRI) const {
    MachineInstr* MI = MRI.getUniqueVRegDef(SrcReg);
    if (!MI) {
        return false;
    }
...
bool AArch64InstrInfo::expandPostRAPseudo(MachineInstr& MI) const {
    if (MI.getOpcode() != TargetOpcode::LOAD_STACK_GUARD && MI.getOpcode() != AArch64::CATCHRET) {
        return false;
    }
    ...
    if (MI.getOpcode() == AArch64::CATCHRET) {
        ...
        while (FirstEpilogSEH->getFlag(MachineInstr::FrameDestroy) && FirstEpilogSEH != MBB.begin()) {
    ...
    if (M.getStackProtectorGuard() == "sysreg") {
        const AArch64SysReg::SysReg* SrcReg = AArch64SysReg::lookupSysRegByName(M.getStackProtectorGuardReg());
        if (!SrcReg) {
    ...
    const GlobalValue* GV = cast<GlobalValue>((*MI.memoperands_begin())->getValue());
    ...
    switch (MI.getOpcode()) {
        ...
        case AArch64::MOVZXi: // movz Rd, #0 (LSL #0)
            if (MI.getOperand(1).isImm() && MI.getOperand(1).getImm() == 0) {
                assert(MI.getDesc().getNumOperands() == 3 && MI.getOperand(2).getImm() == 0 && "invalid MOVZi operands");
                return true;
            }
            break;
...
bool AArch64InstrInfo::isGPRCopy(const MachineInstr& MI) {
    switch (MI.getOpcode()) {
        ...
        case TargetOpcode::COPY: {
            ...
            return (AArch64::GPR32RegClass.contains(DstReg) || AArch64::GPR64RegClass.contains(DstReg));
        }
        case AArch64::ORRXrs: // orr Xd, Xzr, Xm (LSL #0)
            if (MI.getOperand(1).getReg() == AArch64::XZR) {
                assert(MI.getDesc().getNumOperands() == 4 && MI.getOperand(3).getImm() == 0 && "invalid ORRrs operands");
                return true;
            }
            break;
        case AArch64::ADDXri: // add Xd, Xn, #0 (LSL #0)
            if (MI.getOperand(2).getImm() == 0) {
                assert(MI.getDesc().getNumOperands() == 4 && MI.getOperand(3).getImm() == 0 && "invalid ADDXri operands");
                return true;
            }
            break;
    }
    ...
    switch (MI.getOpcode()) {
        ...
        case AArch64::ORRv16i8:
            if (MI.getOperand(1).getReg() == MI.getOperand(2).getReg()) {
                assert(MI.getDesc().getNumOperands() == 3 && MI.getOperand(0).isReg() && "invalid ORRv16i8 operands");
                return true;
            }
            break;
    }
...
bool AArch64InstrInfo::isTailCallReturnInst(const MachineInstr& MI) {
    switch (MI.getOpcode()) {
        default:
            assert((!MI.isCall() || !MI.isReturn()) && "Unexpected instruction - was a new tail call opcode introduced?");
            return false;
...
bool AArch64InstrInfo::isCandidateToMergeOrPair(const MachineInstr& MI) const {
    ...
    assert((MI.getOperand(IsPreLdSt ? 2 : 1).isReg() || MI.getOperand(IsPreLdSt ? 2 : 1).isFI()) && "Expected a reg or frame index operand.");
    ...
    bool NeedsWinCFI = MAI->usesWindowsCFI() && MI.getMF()->getFunction().needsUnwindTableEntry();
    if (NeedsWinCFI && (MI.getFlag(MachineInstr::FrameSetup) || MI.getFlag(MachineInstr::FrameDestroy))) {
        return false;
    }
...
bool AArch64InstrInfo::getMemOperandsWithOffsetWidth(
    const MachineInstr& LdSt, SmallVectorImpl<const MachineOperand*>& BaseOps, int64_t& Offset, bool& OffsetIsScalable, LocationSize& Width, const TargetRegisterInfo* TRI) const {
...
bool AArch64InstrInfo::canFoldIntoAddrMode(const MachineInstr& MemI,
    ...
    if (MemI.getOperand(2).isReg()) {
        ...
        switch (AddrI.getOpcode()) {
            ...
            case AArch64::SBFMXri:
                ...
                // ->
                // ldr Xd, [Xn, Wm, sxtw #N]
                if (AddrI.getOperand(2).getImm() != 0 || AddrI.getOperand(3).getImm() != 31) {
                    return false;
                }
            ...
            case TargetOpcode::SUBREG_TO_REG: {
                ...
                if (AddrI.getOperand(1).getImm() != 0 || AddrI.getOperand(3).getImm() != AArch64::sub_32) {
                    return false;
                }
                ...
                if (DefMI.getOpcode() != AArch64::ORRWrs || DefMI.getOperand(1).getReg() != AArch64::WZR || DefMI.getOperand(3).getImm() != 0) {
                    return false;
                }
    ...
    auto validateOffsetForLDP = [](unsigned NumBytes, int64_t OldOffset,
        ...
        return OldOffset < MinOffset || OldOffset > MaxOffset || (NewOffset >= MinOffset && NewOffset <= MaxOffset);
    };
    ...
    auto avoidSlowSTRQ = [&](const MachineInstr& MemI) {
        ...
        return (Opcode == AArch64::STURQi || Opcode == AArch64::STRQui) && Subtarget.isSTRQroSlow();
    };
...
MachineInstr* AArch64InstrInfo::emitLdStWithAddr(MachineInstr& MemI,
    ...
    if (AM.Form == ExtAddrMode::Formula::Basic) {
        ...
        assert(AM.ScaledReg == 0 && AM.Scale == 0 && "Addressing mode not supported for folding");
    ...
    if (AM.Form == ExtAddrMode::Formula::SExtScaledReg || AM.Form == ExtAddrMode::Formula::ZExtScaledReg) {
        // The new instruction will be in the form `ldr Rt, [Xn, Wm, {s,u}xtw #N]`.
        assert(AM.ScaledReg && !AM.Displacement && "Address offset can be a register or an immediate, but not both");
...
bool AArch64InstrInfo::getMemOperandWithOffsetWidth(
    const MachineInstr& LdSt, const MachineOperand*& BaseOp, int64_t& Offset, bool& OffsetIsScalable, TypeSize& Width, const TargetRegisterInfo* TRI) const {
    ...
    if (LdSt.getNumExplicitOperands() == 3) {
        ...
        if (!LdSt.getOperand(1).isReg() || (!LdSt.getOperand(2).isReg() && !LdSt.getOperand(2).isFI()) || !LdSt.getOperand(3).isImm()) {
            return false;
        }
    } else {
        return false;
...
const MachineOperand& AArch64InstrInfo::getLdStBaseOp(const MachineInstr& MI) {
    ...
    unsigned Idx = AArch64InstrInfo::isPairedLdSt(MI) || AArch64InstrInfo::isPreLdSt(MI) ? 2
                                                                                         : 1;
...
AArch64InstrInfo::getLdStOffsetOp(const MachineInstr& MI) {
    assert(MI.mayLoadOrStore() && "Load or store instruction expected");
    unsigned Idx = AArch64InstrInfo::isPairedLdSt(MI) || AArch64InstrInfo::isPreLdSt(MI) ? 3
                                                                                         : 2;
    return MI.getOperand(Idx);
}
...
bool AArch64InstrInfo::isQForm(const MachineInstr& MI) {
    auto IsQFPR = [&](const MachineOperand& Op) {
        ...
        const TargetRegisterClass* TRC = ::getRegClass(MI, Reg);
        return TRC == &AArch64::FPR128RegClass || TRC == &AArch64::FPR128_loRegClass;
    };
...
bool AArch64InstrInfo::isFpOrNEON(Register Reg) {
    ...
    return AArch64::FPR128RegClass.contains(Reg) || AArch64::FPR64RegClass.contains(Reg) || AArch64::FPR32RegClass.contains(Reg) || AArch64::FPR16RegClass.contains(Reg) || AArch64::FPR8RegClass.contains(Reg);
}
...
bool AArch64InstrInfo::isFpOrNEON(const MachineInstr& MI) {
    auto IsFPR = [&](const MachineOperand& Op) {
        ...
        const TargetRegisterClass* TRC = ::getRegClass(MI, Reg);
        return TRC == &AArch64::FPR128RegClass || TRC == &AArch64::FPR128_loRegClass || TRC == &AArch64::FPR64RegClass || TRC == &AArch64::FPR64_loRegClass || TRC == &AArch64::FPR32RegClass || TRC == &AArch64::FPR16RegClass || TRC == &AArch64::FPR8RegClass;
    };
...
bool AArch64InstrInfo::shouldClusterMemOps(
    ArrayRef<const MachineOperand*> BaseOps1, int64_t OpOffset1, bool OffsetIsScalable1, ArrayRef<const MachineOperand*> BaseOps2, int64_t OpOffset2, bool OffsetIsScalable2, unsigned ClusterSize, unsigned NumBytes) const {
    ...
    assert((BaseOp1.isReg() || BaseOp1.isFI()) && "Only base registers and frame indices are supported.");
    ...
    if (!isCandidateToMergeOrPair(FirstLdSt) || !isCandidateToMergeOrPair(SecondLdSt)) {
        return false;
    }
    ...
    if (BaseOp1.isFI()) {
        assert((!BaseOp1.isIdenticalTo(BaseOp2) || Offset1 <= Offset2) && "Caller should have ordered offsets.");

        const MachineFrameInfo& MFI = FirstLdSt.getParent()->getParent()->getFrameInfo();
...
void AArch64InstrInfo::copyGPRRegTuple(MachineBasicBlock& MBB,
    ...
    uint16_t DestEncoding = TRI->getEncodingValue(DestReg);
    uint16_t SrcEncoding = TRI->getEncodingValue(SrcReg);
    assert(DestEncoding % NumRegs == 0 && SrcEncoding % NumRegs == 0 && "GPR reg sequences should not be able to overlap");
#endif
...
void AArch64InstrInfo::copyPhysReg(MachineBasicBlock& MBB,
    ...
    if (AArch64::GPR32spRegClass.contains(DestReg) && (AArch64::GPR32spRegClass.contains(SrcReg) || SrcReg == AArch64::WZR)) {
        const TargetRegisterInfo* TRI = &getRegisterInfo();
    ...
    if (AArch64::PPRRegClass.contains(DestReg) && AArch64::PPRRegClass.contains(SrcReg)) {
        assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected SVE register.");
    ...
    if (AArch64::ZPRRegClass.contains(DestReg) && AArch64::ZPRRegClass.contains(SrcReg)) {
        assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected SVE register.");
    ...
    if ((AArch64::ZPR2RegClass.contains(DestReg) || AArch64::ZPR2StridedOrContiguousRegClass.contains(DestReg)) && (AArch64::ZPR2RegClass.contains(SrcReg) || AArch64::ZPR2StridedOrContiguousRegClass.contains(SrcReg))) {
        assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected SVE register.");
    ...
    if (AArch64::ZPR3RegClass.contains(DestReg) && AArch64::ZPR3RegClass.contains(SrcReg)) {
        assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected SVE register.");
        static const unsigned Indices[] = {AArch64::zsub0, AArch64::zsub1,
            AArch64::zsub2};
        copyPhysRegTuple(MBB, I, DL, DestReg, SrcReg, KillSrc, AArch64::ORR_ZZZ,
    ...
    if ((AArch64::ZPR4RegClass.contains(DestReg) || AArch64::ZPR4StridedOrContiguousRegClass.contains(DestReg)) && (AArch64::ZPR4RegClass.contains(SrcReg) || AArch64::ZPR4StridedOrContiguousRegClass.contains(SrcReg))) {
        assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected SVE register.");
        static const unsigned Indices[] = {AArch64::zsub0, AArch64::zsub1,
    ...
    if (AArch64::GPR64spRegClass.contains(DestReg) && (AArch64::GPR64spRegClass.contains(SrcReg) || SrcReg == AArch64::XZR)) {
    ...
    if (AArch64::DDDDRegClass.contains(DestReg) && AArch64::DDDDRegClass.contains(SrcReg)) {
    ...
    if (AArch64::DDDRegClass.contains(DestReg) && AArch64::DDDRegClass.contains(SrcReg)) {
        static const unsigned Indices[] = {AArch64::dsub0, AArch64::dsub1,
            AArch64::dsub2};
        copyPhysRegTuple(MBB, I, DL, DestReg, SrcReg, KillSrc, AArch64::ORRv8i8,
            Indices);
    ...
    if (AArch64::DDRegClass.contains(DestReg) && AArch64::DDRegClass.contains(SrcReg)) {
        static const unsigned Indices[] = {AArch64::dsub0, AArch64::dsub1};
        copyPhysRegTuple(MBB, I, DL, DestReg, SrcReg, KillSrc, AArch64::ORRv8i8,
            Indices);
        return;
    ...
    if (AArch64::QQQQRegClass.contains(DestReg) && AArch64::QQQQRegClass.contains(SrcReg)) {
    ...
    if (AArch64::QQQRegClass.contains(DestReg) && AArch64::QQQRegClass.contains(SrcReg)) {
        static const unsigned Indices[] = {AArch64::qsub0, AArch64::qsub1,
            AArch64::qsub2};
        copyPhysRegTuple(MBB, I, DL, DestReg, SrcReg, KillSrc, AArch64::ORRv16i8,
            Indices);
    ...
    if (AArch64::QQRegClass.contains(DestReg) && AArch64::QQRegClass.contains(SrcReg)) {
        static const unsigned Indices[] = {AArch64::qsub0, AArch64::qsub1};
        copyPhysRegTuple(MBB, I, DL, DestReg, SrcReg, KillSrc, AArch64::ORRv16i8,
            Indices);
        return;
    ...
    if (AArch64::XSeqPairsClassRegClass.contains(DestReg) && AArch64::XSeqPairsClassRegClass.contains(SrcReg)) {
    ...
    if (AArch64::WSeqPairsClassRegClass.contains(DestReg) && AArch64::WSeqPairsClassRegClass.contains(SrcReg)) {
    ...
    if (AArch64::FPR128RegClass.contains(DestReg) && AArch64::FPR128RegClass.contains(SrcReg)) {
        if (Subtarget.isSVEorStreamingSVEAvailable() && !Subtarget.isNeonAvailable()) {
    ...
    if (AArch64::FPR64RegClass.contains(DestReg) && AArch64::FPR64RegClass.contains(SrcReg)) {
        BuildMI(MBB, I, DL, get(AArch64::FMOVDr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }

    if (AArch64::FPR32RegClass.contains(DestReg) && AArch64::FPR32RegClass.contains(SrcReg)) {
        BuildMI(MBB, I, DL, get(AArch64::FMOVSr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }

    if (AArch64::FPR16RegClass.contains(DestReg) && AArch64::FPR16RegClass.contains(SrcReg)) {
        DestReg = RI.getMatchingSuperReg(DestReg, AArch64::hsub, &AArch64::FPR32RegClass);
        SrcReg = RI.getMatchingSuperReg(SrcReg, AArch64::hsub, &AArch64::FPR32RegClass);
        BuildMI(MBB, I, DL, get(AArch64::FMOVSr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }

    if (AArch64::FPR8RegClass.contains(DestReg) && AArch64::FPR8RegClass.contains(SrcReg)) {
        DestReg = RI.getMatchingSuperReg(DestReg, AArch64::bsub, &AArch64::FPR32RegClass);
        SrcReg = RI.getMatchingSuperReg(SrcReg, AArch64::bsub, &AArch64::FPR32RegClass);
        BuildMI(MBB, I, DL, get(AArch64::FMOVSr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }
    ...
    if (AArch64::FPR64RegClass.contains(DestReg) && AArch64::GPR64RegClass.contains(SrcReg)) {
        BuildMI(MBB, I, DL, get(AArch64::FMOVXDr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }
    if (AArch64::GPR64RegClass.contains(DestReg) && AArch64::FPR64RegClass.contains(SrcReg)) {
        BuildMI(MBB, I, DL, get(AArch64::FMOVDXr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }
    // Copies between GPR32 and FPR32.
    if (AArch64::FPR32RegClass.contains(DestReg) && AArch64::GPR32RegClass.contains(SrcReg)) {
        BuildMI(MBB, I, DL, get(AArch64::FMOVWSr), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
        return;
    }
    if (AArch64::GPR32RegClass.contains(DestReg) && AArch64::FPR32RegClass.contains(SrcReg)) {
...
void AArch64InstrInfo::storeRegToStackSlot(MachineBasicBlock& MBB,
    ...
    MachineMemOperand* MMO = MF.getMachineMemOperand(PtrInfo, MachineMemOperand::MOStore,
    ...
    switch (TRI->getSpillSize(*RC)) {
        ...
        case 2: {
            if (AArch64::FPR16RegClass.hasSubClassEq(RC)) {
                Opc = AArch64::STRHui;
            } else if (AArch64::PNRRegClass.hasSubClassEq(RC) || AArch64::PPRRegClass.hasSubClassEq(RC)) {
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected register store without SVE store instructions");
                Opc = AArch64::STR_PXI;
        ...
        case 16:
            if (AArch64::FPR128RegClass.hasSubClassEq(RC)) {
                ...
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected predicate store without SVE store instructions");
        ...
        case 32:
            if (AArch64::DDDDRegClass.hasSubClassEq(RC)) {
                ...
                Offset = false;
            } else if (AArch64::ZPR2RegClass.hasSubClassEq(RC) || AArch64::ZPR2StridedOrContiguousRegClass.hasSubClassEq(RC)) {
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected register store without SVE store instructions");
        ...
        case 64:
            if (AArch64::QQQQRegClass.hasSubClassEq(RC)) {
                ...
                Offset = false;
            } else if (AArch64::ZPR4RegClass.hasSubClassEq(RC) || AArch64::ZPR4StridedOrContiguousRegClass.hasSubClassEq(RC)) {
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected register store without SVE store instructions");
...
void AArch64InstrInfo::loadRegFromStackSlot(
    MachineBasicBlock& MBB, MachineBasicBlock::iterator MBBI, Register DestReg, int FI, const TargetRegisterClass* RC, const TargetRegisterInfo* TRI, Register VReg, MachineInstr::MIFlag Flags) const {
    MachineFunction& MF = *MBB.getParent();
    MachineFrameInfo& MFI = MF.getFrameInfo();
    MachinePointerInfo PtrInfo = MachinePointerInfo::getFixedStack(MF, FI);
    MachineMemOperand* MMO = MF.getMachineMemOperand(PtrInfo, MachineMemOperand::MOLoad,
        MFI.getObjectSize(FI), MFI.getObjectAlign(FI));
    ...
    switch (TRI->getSpillSize(*RC)) {
        ...
        case 2: {
            ...
            if (AArch64::FPR16RegClass.hasSubClassEq(RC)) {
                ...
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected register load without SVE load instructions");
                if (IsPNR) {
                    PNRReg = DestReg;
                }
                Opc = AArch64::LDR_PXI;
        ...
        case 16:
            if (AArch64::FPR128RegClass.hasSubClassEq(RC)) {
                ...
                assert(Subtarget.isSVEorStreamingSVEAvailable() && "Unexpected predicate load without SVE load instructions");
...
void AArch64InstrInfo::decomposeStackOffsetForFrameOffsets(
    const StackOffset& Offset, int64_t& NumBytes, int64_t& NumPredicateVectors, int64_t& NumDataVectors) {
    // The smallest scalable element supported by scaled SVE addressing
    // modes are predicates, which are 2 scalable bytes in size. So the scalable
    // byte offset must always be a multiple of 2.
    assert(Offset.getScalable() % 2 == 0 && "Invalid frame offset");
    ...
    if (NumPredicateVectors % 8 == 0 || NumPredicateVectors < -64 || NumPredicateVectors > 62) {
...
static void emitFrameOffsetAdj(MachineBasicBlock& MBB,
    ...
    do {
        ...
        assert((ThisVal >> ShiftSize) <= MaxEncoding && "Encoding cannot handle value that big");
        ...
        if (NeedsWinCFI) {
            ...
            if ((DestReg == AArch64::FP && SrcReg == AArch64::SP) || (SrcReg == AArch64::FP && DestReg == AArch64::SP)) {
                if (HasWinCFI) {
                    *HasWinCFI = true;
                }
                if (Imm == 0) {
...
void llvm::emitFrameOffset(MachineBasicBlock& MBB,
    ...
    if (Bytes || (!Offset && SrcReg != DestReg)) {
        assert((DestReg != AArch64::SP || Bytes % 8 == 0) && "SP increment/decrement not 8-byte aligned");
    ...
    assert(!(SetNZCV && (NumPredicateVectors || NumDataVectors)) && "SetNZCV not supported with SVE vectors");
    assert(!(NeedsWinCFI && (NumPredicateVectors || NumDataVectors)) && "WinCFI not supported with SVE vectors");
...
MachineInstr* AArch64InstrInfo::foldMemoryOperandImpl(
    MachineFunction& MF, MachineInstr& MI, ArrayRef<unsigned> Ops, MachineBasicBlock::iterator InsertPt, int FrameIndex, LiveIntervals* LIS, VirtRegMap* VRM) const {
    ...
    if (MI.isCopy() && Ops.size() == 1 &&
        ...
        if (DstMO.getSubReg() == 0 && SrcMO.getSubReg() == 0) {
            assert(TRI.getRegSizeInBits(*getRegClass(DstReg)) == TRI.getRegSizeInBits(*getRegClass(SrcReg)) && "Mismatched register size in non subreg COPY");
            if (IsSpill) {
        ...
        //   STRXui %xzr, %stack.0
        //
        if (IsSpill && DstMO.isUndef() && SrcReg == AArch64::WZR && TRI.getRegSizeInBits(*getRegClass(DstReg)) == 64) {
            assert(SrcMO.getSubReg() == 0 && "Unexpected subreg on physical register");
        ...
        if (IsFill && SrcMO.getSubReg() == 0 && DstMO.isUndef()) {
            ...
            if (FillRC) {
                assert(TRI.getRegSizeInBits(*getRegClass(SrcReg)) == TRI.getRegSizeInBits(*FillRC) && "Mismatched regclass size on folded subreg COPY");
...
int llvm::isAArch64FrameOffsetLegal(const MachineInstr& MI,
    ...
    const MachineOperand& ImmOpnd = MI.getOperand(AArch64InstrInfo::getLoadStoreImmIdx(MI.getOpcode()));
    Offset += ImmOpnd.getImm() * Scale;

    // If the offset doesn't match the scale, we rewrite the instruction to
    // use the unscaled instruction instead. Likewise, if we have a negative
    // offset and there is an unscaled op to use.
    std::optional<unsigned> UnscaledOp = AArch64InstrInfo::getUnscaledLdSt(MI.getOpcode());
    bool useUnscaledOp = UnscaledOp && (Offset % Scale || Offset < 0);
    if (useUnscaledOp && !AArch64InstrInfo::getMemOpInfo(*UnscaledOp, ScaleValue, Width, MinOff, MaxOff)) {
        llvm_unreachable("unhandled opcode in isAArch64FrameOffsetLegal");
    }

    Scale = ScaleValue.getKnownMinValue();
    assert(IsMulVL == ScaleValue.isScalable() && "Unscaled opcode has different value for scalable");

    int64_t Remainder = Offset % Scale;
    assert(!(Remainder && useUnscaledOp) && "Cannot have remainder when using unscaled op");
    ...
    return AArch64FrameOffsetCanUpdate | (SOffset ? 0 : AArch64FrameOffsetIsLegal);
}
...
static bool isCombineInstrCandidateFP(const MachineInstr& Inst) {
    switch (Inst.getOpcode()) {
        ...
        case AArch64::FSUBv4f32:
            ...
            return Options.UnsafeFPMath || Options.AllowFPOpFusion == FPOpFusion::Fast || Inst.getFlag(MachineInstr::FmContract);
            return true;
    }
    return false;
}
...
static bool canCombine(MachineBasicBlock& MBB, MachineOperand& MO, unsigned CombineOpc, unsigned ZeroReg = 0, bool CheckZeroReg = false) {
    ...
    if (CheckZeroReg) {
        assert(MI->getNumOperands() >= 4 && MI->getOperand(0).isReg() && MI->getOperand(1).isReg() && MI->getOperand(2).isReg() && MI->getOperand(3).isReg() && "MAdd/MSub must have a least 4 regs");
    ...
    if (isCombineInstrSettingFlag(CombineOpc) && MI->findRegisterDefOperandIdx(AArch64::NZCV, /*TRI=*/nullptr, true) == -1) {
        return false;
    }
...
bool AArch64InstrInfo::isAssociativeAndCommutative(const MachineInstr& Inst,
    ...
    switch (Inst.getOpcode()) {
        ...
        case AArch64::FMUL_ZZZ_D:
            return Inst.getParent()->getParent()->getTarget().Options.UnsafeFPMath || (Inst.getFlag(MachineInstr::MIFlag::FmReassoc) && Inst.getFlag(MachineInstr::MIFlag::FmNsz));
...
static bool getMaddPatterns(MachineInstr& Root,
    ...
    if (isCombineInstrSettingFlag(Opc)) {
        int Cmp_NZCV = Root.findRegisterDefOperandIdx(AArch64::NZCV, /*TRI=*/nullptr, true);
    ...
    switch (Opc) {
        default:
            break;
        case AArch64::ADDWrr:
            assert(Root.getOperand(1).isReg() && Root.getOperand(2).isReg() && "ADDWrr does not have register operands");
...
static bool getFMAPatterns(MachineInstr& Root,
    ...
    switch (Root.getOpcode()) {
        default:
            ...
            break;
        case AArch64::FADDHrr:
            assert(Root.getOperand(1).isReg() && Root.getOperand(2).isReg() && "FADDHrr does not have register operands");

            Found = Match(AArch64::FMULHrr, 1, MCP::FMULADDH_OP1);
            Found |= Match(AArch64::FMULHrr, 2, MCP::FMULADDH_OP2);
            break;
        case AArch64::FADDSrr:
            assert(Root.getOperand(1).isReg() && Root.getOperand(2).isReg() && "FADDSrr does not have register operands");

            Found |= Match(AArch64::FMULSrr, 1, MCP::FMULADDS_OP1) || Match(AArch64::FMULv1i32_indexed, 1, MCP::FMLAv1i32_indexed_OP1);

            Found |= Match(AArch64::FMULSrr, 2, MCP::FMULADDS_OP2) || Match(AArch64::FMULv1i32_indexed, 2, MCP::FMLAv1i32_indexed_OP2);
            break;
        case AArch64::FADDDrr:
            Found |= Match(AArch64::FMULDrr, 1, MCP::FMULADDD_OP1) || Match(AArch64::FMULv1i64_indexed, 1, MCP::FMLAv1i64_indexed_OP1);

            Found |= Match(AArch64::FMULDrr, 2, MCP::FMULADDD_OP2) || Match(AArch64::FMULv1i64_indexed, 2, MCP::FMLAv1i64_indexed_OP2);
            break;
        case AArch64::FADDv4f16:
            Found |= Match(AArch64::FMULv4i16_indexed, 1, MCP::FMLAv4i16_indexed_OP1) || Match(AArch64::FMULv4f16, 1, MCP::FMLAv4f16_OP1);

            Found |= Match(AArch64::FMULv4i16_indexed, 2, MCP::FMLAv4i16_indexed_OP2) || Match(AArch64::FMULv4f16, 2, MCP::FMLAv4f16_OP2);
            break;
        case AArch64::FADDv8f16:
            Found |= Match(AArch64::FMULv8i16_indexed, 1, MCP::FMLAv8i16_indexed_OP1) || Match(AArch64::FMULv8f16, 1, MCP::FMLAv8f16_OP1);

            Found |= Match(AArch64::FMULv8i16_indexed, 2, MCP::FMLAv8i16_indexed_OP2) || Match(AArch64::FMULv8f16, 2, MCP::FMLAv8f16_OP2);
            break;
        case AArch64::FADDv2f32:
            Found |= Match(AArch64::FMULv2i32_indexed, 1, MCP::FMLAv2i32_indexed_OP1) || Match(AArch64::FMULv2f32, 1, MCP::FMLAv2f32_OP1);

            Found |= Match(AArch64::FMULv2i32_indexed, 2, MCP::FMLAv2i32_indexed_OP2) || Match(AArch64::FMULv2f32, 2, MCP::FMLAv2f32_OP2);
            break;
        case AArch64::FADDv2f64:
            Found |= Match(AArch64::FMULv2i64_indexed, 1, MCP::FMLAv2i64_indexed_OP1) || Match(AArch64::FMULv2f64, 1, MCP::FMLAv2f64_OP1);

            Found |= Match(AArch64::FMULv2i64_indexed, 2, MCP::FMLAv2i64_indexed_OP2) || Match(AArch64::FMULv2f64, 2, MCP::FMLAv2f64_OP2);
            break;
        case AArch64::FADDv4f32:
            Found |= Match(AArch64::FMULv4i32_indexed, 1, MCP::FMLAv4i32_indexed_OP1) || Match(AArch64::FMULv4f32, 1, MCP::FMLAv4f32_OP1);

            Found |= Match(AArch64::FMULv4i32_indexed, 2, MCP::FMLAv4i32_indexed_OP2) || Match(AArch64::FMULv4f32, 2, MCP::FMLAv4f32_OP2);
            break;
        case AArch64::FSUBHrr:
        ...
        case AArch64::FSUBSrr:
            ...
            Found |= Match(AArch64::FMULSrr, 2, MCP::FMULSUBS_OP2) || Match(AArch64::FMULv1i32_indexed, 2, MCP::FMLSv1i32_indexed_OP2);
        ...
        case AArch64::FSUBDrr:
            ...
            Found |= Match(AArch64::FMULDrr, 2, MCP::FMULSUBD_OP2) || Match(AArch64::FMULv1i64_indexed, 2, MCP::FMLSv1i64_indexed_OP2);

            Found |= Match(AArch64::FNMULDrr, 1, MCP::FNMULSUBD_OP1);
            break;
        case AArch64::FSUBv4f16:
            Found |= Match(AArch64::FMULv4i16_indexed, 2, MCP::FMLSv4i16_indexed_OP2) || Match(AArch64::FMULv4f16, 2, MCP::FMLSv4f16_OP2);

            Found |= Match(AArch64::FMULv4i16_indexed, 1, MCP::FMLSv4i16_indexed_OP1) || Match(AArch64::FMULv4f16, 1, MCP::FMLSv4f16_OP1);
            break;
        case AArch64::FSUBv8f16:
            Found |= Match(AArch64::FMULv8i16_indexed, 2, MCP::FMLSv8i16_indexed_OP2) || Match(AArch64::FMULv8f16, 2, MCP::FMLSv8f16_OP2);

            Found |= Match(AArch64::FMULv8i16_indexed, 1, MCP::FMLSv8i16_indexed_OP1) || Match(AArch64::FMULv8f16, 1, MCP::FMLSv8f16_OP1);
            break;
        case AArch64::FSUBv2f32:
            Found |= Match(AArch64::FMULv2i32_indexed, 2, MCP::FMLSv2i32_indexed_OP2) || Match(AArch64::FMULv2f32, 2, MCP::FMLSv2f32_OP2);

            Found |= Match(AArch64::FMULv2i32_indexed, 1, MCP::FMLSv2i32_indexed_OP1) || Match(AArch64::FMULv2f32, 1, MCP::FMLSv2f32_OP1);
            break;
        case AArch64::FSUBv2f64:
            Found |= Match(AArch64::FMULv2i64_indexed, 2, MCP::FMLSv2i64_indexed_OP2) || Match(AArch64::FMULv2f64, 2, MCP::FMLSv2f64_OP2);

            Found |= Match(AArch64::FMULv2i64_indexed, 1, MCP::FMLSv2i64_indexed_OP1) || Match(AArch64::FMULv2f64, 1, MCP::FMLSv2f64_OP1);
            break;
        case AArch64::FSUBv4f32:
            Found |= Match(AArch64::FMULv4i32_indexed, 2, MCP::FMLSv4i32_indexed_OP2) || Match(AArch64::FMULv4f32, 2, MCP::FMLSv4f32_OP2);

            Found |= Match(AArch64::FMULv4i32_indexed, 1, MCP::FMLSv4i32_indexed_OP1) || Match(AArch64::FMULv4f32, 1, MCP::FMLSv4f32_OP1);
            break;
    }
    return Found;
}
...
static bool getFMULPatterns(MachineInstr& Root,
    ...
    auto Match = [&](unsigned Opcode, int Operand, unsigned Pattern) -> bool {
        ...
        if (MI && MI->getOpcode() == TargetOpcode::COPY && MI->getOperand(1).getReg().isVirtual()) {
...
static bool getFNEGPatterns(MachineInstr& Root,
    ...
    auto Match = [&](unsigned Opcode, unsigned Pattern) -> bool {
        ...
        if (MI != nullptr && (MI->getOpcode() == Opcode) && MRI.hasOneNonDBGUse(MI->getOperand(0).getReg()) && Root.getFlag(MachineInstr::MIFlag::FmContract) && Root.getFlag(MachineInstr::MIFlag::FmNsz) && MI->getFlag(MachineInstr::MIFlag::FmContract) && MI->getFlag(MachineInstr::MIFlag::FmNsz)) {
            Patterns.push_back(Pattern);
            return true;
        }
        return false;
...
static bool getMiscPatterns(MachineInstr& Root, SmallVectorImpl<unsigned>& Patterns) {
    ...
    if (isCombineInstrSettingFlag(Opc) && Root.findRegisterDefOperandIdx(AArch64::NZCV, /*TRI=*/nullptr, true) == -1) {
        return false;
    }

    if (canCombine(MBB, Root.getOperand(2), AArch64::ADDWrr) || canCombine(MBB, Root.getOperand(2), AArch64::ADDSWrr) || 
        canCombine(MBB, Root.getOperand(2), AArch64::ADDXrr) || canCombine(MBB, Root.getOperand(2), AArch64::ADDSXrr)
      ) {
...
enum class FMAInstKind { Default,
    Indexed,
    Accumulator };
...
static MachineInstr*
genFusedMultiply(MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, unsigned IdxMulOpd, unsigned MaddOpc, const TargetRegisterClass* RC, FMAInstKind kind = FMAInstKind::Default, const Register* ReplacedAddend = nullptr) {
...
static MachineInstr*
genFNegatedMAD(MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs) {
    ...
    MachineInstrBuilder MIB = BuildMI(MF, MIMetadata(Root), TII->get(Opc), ResultReg)
                                  .addReg(SrcReg0, getKillRegState(Src0IsKill))
                                  .addReg(SrcReg1, getKillRegState(Src1IsKill))
                                  .addReg(SrcReg2, getKillRegState(Src2IsKill));
    InsInstrs.push_back(MIB);
...
genIndexedMultiply(MachineInstr& Root,
    ...
    MachineInstr* Dup = MF.getRegInfo().getUniqueVRegDef(Root.getOperand(IdxDupOp).getReg());
...
static MachineInstr* genFusedMultiplyAcc(
    MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, unsigned IdxMulOpd, unsigned MaddOpc, const TargetRegisterClass* RC) {
...
static Register genNeg(MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg, unsigned MnegOpc, const TargetRegisterClass* RC) {
    ...
    MachineInstrBuilder MIB = BuildMI(MF, MIMetadata(Root), TII->get(MnegOpc), NewVR)
...
static MachineInstr* genFusedMultiplyAccNeg(
    MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg, unsigned IdxMulOpd, unsigned MaddOpc, unsigned MnegOpc, const TargetRegisterClass* RC) {
    assert(IdxMulOpd == 1);

    Register NewVR = genNeg(MF, MRI, TII, Root, InsInstrs, InstrIdxForVirtReg, MnegOpc, RC);
    return genFusedMultiply(MF, MRI, TII, Root, InsInstrs, IdxMulOpd, MaddOpc, RC,
...
static MachineInstr* genMaddR(MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, unsigned IdxMulOpd, unsigned MaddOpc, unsigned VR, const TargetRegisterClass* RC) {
    ...
    MachineInstrBuilder MIB = BuildMI(MF, MIMetadata(Root), TII->get(MaddOpc), ResultReg)
                                  .addReg(SrcReg0, getKillRegState(Src0IsKill))
                                  .addReg(SrcReg1, getKillRegState(Src1IsKill))
                                  .addReg(VR);
    // Insert the MADD
...
genSubAdd2SubSub(MachineFunction& MF, MachineRegisterInfo& MRI, const TargetInstrInfo* TII, MachineInstr& Root, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, unsigned IdxOpd1, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg) {
    ...
    Register NewVR = MRI.createVirtualRegister(MRI.getRegClass(Root.getOperand(2).getReg()));
    ...
    if (Opcode == AArch64::SUBSWrr) {
        ...
        assert((Opcode == AArch64::SUBWrr || Opcode == AArch64::SUBXrr) && "Unexpected instruction opcode.");
    }
    ...
    MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(Opcode), NewVR)
                                   .addReg(RegA, getKillRegState(RegAIsKill))
                                   .addReg(RegB, getKillRegState(RegBIsKill))
                                   .setMIFlags(Flags);
    MachineInstrBuilder MIB2 = BuildMI(MF, MIMetadata(Root), TII->get(Opcode), ResultReg)
...
void AArch64InstrInfo::genAlternativeCodeSequence(
    MachineInstr& Root, unsigned Pattern, SmallVectorImpl<MachineInstr*>& InsInstrs, SmallVectorImpl<MachineInstr*>& DelInstrs, DenseMap<unsigned, unsigned>& InstrIdxForVirtReg) const {
    MachineBasicBlock& MBB = *Root.getParent();
    MachineRegisterInfo& MRI = MBB.getParent()->getRegInfo();
    MachineFunction& MF = *MBB.getParent();
    const TargetInstrInfo* TII = MF.getSubtarget().getInstrInfo();
    ...
    switch (Pattern) {
        ...
        case AArch64MachineCombinerPattern::MULADDXI_OP1: {
            ...
            if (MovI->Opcode == OrrOpc) {
                ...
                if (BitSize == 32) {
                    assert((MovI->Opcode == AArch64::MOVNWi || MovI->Opcode == AArch64::MOVZWi) && "Expected opcode");
                } else {
                    assert((MovI->Opcode == AArch64::MOVNXi || MovI->Opcode == AArch64::MOVZXi) && "Expected opcode");
                }
        ...
        case AArch64MachineCombinerPattern::MULSUBX_OP1: {
            ...
            // SUB NewVR, 0, C
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(SubOpc), NewVR)
                                           .addReg(ZeroReg)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMLSv4i16_indexed_OP1: {
            RC = &AArch64::FPR64RegClass;
            Register NewVR = MRI.createVirtualRegister(RC);
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(AArch64::FNEGv4f16), NewVR)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMLSv8i16_indexed_OP1: {
            RC = &AArch64::FPR128RegClass;
            Register NewVR = MRI.createVirtualRegister(RC);
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(AArch64::FNEGv8f16), NewVR)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMLSv2i32_indexed_OP1: {
            RC = &AArch64::FPR64RegClass;
            Register NewVR = MRI.createVirtualRegister(RC);
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(AArch64::FNEGv2f32), NewVR)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMLSv4i32_indexed_OP1: {
            RC = &AArch64::FPR128RegClass;
            Register NewVR = MRI.createVirtualRegister(RC);
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(AArch64::FNEGv4f32), NewVR)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMLSv2i64_indexed_OP1: {
            RC = &AArch64::FPR128RegClass;
            Register NewVR = MRI.createVirtualRegister(RC);
            MachineInstrBuilder MIB1 = BuildMI(MF, MIMetadata(Root), TII->get(AArch64::FNEGv2f64), NewVR)
                                           .add(Root.getOperand(2));
            InsInstrs.push_back(MIB1);
            InstrIdxForVirtReg.insert(std::make_pair(NewVR, 0));
        ...
        case AArch64MachineCombinerPattern::FMULv2i32_indexed_OP1:
        case AArch64MachineCombinerPattern::FMULv2i32_indexed_OP2: {
            unsigned IdxDupOp = (Pattern == AArch64MachineCombinerPattern::FMULv2i32_indexed_OP1) ? 1
                                                                                                  : 2;
        ...
        case AArch64MachineCombinerPattern::FMULv2i64_indexed_OP1:
        case AArch64MachineCombinerPattern::FMULv2i64_indexed_OP2: {
            unsigned IdxDupOp = (Pattern == AArch64MachineCombinerPattern::FMULv2i64_indexed_OP1) ? 1
                                                                                                  : 2;
        ...
        case AArch64MachineCombinerPattern::FMULv4i16_indexed_OP1:
        case AArch64MachineCombinerPattern::FMULv4i16_indexed_OP2: {
            unsigned IdxDupOp = (Pattern == AArch64MachineCombinerPattern::FMULv4i16_indexed_OP1) ? 1
                                                                                                  : 2;
        ...
        case AArch64MachineCombinerPattern::FMULv4i32_indexed_OP1:
        case AArch64MachineCombinerPattern::FMULv4i32_indexed_OP2: {
            unsigned IdxDupOp = (Pattern == AArch64MachineCombinerPattern::FMULv4i32_indexed_OP1) ? 1
                                                                                                  : 2;
        ...
        case AArch64MachineCombinerPattern::FMULv8i16_indexed_OP1:
        case AArch64MachineCombinerPattern::FMULv8i16_indexed_OP2: {
            unsigned IdxDupOp = (Pattern == AArch64MachineCombinerPattern::FMULv8i16_indexed_OP1) ? 1
                                                                                                  : 2;
...
bool AArch64InstrInfo::optimizeCondBranch(MachineInstr& MI) const {
    ...
    switch (DefMI->getOpcode()) {
        ...
        case AArch64::CSINCWr:
        case AArch64::CSINCXr: {
            if (!(DefMI->getOperand(1).getReg() == AArch64::WZR && DefMI->getOperand(2).getReg() == AArch64::WZR) && !(DefMI->getOperand(1).getReg() == AArch64::XZR && DefMI->getOperand(2).getReg() == AArch64::XZR)) {
                return false;
            }
...
AArch64InstrInfo::getSerializableMachineMemOperandTargetFlags() const {
    static const std::pair<MachineMemOperand::Flags, const char*> TargetFlags[] = {{MOSuppressPair, "aarch64-suppress-pair"},
...
AArch64InstrInfo::findRegisterToSaveLRTo(outliner::Candidate& C) const {
    ...
    for (unsigned Reg : AArch64::GPR64RegClass) {
        if (!ARI->isReservedReg(*MF, Reg) && Reg != AArch64::LR && // LR is not reserved, but don't use it.
            Reg != AArch64::X16 && // X16 is not guaranteed to be preserved.
            Reg != AArch64::X17 && // Ditto for X17.
            C.isAvailableAcrossAndOutOfSeq(Reg, TRI) && C.isAvailableInsideSeq(Reg, TRI)) {
            return Reg;
        }
    }
    return Register();
...
outliningCandidatesSigningScopeConsensus(const outliner::Candidate& a,
    ...
    return MFIa->shouldSignReturnAddress(false) == MFIb->shouldSignReturnAddress(false) && MFIa->shouldSignReturnAddress(true) == MFIb->shouldSignReturnAddress(true);
}
...
static bool outliningCandidatesV8_3OpsConsensus(const outliner::Candidate& a,
    const outliner::Candidate& b) {
    const AArch64Subtarget& SubtargetA = a.getMF()->getSubtarget<AArch64Subtarget>();
    const AArch64Subtarget& SubtargetB = b.getMF()->getSubtarget<AArch64Subtarget>();
...
AArch64InstrInfo::getOutliningCandidateInfo(
    ...
    if (std::adjacent_find(
            ...
            [](const outliner::Candidate& a, const outliner::Candidate& b) {
                ...
                if (outliningCandidatesSigningScopeConsensus(a, b) && outliningCandidatesSigningKeyConsensus(a, b) && outliningCandidatesV8_3OpsConsensus(a, b)) {
                    return false;
                }
                return true;
            })
    ...
    if (RepeatedSequenceLocs[0]
        ...
        NumBytesToCheckLRInTCEpilogue = AArch64PAuth::getCheckerSizeInBytes(LRCheckMethod);
                ...
                if (MI.modifiesRegister(AArch64::SP, &TRI)) {
                    switch (MI.getOpcode()) {
                        ...
                        case AArch64::ADDWri:
                            ...
                            assert(MI.getOperand(2).isImm() && "Expected operand to be immediate");
                            assert(MI.getOperand(1).isReg() && "Expected operand to be a register");
    ...
    auto IsSafeToFixup = [this, &TRI](MachineInstr& MI) {
        ...
        if (!MI.modifiesRegister(AArch64::SP, &TRI) && !MI.readsRegister(AArch64::SP, &TRI)) {
            return true;
        }
        ...
        if (MI.mayLoadOrStore()) {
            ...
            // register SP?
            if (!getMemOperandWithOffset(MI, Base, Offset, OffsetIsScalable, &TRI) || !Base->isReg() || Base->getReg() != AArch64::SP) {
                return false;
            }
            ...
            if (Offset < MinOffset * (int64_t)Scale.getFixedValue() || Offset > MaxOffset * (int64_t)Scale.getFixedValue()) {
                return false;
            }
    ...
    bool AllStackInstrsSafe = llvm::all_of(RepeatedSequenceLocs[0], IsSafeToFixup);
    ...
    if (RepeatedSequenceLocs[0].back().isTerminator()) {
    ...
    else if (LastInstrOpcode == AArch64::BL || ((LastInstrOpcode == AArch64::BLR || LastInstrOpcode == AArch64::BLRNoIP) && !HasBTI)) {
        ...
        for (outliner::Candidate& C : RepeatedSequenceLocs) {
            bool LRAvailable = (C.Flags & MachineOutlinerMBBFlags::LRUnavailableSomewhere)
            ...
            bool IsNoReturn = C.getMF()->getFunction().hasFnAttribute(Attribute::NoReturn);
        ...
        if (!AllStackInstrsSafe || NumBytesNoStackCalls <= RepeatedSequenceLocs.size() * 12) {
            ...
            if (FlagsSetInAll & MachineOutlinerMBBFlags::HasCalls) {
                erase_if(RepeatedSequenceLocs, [this, &TRI](outliner::Candidate& C) {
                    ...
                    return (llvm::any_of(C, IsCall)) && (!C.isAvailableAcrossAndOutOfSeq(AArch64::LR, TRI) || !findRegisterToSaveLRTo(C));
                });
            }
        }
    ...
    if (FlagsSetInAll & MachineOutlinerMBBFlags::HasCalls) {
        ...
        // well.
        else if (FrameID != MachineOutlinerThunk && FrameID != MachineOutlinerTailCall && FirstCand.back().isCall()) {
            ModStackToSaveLR = true;
        }
...
AArch64InstrInfo::getOutlinableRanges(MachineBasicBlock& MBB,
    ...
    assert(MBB.getParent()->getRegInfo().tracksLiveness() && "Must track liveness!");
    SmallVector<
    ...
    auto AreAllUnsafeRegsDead = [&LRU]() {
        return LRU.available(AArch64::W16) && LRU.available(AArch64::W17) && LRU.available(AArch64::NZCV);
    };
...
AArch64InstrInfo::getOutliningTypeImpl(const MachineModuleInfo& MMI,
    ...
    for (const MachineOperand& MOP : MI.operands()) {
        ...
        if (MOP.isReg() && !MOP.isImplicit() && (MOP.getReg() == AArch64::LR || MOP.getReg() == AArch64::W30)) {
            return outliner::InstrType::Illegal;
        }
    }
    ...
    if (MI.isCall()) {
        ...
        if (MI.getOpcode() == AArch64::BLR || MI.getOpcode() == AArch64::BLRNoIP || MI.getOpcode() == AArch64::BL) {
        ...
        if (!MFI.isCalleeSavedInfoValid() || MFI.getStackSize() > 0 || MFI.getNumObjects() > 0) {
            return UnknownCallOutlineType;
        }
    ...
    if (MI.readsRegister(AArch64::W30, &getRegisterInfo()) || MI.modifiesRegister(AArch64::W30, &getRegisterInfo())) {
        return outliner::InstrType::Illegal;
    }
...
void AArch64InstrInfo::fixupPostOutline(MachineBasicBlock& MBB) const {
    for (MachineInstr& MI : MBB) {
        ...
        if (!MI.mayLoadOrStore() || !getMemOperandWithOffsetWidth(MI, Base, Offset, OffsetIsScalable, Width, &RI) || (Base->isReg() && Base->getReg() != AArch64::SP)) {
            continue;
        }
...
void AArch64InstrInfo::buildOutlinedFrame(
    MachineBasicBlock& MBB, MachineFunction& MF, const outliner::OutlinedFunction& OF) const {
    ...
    if (OF.FrameConstructionID == MachineOutlinerTailCall) {
        ...
        if (Call->getOpcode() == AArch64::BL) {
            ...
            assert(Call->getOpcode() == AArch64::BLR || Call->getOpcode() == AArch64::BLRNoIP);
    ...
    if (llvm::any_of(MBB.instrs(), IsNonTailCall)) {
        ...
        assert(OF.FrameConstructionID != MachineOutlinerDefault && "Can only fix up stack references once");
        ...
        if (OF.FrameConstructionID == MachineOutlinerTailCall || OF.FrameConstructionID == MachineOutlinerThunk) {
            Et = std::prev(MBB.end());
        }
        ...
        if (MF.getInfo<AArch64FunctionInfo>()->needsDwarfUnwindInfo(MF)) {
            ...
            int64_t StackPosEntry = MF.addFrameInst(MCCFIInstruction::cfiDefCfaOffset(nullptr, 16));
...
MachineBasicBlock::iterator AArch64InstrInfo::insertOutlinedCall(
    Module& M, MachineBasicBlock& MBB, MachineBasicBlock::iterator& It, MachineFunction& MF, outliner::Candidate& C) const {

    // Are we tail calling?
    if (C.CallConstructionID == MachineOutlinerTailCall) {
        // If yes, then we can just branch to the label.
        It = MBB.insert(It, BuildMI(MF, DebugLoc(), get(AArch64::TCRETURNdi)).addGlobalAddress(M.getNamedValue(MF.getName())).addImm(0));
        return It;
    }

    // Are we saving the link register?
    if (C.CallConstructionID == MachineOutlinerNoLRSave || C.CallConstructionID == MachineOutlinerThunk) {
        // No, so just insert the call.
        It = MBB.insert(It, BuildMI(MF, DebugLoc(), get(AArch64::BL)).addGlobalAddress(M.getNamedValue(MF.getName())));
        return It;
    }
...
void AArch64InstrInfo::buildClearRegister(Register Reg, MachineBasicBlock& MBB, MachineBasicBlock::iterator Iter, DebugLoc& DL, bool AllowSideEffects) const {
    const MachineFunction& MF = *MBB.getParent();
...
AArch64InstrInfo::isCopyInstrImpl(const MachineInstr& MI) const {
    ...
    if (((MI.getOpcode() == AArch64::ORRWrs && MI.getOperand(1).getReg() == AArch64::WZR && MI.getOperand(3).getImm() == 0x0) || (MI.getOpcode() == AArch64::ORRWrr && MI.getOperand(1).getReg() == AArch64::WZR)) &&
        // Check that the w->w move is not a zero-extending w->x mov.
        (!MI.getOperand(0).getReg().isVirtual() || MI.getOperand(0).getSubReg() == 0) && (!MI.getOperand(0).getReg().isPhysical() || MI.findRegisterDefOperandIdx(MI.getOperand(0).getReg() - AArch64::W0 + AArch64::X0,
                                                                                                                                         /*TRI=*/nullptr)
                == -1)) {
        return DestSourcePair{MI.getOperand(0), MI.getOperand(2)};
    }

    if (MI.getOpcode() == AArch64::ORRXrs && MI.getOperand(1).getReg() == AArch64::XZR && MI.getOperand(3).getImm() == 0x0) {
        return DestSourcePair{MI.getOperand(0), MI.getOperand(2)};
    }
...
describeORRLoadedValue(const MachineInstr& MI, Register DescribedReg, const TargetInstrInfo* TII, const TargetRegisterInfo* TRI) {
    ...
    if (MI.getOpcode() == AArch64::ORRWrs && TRI->isSuperRegister(DestReg, DescribedReg)) {
        return ParamLoadedValue(MachineOperand::CreateReg(SrcReg, false), Expr);
    }

    // We may need to describe the lower part of a ORRXrs move.
    if (MI.getOpcode() == AArch64::ORRXrs && TRI->isSubRegister(DestReg, DescribedReg)) {
    ...
    assert(!TRI->isSuperOrSubRegisterEq(DestReg, DescribedReg) && "Unhandled ORR[XW]rs copy case");
...
bool AArch64InstrInfo::isExtendLikelyToBeFolded(
    ...
    assert(ExtMI.getOpcode() == TargetOpcode::G_SEXT || ExtMI.getOpcode() == TargetOpcode::G_ZEXT || ExtMI.getOpcode() == TargetOpcode::G_ANYEXT);
...
bool AArch64InstrInfo::isLegalAddressingMode(unsigned NumBytes, int64_t Offset, unsigned Scale) const {
    if (Offset && Scale) {
        return false;
    }
...
AArch64InstrInfo::probedStackAlloc(MachineBasicBlock::iterator MBBI,
    ...
    MachineBasicBlock* LoopTestMBB = MF.CreateMachineBasicBlock(MBB.getBasicBlock());
    MF.insert(MBBInsertPoint, LoopTestMBB);
    MachineBasicBlock* LoopBodyMBB = MF.CreateMachineBasicBlock(MBB.getBasicBlock());
    MF.insert(MBBInsertPoint, LoopBodyMBB);
    MachineBasicBlock* ExitMBB = MF.CreateMachineBasicBlock(MBB.getBasicBlock());
    MF.insert(MBBInsertPoint, ExitMBB);
    MachineInstr::MIFlag Flags = FrameSetup ? MachineInstr::FrameSetup : MachineInstr::NoFlags;
...
public:
    AArch64PipelinerLoopInfo(MachineBasicBlock* LoopBB, MachineInstr* CondBranch, MachineInstr* Comp, unsigned CompCounterOprNum, MachineInstr* Update, unsigned UpdateCounterOprNum, Register Init, bool IsUpdatePriorComp, const SmallVectorImpl<MachineOperand>& Cond)
    ...
    std::optional<bool> createTripCountGreaterCondition(
        int TC, MachineBasicBlock& MBB, SmallVectorImpl<MachineOperand>& CondParam) override {
    ...
    void createRemainingIterationsGreaterCondition(
        int TC, MachineBasicBlock& MBB, SmallVectorImpl<MachineOperand>& Cond, DenseMap<MachineInstr*, MachineInstr*>& LastStage0Insts) override;
...
void AArch64PipelinerLoopInfo::createRemainingIterationsGreaterCondition(
    int TC, MachineBasicBlock& MBB, SmallVectorImpl<MachineOperand>& Cond, DenseMap<MachineInstr*, MachineInstr*>& LastStage0Insts) {
    ...
    AArch64CC::CondCode CC = (AArch64CC::CondCode)CondBranch->getOperand(0).getImm();
    ...
    if (!LastStage0Insts.empty() && LastStage0Insts[Comp]->getParent() == &MBB) {
        ...
        for (int I = 0; I <= TC; ++I) {
            ...
            if (I != 0) {
                NextCounter = cloneInstr(Comp, CompCounterOprNum, Counter, MBB, MBB.end());
            }
            ...
            if (I != TC) {
                if (I == 0) {
                    ...
                    NextCounter = cloneInstr(Update, UpdateCounterOprNum, Counter, MBB, MBB.end());
                }
            }
            Counter = NextCounter;
        }
        ...
        if (LastStage0Insts.empty()) {
            ...
            if (IsUpdatePriorComp) {
                Counter = cloneInstr(Update, UpdateCounterOprNum, Counter, MBB, MBB.end());
            }
        } else {
...
AArch64InstrInfo::analyzeLoopForPipelining(MachineBasicBlock* LoopBB) const {
    ...
    assert((TBB == LoopBB || FBB == LoopBB) && "The Loop must be a single-basic-block loop");
