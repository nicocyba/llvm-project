...
bool GIMatchTableExecutor::executeMatchTable(
    ...
    enum RejectAction { RejectAndGiveUp,
        RejectAndResume };
        ...
        switch (MatcherOpcode) {
            ...
            case GIM_RecordInsnIgnoreCopies: {
                ...
                if ((size_t)NewInsnID < State.MIs.size()) {
                    ...
                    assert((size_t)NewInsnID == State.MIs.size() && "Expected to store MIs in order");
            ...
            case GIM_CheckFeatures: {
                ...
                DEBUG_WITH_TYPE(TgtExecutor::getName(),
                    dbgs() << CurrentIdx
                           ...
                           << ExpectedBitsetID << ")\n");
                if ((AvailableFeatures & ExecInfo.FeatureBitsets[ExpectedBitsetID]) != ExecInfo.FeatureBitsets[ExpectedBitsetID]) {
                    if (handleReject() == RejectAndGiveUp) {
                        return false;
                    }
                }
            ...
            case GIM_SwitchOpcode: {
                ...
                CurrentIdx = readBytesAs<uint32_t>(MatchTable + CurrentIdx + (EntryIdx * 4));
                if (!CurrentIdx) {
                    CurrentIdx = Default;
                    break;
                }
            ...
            case GIM_SwitchType: {
                ...
                // Each entry is 4 bytes
                CurrentIdx = readBytesAs<uint32_t>(MatchTable + CurrentIdx + (NumEntry * 4));
                if (!CurrentIdx) {
                    CurrentIdx = Default;
                    break;
                }
            ...
            case GIM_CheckImmOperandPredicate: {
                uint64_t InsnID = readULEB();
                unsigned OpIdx = MatcherOpcode == GIM_CheckImmOperandPredicate ? readULEB() : 1;
                ...
                DEBUG_WITH_TYPE(TgtExecutor::getName(),
                    dbgs() << CurrentIdx << ": GIM_CheckImmPredicate(MIs["
                           ...
                           << "), Predicate=" << Predicate << ")\n");
                assert(State.MIs[InsnID] != nullptr && "Used insn before defined");
                assert((State.MIs[InsnID]->getOperand(OpIdx).isImm() || State.MIs[InsnID]->getOperand(OpIdx).isCImm()) && "Expected immediate operand");
            ...
            case GIM_CheckAPIntImmPredicate: {
                ...
                assert(State.MIs[InsnID] != nullptr && "Used insn before defined");
                assert(State.MIs[InsnID]->getOpcode() == TargetOpcode::G_CONSTANT && "Expected G_CONSTANT");
                assert(Predicate > GICXXPred_Invalid && "Expected a valid predicate");
                if (!State.MIs[InsnID]->getOperand(1).isCImm()) {
                    llvm_unreachable("Expected Imm or CImm operand");
                }

                const APInt& Value = State.MIs[InsnID]->getOperand(1).getCImm()->getValue();
            ...
            case GIM_CheckAPFloatImmPredicate: {
                ...
                DEBUG_WITH_TYPE(TgtExecutor::getName(),
                    dbgs()
                        ...
                        << InsnID << "], Predicate=" << Predicate << ")\n");
                assert(State.MIs[InsnID] != nullptr && "Used insn before defined");
                assert(State.MIs[InsnID]->getOpcode() == TargetOpcode::G_FCONSTANT && "Expected G_FCONSTANT");
                assert(State.MIs[InsnID]->getOperand(1).isFPImm() && "Expected FPImm operand");
                assert(Predicate > GICXXPred_Invalid && "Expected a valid predicate");
                const APFloat& Value = State.MIs[InsnID]->getOperand(1).getFPImm()->getValueAPF();
            ...
            case GIM_CheckIsBuildVectorAllZeros: {
                ...
                assert((MI->getOpcode() == TargetOpcode::G_BUILD_VECTOR || MI->getOpcode() == TargetOpcode::G_BUILD_VECTOR_TRUNC) && "Expected G_BUILD_VECTOR or G_BUILD_VECTOR_TRUNC");
            ...
            case GIM_CheckMemoryAddressSpace: {
                ...
                const MachineMemOperand* MMO = *(State.MIs[InsnID]->memoperands_begin() + MMOIdx);
            ...
            case GIM_CheckMemorySizeGreaterThanLLT: {
                ...
                if (MatcherOpcode == GIM_CheckMemorySizeEqualToLLT && MMO->getSizeInBits() != Size) {
                    if (handleReject() == RejectAndGiveUp) {
                        return false;
                    }
                } else if (MatcherOpcode == GIM_CheckMemorySizeLessThanLLT && TypeSize::isKnownGE(MMO->getSizeInBits().getValue(), Size)) {
                    if (handleReject() == RejectAndGiveUp) {
                        return false;
                    }
                } else if (MatcherOpcode == GIM_CheckMemorySizeGreaterThanLLT && TypeSize::isKnownLE(MMO->getSizeInBits().getValue(), Size)) {
                    if (handleReject() == RejectAndGiveUp) {
                        return false;
                    }
                }
            ...
            case GIM_CheckRegBankForClass: {
                uint64_t InsnID = (MatcherOpcode == GIM_RootCheckRegBankForClass) ? 0 : readULEB();
                uint64_t OpIdx = readULEB();
                ...
                assert(State.MIs[InsnID] != nullptr && "Used insn before defined");
                MachineOperand& MO = State.MIs[InsnID]->getOperand(OpIdx);
                if (!MO.isReg() || &RBI.getRegBankFromRegClass(*TRI.getRegClass(RCEnum), MRI.getType(MO.getReg())) != RBI.getRegBank(MO.getReg(), MRI, TRI)) {
                    if (handleReject() == RejectAndGiveUp) {
                        return false;
                    }
                }
            ...
            case GIM_CheckComplexPattern: {
                ...
                ComplexRendererFns Renderer = (Exec.*ExecInfo.ComplexPredicates[ComplexPredicateID])(
            ...
            case GIM_CheckIsSameOperandIgnoreCopies: {
                ...
                if (MatcherOpcode == GIM_CheckIsSameOperandIgnoreCopies) {
                    if (Op.isReg() && OtherOp.isReg()) {
                        if (getSrcRegIgnoringCopies(Op.getReg(), MRI) == getSrcRegIgnoringCopies(OtherOp.getReg(), MRI)) {
                            break;
                        }
                    }
                }
            ...
            case GIR_Copy: {
                uint64_t NewInsnID = (MatcherOpcode == GIR_RootToRootCopy) ? 0 : readULEB();
                uint64_t OldInsnID = (MatcherOpcode == GIR_RootToRootCopy) ? 0 : readULEB();
                uint64_t OpIdx = readULEB();
            ...
            case GIR_MakeTempReg: {
                ...
                State.TempRegisters[TempRegID] = MRI.createGenericVirtualRegister(getTypeFromIdx(TypeID));
                DEBUG_WITH_TYPE(TgtExecutor::getName(),
            ...
            case GIR_Coverage: {
                ...
                DEBUG_WITH_TYPE(TgtExecutor::getName(), dbgs() << CurrentIdx << ": GIR_Coverage(" << RuleID << ")");
                break;
            }
