        ...
        if (mi_match(Op, MRI, m_MInstr(TmpMI))) {
            if (TmpMI->getOpcode() == Opcode && TmpMI->getNumOperands() == 3) {
                ...
                return (L.match(MRI, TmpMI->getOperand(1).getReg()) && R.match(MRI, TmpMI->getOperand(2).getReg())) ||
                    ...
                    // expected.
                    (Commutable && (L.match(MRI, TmpMI->getOperand(2).getReg()) && R.match(MRI, TmpMI->getOperand(1).getReg())));
            }
        }
        return false;
    }
    ...
    bool match(const MachineRegisterInfo& MRI, OpTy&& Op) {
        ...
        if (mi_match(Op, MRI, m_MInstr(TmpMI))) {
            if (TmpMI->getOpcode() == Opcode && TmpMI->getNumOperands() == 2) {
                const TargetInstrInfo *TII = TmpMI->getMF()->getSubtarget().getInstrInfo();
                llvm::outs() << "\t\t\t[MIPatternMatch]" << to_string(current_stage) << " | " << "FuncNameTODO" << " | BinaryOpc_match: op="
                       << (TII ? TII->getName(TmpMI->getOpcode()) : "<unknown>")
...
struct CompareOp_match {
    ...
    bool match(const MachineRegisterInfo& MRI, OpTy&& Op) {
        ...
        auto TmpPred = static_cast<CmpInst::Predicate>(TmpMI->getOperand(1).getPredicate());
        ...
        if (Commutable && L.match(MRI, RHS) && R.match(MRI, LHS) && P.match(MRI, CmpInst::getSwappedPredicate(TmpPred))) {
            return true;
        }
        return false;
    }
    ...
    bool match(const MachineRegisterInfo& MRI, OpTy&& Op) {
        ...
        if (mi_match(Op, MRI, m_MInstr(TmpMI))) {
            if (TmpMI->getOpcode() == Opcode && TmpMI->getNumOperands() == 4) {
                return (Src0.match(MRI, TmpMI->getOperand(1).getReg()) && Src1.match(MRI, TmpMI->getOperand(2).getReg()) && Src2.match(MRI, TmpMI->getOperand(3).getReg()));
            }
        }
        return false;
    }
...
inline TernaryOp_match<Src0Ty, Src1Ty, Src2Ty, TargetOpcode::G_INSERT_VECTOR_ELT>
