...
PatternParser::parseInstructionPattern(const Init& Arg, StringRef Name) {
    ...
    if (const DagInit* IP = getDagWithOperatorOfSubClass(Arg, "Instruction")) {
        ...
        Pat = std::make_unique<CodeGenInstructionPattern>(Instr, insertStrRef(Name));
    } else if (const DagInit* IP = getDagWithOperatorOfSubClass(Arg, "Intrinsic")) {
    ...
    } else if (const DagInit* PFP = getDagWithOperatorOfSubClass(Arg, PatFrag::ClassName)) {
    ...
    } else if (const DagInit* BP = getDagWithOperatorOfSubClass(Arg, BuiltinPattern::ClassName)) {
...
bool PatternParser::parseInstructionPatternOperand(InstructionPattern& IP,
    ...
    if (const auto* DagOp = dyn_cast<DagInit>(OpInit)) {
        ...
        auto ImmTy = PatternType::get(DiagLoc, TyDef,
            "cannot parse immediate '" + DagOp->getAsUnquotedString() + '\'');
        if (!ImmTy) {
            return false;
        }

        if (!IP.hasAllDefs()) {
            PrintError(DiagLoc, "out operand of '" + IP.getInstName() + "' cannot be an immediate");
            return false;
        }
    ...
    if (auto* DefI = dyn_cast<DefInit>(OpInit)) {
        if (!OpName) {
            PrintError(DiagLoc, "expected an operand name after '" + OpInit->getAsString() + '\'');
            return false;
        }
...
bool PatternParser::parseInstructionPatternMIFlags(InstructionPattern& IP,
    ...
    const auto CheckFlagEnum = [&](const Record* R) {
        if (!R->isSubClassOf(MIFlagsEnumClassName)) {
            PrintError(DiagLoc, "'" + R->getName() + "' is not a subclass of '" + MIFlagsEnumClassName + "'");
            return false;
        }
    ...
    for (unsigned K = 0; K < Op->getNumArgs(); ++K) {
        ...
        if (const DagInit* NotDag = getDagWithSpecificOperator(*Arg, "not")) {
            for (const Init* NotArg : NotDag->getArgs()) {
                ...
                if (!DefArg) {
                    PrintError(DiagLoc, "cannot parse '" + NotArg->getAsUnquotedString() + "': expected a '" + MIFlagsEnumClassName + "'");
                    return false;
                }
...
std::unique_ptr<PatFrag> PatternParser::parsePatFragImpl(const Record* Def) {
    ...
    if (Ins->getOperatorAsDef(Def->getLoc())->getName() != "ins") {
        PrintError(Def, "expected 'ins' operator for " + PatFrag::ClassName + " in operands list");
        return nullptr;
    }

    const DagInit* Outs = Def->getValueAsDag("OutOperands");
    if (Outs->getOperatorAsDef(Def->getLoc())->getName() != "outs") {
        PrintError(Def, "expected 'outs' operator for " + PatFrag::ClassName + " out operands list");
        return nullptr;
    }
...
bool PatternParser::parsePatFragParamList(
    ...
    for (unsigned K = 0; K < OpsList.getNumArgs(); ++K) {
        ...
        } else if (isa<UnsetInit>(Ty) || isSpecificDef(*Ty, "gi_mo")) { // no type = gi_mo.
            OpKind = PatFrag::PK_MachineOperand;
        } else {
            PrintError(
                DiagLoc,
                '\'' + NameStr + "' operand type was expected to be 'root', 'gi_imm' or 'gi_mo'");
            return false;
        }
...
const PatFrag* PatternParser::parsePatFrag(const Record* Def) {
    ...
    if (!NewPatFrag) {
        PrintError(Def, "Could not parse " + PatFrag::ClassName + " '" + Def->getName() + "'");
