...
static cl::opt<bool> WarnOnSkippedPatterns("warn-on-skipped-patterns",
    cl::desc("Explain why a pattern was skipped for inclusion "
             "in the GlobalISel selector"),
    cl::init(false),
    cl::cat(GlobalISelEmitterCat));

static cl::opt<bool> GenerateCoverage("instrument-gisel-coverage", cl::desc("Generate coverage instrumentation for GlobalISel"), cl::init(false), cl::cat(GlobalISelEmitterCat));

static cl::opt<std::string> UseCoverageFile("gisel-coverage-file", cl::init(""), cl::desc("Specify file to retrieve coverage information from"), cl::cat(GlobalISelEmitterCat));

static cl::opt<bool> OptimizeMatchTable("optimize-match-table", cl::desc("Generate an optimized version of the match table"), cl::init(true), cl::cat(GlobalISelEmitterCat));
...
static std::string explainPredicates(const TreePatternNode& N) {
    ...
    for (const TreePredicateCall& Call : N.getPredicateCalls()) {
        ...
        Explanation += (Separator + P.getOrigPatFragRecord()->getRecord()->getName()).str();
        Separator = ", ";
...
std::string explainOperator(const Record* Operator) {
    ...
    if (Operator->isSubClassOf("ComplexPattern")) {
        return (" (Operator is an unmapped ComplexPattern, " + Operator->getName() + ")").str();
    }

    if (Operator->isSubClassOf("SDNodeXForm")) {
        return (" (Operator is an unmapped SDNodeXForm, " + Operator->getName() + ")").str();
    }
...
static Error failedImport(const Twine& Reason) { return make_error<StringError>(Reason, inconvertibleErrorCode()); }
...
static Error isTrivialOperatorNode(const TreePatternNode& N) {
    ...
    for (const TreePredicateCall& Call : N.getPredicateCalls()) {
        ...
        if (Predicate.isNonExtLoad() || Predicate.isAnyExtLoad() || Predicate.isSignExtLoad() || Predicate.isZeroExtLoad()) {
            continue;
        }
        ...
        if (Predicate.isAtomic()
            && (Predicate.isAtomicOrderingMonotonic() || Predicate.isAtomicOrderingAcquire() || Predicate.isAtomicOrderingRelease() || Predicate.isAtomicOrderingAcquireRelease()
                || Predicate.isAtomicOrderingSequentiallyConsistent() || Predicate.isAtomicOrderingAcquireOrStronger() || Predicate.isAtomicOrderingWeakerThanAcquire()
                || Predicate.isAtomicOrderingReleaseOrStronger() || Predicate.isAtomicOrderingWeakerThanRelease())) {
            continue;
        }
        ...
        Separator = ", ";
        Explanation += (Separator + "first-failing:" + Predicate.getOrigPatFragRecord()->getRecord()->getName()).str();
        break;
    }
...
static std::string getScopedName(unsigned Scope, const std::string& Name) { return ("pred:" + Twine(Scope) + ":" + Name).str(); }

static std::string getMangledRootDefName(StringRef DefOperandName) { return ("DstI[" + DefOperandName + "]").str(); }

//===- GlobalISelEmitter class --------------------------------------------===//

static Expected<LLTCodeGen> getInstResultType(const TreePatternNode& Dst, const CodeGenTarget& Target) {
...
private:
    ...
    const CodeGenInstruction* getEquivNode(const Record& Equiv, const TreePatternNode& N) const;

    Error importRulePredicates(RuleMatcher& M, ArrayRef<const Record*> Predicates);
    Expected<InstructionMatcher&> createAndImportSelDAGMatcher(RuleMatcher& Rule, InstructionMatcher& InsnMatcher, const TreePatternNode& Src, unsigned& TempOpIdx);
    Error importComplexPatternOperandMatcher(OperandMatcher& OM, const Record* R, unsigned& TempOpIdx) const;
    Error importChildMatcher(RuleMatcher& Rule, InstructionMatcher& InsnMatcher, const TreePatternNode& SrcChild, bool OperandIsAPointer, bool OperandIsImmArg, unsigned OpIdx, unsigned& TempOpIdx);

    Expected<BuildMIAction&> createAndImportInstructionRenderer(RuleMatcher& M, InstructionMatcher& InsnMatcher, const TreePatternNode& Dst) const;
    Expected<action_iterator> createAndImportSubInstructionRenderer(action_iterator InsertPt, RuleMatcher& M, const TreePatternNode& Dst, unsigned TempReg) const;
    Expected<action_iterator> createInstructionRenderer(action_iterator InsertPt, RuleMatcher& M, const TreePatternNode& Dst) const;

    Expected<action_iterator> importExplicitDefRenderers(action_iterator InsertPt, RuleMatcher& M, BuildMIAction& DstMIBuilder, const TreePatternNode& Dst, bool IsRoot) const;

    Expected<action_iterator> importExplicitUseRenderers(action_iterator InsertPt, RuleMatcher& M, BuildMIAction& DstMIBuilder, const TreePatternNode& Dst) const;

    Error importNamedNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N) const;

    Error importLeafNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator InsertPt) const;

    Error importXFormNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N) const;

    Error importInstructionNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator& InsertPt) const;

    Error importNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator& InsertPt) const;

    Error importImplicitDefRenderers(BuildMIAction& DstMIBuilder, ArrayRef<const Record*> ImplicitDefs) const;
    ...
    MatchTable buildMatchTable(MutableArrayRef<RuleMatcher> Rules, bool Optimize, bool WithCoverage);
    ...
    const CodeGenRegisterClass* inferSuperRegisterClassForNode(const TypeSetByHwMode& Ty, const TreePatternNode& SuperRegNode, const TreePatternNode& SubRegIdxNode) const;
    const CodeGenSubRegIndex* inferSubRegIndexForNode(const TreePatternNode& SubRegIdxNode) const;

    /// Infer a CodeGenRegisterClass which suppoorts \p Ty and \p SubRegIdxNode.
    /// Return nullptr if no such class exists.
    const CodeGenRegisterClass* inferSuperRegisterClass(const TypeSetByHwMode& Ty, const TreePatternNode& SubRegIdxNode) const;

    /// Return the CodeGenRegisterClass associated with \p Leaf if it has one.
    const CodeGenRegisterClass* getRegClassFromLeaf(const TreePatternNode& Leaf) const;

    /// Return a CodeGenRegisterClass for \p N if one can be found. Return
    /// nullptr otherwise.
    const CodeGenRegisterClass* inferRegClassFromPattern(const TreePatternNode& N) const;

    const CodeGenRegisterClass* inferRegClassFromInstructionPattern(const TreePatternNode& N, unsigned ResIdx) const;

    Error constrainOperands(action_iterator InsertPt, RuleMatcher& M, unsigned InsnID, const TreePatternNode& Dst) const;

    /// Return the size of the MemoryVT in this predicate, if possible.
    std::optional<unsigned> getMemSizeBitsFromPredicate(const TreePredicateFn& Predicate);

    // Add builtin predicates.
    Expected<InstructionMatcher&> addBuiltinPredicates(const Record* SrcGIEquivOrNull, const TreePredicateFn& Predicate, InstructionMatcher& InsnMatcher, bool& HasAddedMatcher);
};

StringRef getPatFragPredicateEnumName(const Record* R) { return R->getName(); }

void GlobalISelEmitter::gatherOpcodeValues() { InstructionOpcodeMatcher::initOpcodeValuesMap(Target); }

void GlobalISelEmitter::gatherTypeIDValues() { LLTOperandMatcher::initTypeIDValuesMap(); }
...
void GlobalISelEmitter::gatherNodeEquivs() {
    ...
    for (const Record* Equiv : RK.getAllDerivedDefinitions("GIComplexPatternEquiv")) {
    ...
    for (const Record* Equiv : RK.getAllDerivedDefinitions("GISDNodeXFormEquiv")) {
        const Record* SelDAGEquiv = Equiv->getValueAsDef("SelDAGEquivalent");
        if (!SelDAGEquiv) {
            continue;
        }
...
const Record* GlobalISelEmitter::findNodeEquiv(const Record* N) const { return NodeEquivs.lookup(N); }

const CodeGenInstruction* GlobalISelEmitter::getEquivNode(const Record& Equiv, const TreePatternNode& N) const {
    if (N.getNumChildren() >= 1) {
        // setcc operation maps to two different G_* instructions based on the type.
        if (!Equiv.isValueUnset("IfFloatingPoint") && MVT(N.getChild(0).getSimpleType(0)).isFloatingPoint()) {
            return &Target.getInstruction(Equiv.getValueAsDef("IfFloatingPoint"));
        }
    }

    if (!Equiv.isValueUnset("IfConvergent") && N.getIntrinsicInfo(CGP)->isConvergent) {
        return &Target.getInstruction(Equiv.getValueAsDef("IfConvergent"));
    }

    for (const TreePredicateCall& Call : N.getPredicateCalls()) {
        const TreePredicateFn& Predicate = Call.Fn;
        if (!Equiv.isValueUnset("IfSignExtend") && (Predicate.isLoad() || Predicate.isAtomic()) && Predicate.isSignExtLoad()) {
            return &Target.getInstruction(Equiv.getValueAsDef("IfSignExtend"));
        }
        if (!Equiv.isValueUnset("IfZeroExtend") && (Predicate.isLoad() || Predicate.isAtomic()) && Predicate.isZeroExtLoad()) {
...
GlobalISelEmitter::GlobalISelEmitter(const RecordKeeper& RK) : GlobalISelMatchTableExecutorEmitter(), RK(RK), CGP(RK), Target(CGP.getTargetInfo()), CGRegs(Target.getRegBank()) {
    ClassName = Target.getName().str() + "InstructionSelector";
}

//===- Emitter ------------------------------------------------------------===//

Error GlobalISelEmitter::importRulePredicates(RuleMatcher& M, ArrayRef<const Record*> Predicates) {
...
std::optional<unsigned> GlobalISelEmitter::getMemSizeBitsFromPredicate(const TreePredicateFn& Predicate) {
    std::optional<LLTCodeGen> MemTyOrNone = MVTToLLT(getValueType(Predicate.getMemoryVT()));
    ...
    return llvm::alignTo(static_cast<unsigned>(MemTyOrNone->get().getSizeInBits()), 8);
}

Expected<InstructionMatcher&> GlobalISelEmitter::addBuiltinPredicates(const Record* SrcGIEquivOrNull, const TreePredicateFn& Predicate, InstructionMatcher& InsnMatcher, bool& HasAddedMatcher) {
    if (Predicate.isLoad() || Predicate.isStore() || Predicate.isAtomic()) {
        if (const ListInit* AddrSpaces = Predicate.getAddressSpaces()) {
            ...
            if (!ParsedAddrSpaces.empty()) {
                InsnMatcher.addPredicate<MemoryAddressSpacePredicateMatcher>(0, ParsedAddrSpaces);
                return InsnMatcher;
            }
        }
    ...
    if (Predicate.isLoad() && Predicate.isNonExtLoad()) {
        InsnMatcher.addPredicate<MemoryVsLLTSizePredicateMatcher>(0, MemoryVsLLTSizePredicateMatcher::EqualTo, 0);
        return InsnMatcher;
    }
    if (Predicate.isLoad() && Predicate.isAnyExtLoad()) {
        InsnMatcher.addPredicate<MemoryVsLLTSizePredicateMatcher>(0, MemoryVsLLTSizePredicateMatcher::LessThan, 0);
        return InsnMatcher;
    }
    ...
    if (Predicate.isStore()) {
        if (Predicate.isTruncStore()) {
            if (Predicate.getMemoryVT() != nullptr) {
                ...
                InsnMatcher.addPredicate<MemorySizePredicateMatcher>(0, *MemSizeInBits / 8);
            } else {
                InsnMatcher.addPredicate<MemoryVsLLTSizePredicateMatcher>(0, MemoryVsLLTSizePredicateMatcher::LessThan, 0);
            }
            return InsnMatcher;
    ...
    if (!SrcGIEquivOrNull->isValueUnset("IfSignExtend") && Predicate.isSignExtLoad()) {
        return InsnMatcher;
    }

    // No check required. We already did it by swapping the opcode.
    if (!SrcGIEquivOrNull->isValueUnset("IfZeroExtend") && Predicate.isZeroExtLoad()) {
        return InsnMatcher;
    }
    ...
    if (Predicate.isAtomic()) {
        ...
        if (Predicate.isAtomicOrderingAcquireRelease()) {
            InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("AcquireRelease");
            return InsnMatcher;
        }
        if (Predicate.isAtomicOrderingSequentiallyConsistent()) {
            InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("SequentiallyConsistent");
            return InsnMatcher;
        }
    }

    if (Predicate.isAtomicOrderingAcquireOrStronger()) {
        InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("Acquire", AtomicOrderingMMOPredicateMatcher::AO_OrStronger);
        return InsnMatcher;
    }
    if (Predicate.isAtomicOrderingWeakerThanAcquire()) {
        InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("Acquire", AtomicOrderingMMOPredicateMatcher::AO_WeakerThan);
        return InsnMatcher;
    }

    if (Predicate.isAtomicOrderingReleaseOrStronger()) {
        InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("Release", AtomicOrderingMMOPredicateMatcher::AO_OrStronger);
        return InsnMatcher;
    }
    if (Predicate.isAtomicOrderingWeakerThanRelease()) {
        InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("Release", AtomicOrderingMMOPredicateMatcher::AO_WeakerThan);
        return InsnMatcher;
    }
...
Expected<InstructionMatcher&> GlobalISelEmitter::createAndImportSelDAGMatcher(RuleMatcher& Rule, InstructionMatcher& InsnMatcher, const TreePatternNode& Src, unsigned& TempOpIdx) {
    ...
    if (Src.isLeaf()) {
        ...
        if (isa<IntInit>(SrcInit)) {
            InsnMatcher.addPredicate<InstructionOpcodeMatcher>(&Target.getInstruction(RK.getDef("G_CONSTANT")));
        } else {
        ...
        if (!SrcGIEquivOrNull) {
            return failedImport("Pattern operator lacks an equivalent Instruction" + explainOperator(Src.getOperator()));
        }
    ...
    if (SrcGIEquivOrNull && SrcGIEquivOrNull->getValueAsBit("CheckMMOIsNonAtomic")) {
        ...
        InsnMatcher.addPredicate<AtomicOrderingMMOPredicateMatcher>("Unordered", AtomicOrderingMMOPredicateMatcher::AO_OrStronger);
    }
    ...
    for (const TypeSetByHwMode& VTy : Src.getExtTypes()) {
        ...
        const bool OperandIsAPointer = SrcGIOrNull && SrcGIOrNull->isOutOperandAPointer(OpIdx);
    ...
    for (const TreePredicateCall& Call : Src.getPredicateCalls()) {
        ...
        auto InsnMatcherOrError = addBuiltinPredicates(SrcGIEquivOrNull, Predicate, InsnMatcher, HasAddedBuiltinMatcher);
        ...
        if (!HasAddedBuiltinMatcher) {
            return failedImport("Src pattern child has predicate (" + explainPredicates(Src) + ")");
        }
    }
    ...
    if (Src.isLeaf()) {
        ...
        if (const IntInit* SrcIntInit = dyn_cast<IntInit>(SrcInit)) {
            OperandMatcher& OM = InsnMatcher.addOperand(OpIdx++, Src.getName(), TempOpIdx);
            OM.addPredicate<LiteralIntOperandMatcher>(SrcIntInit->getValue());
        } else {
            return failedImport("Unable to deduce gMIR opcode to handle Src (which is a leaf)");
        }
    } else {
        assert(SrcGIOrNull && "Expected to have already found an equivalent Instruction");
        if (SrcGIOrNull->TheDef->getName() == "G_CONSTANT" || SrcGIOrNull->TheDef->getName() == "G_FCONSTANT" || SrcGIOrNull->TheDef->getName() == "G_FRAME_INDEX") {
        ...
        if (IsFCmp || SrcGIOrNull->TheDef->getName() == "G_ICMP") {
            ...
            if (SrcChild.isLeaf()) {
                ...
                StringRef PredType = IsFCmp ? CCDef->getValueAsString("FCmpPredicate") : CCDef->getValueAsString("ICmpPredicate");
        ...
        bool IsIntrinsic = SrcGIOrNull->TheDef->getName() == "G_INTRINSIC" || SrcGIOrNull->TheDef->getName() == "G_INTRINSIC_W_SIDE_EFFECTS"
            || SrcGIOrNull->TheDef->getName() == "G_INTRINSIC_CONVERGENT" || SrcGIOrNull->TheDef->getName() == "G_INTRINSIC_CONVERGENT_W_SIDE_EFFECTS";
        ...
        for (unsigned I = 0; I != NumChildren; ++I) {
            ...
            if (auto Error = importChildMatcher(Rule, InsnMatcher, SrcChild, OperandIsAPointer, OperandIsImmArg, OpIdx++, TempOpIdx)) {
                return std::move(Error);
            }
        }
    }
...
Error GlobalISelEmitter::importComplexPatternOperandMatcher(OperandMatcher& OM, const Record* R, unsigned& TempOpIdx) const {
    const auto& ComplexPattern = ComplexPatternEquivs.find(R);
    if (ComplexPattern == ComplexPatternEquivs.end()) {
        return failedImport("SelectionDAG ComplexPattern (" + R->getName() + ") not mapped to GlobalISel");
    }
...
static StringRef getSrcChildName(const TreePatternNode& SrcChild, const Record*& PhysReg) {
...
Error GlobalISelEmitter::importChildMatcher(
    RuleMatcher& Rule, InstructionMatcher& InsnMatcher, const TreePatternNode& SrcChild, bool OperandIsAPointer, bool OperandIsImmArg, unsigned OpIdx, unsigned& TempOpIdx) {

    const Record* PhysReg = nullptr;
    std::string SrcChildName = std::string(getSrcChildName(SrcChild, PhysReg));
    if (!SrcChild.isLeaf() && SrcChild.getOperator()->isSubClassOf("ComplexPattern")) {
    ...
    OperandMatcher& OM = PhysReg ? InsnMatcher.addPhysRegInput(PhysReg, OpIdx, TempOpIdx) : InsnMatcher.addOperand(OpIdx, SrcChildName, TempOpIdx);
    ...
    // registers.
    if (!OperandIsImmArg) {
        if (auto Error = OM.addTypeCheckPredicate(ChildTypes.front(), OperandIsAPointer)) {
            return failedImport(toString(std::move(Error)) + " for Src operand (" + to_string(SrcChild) + ")");
        }
    }
    ...
    if (!SrcChild.isLeaf()) {
        if (SrcChild.getOperator()->isSubClassOf("ComplexPattern")) {
            ...
            if (auto Error = importComplexPatternOperandMatcher(OM, SrcChild.getOperator(), TempOpIdx)) {
                return Error;
            }
            ...
            for (unsigned I = 0, E = SrcChild.getNumChildren(); I != E; ++I) {
                ...
                if (!SubOperand.getName().empty()) {
                    if (auto Error = Rule.defineComplexSubOperand(SubOperand.getName(), SrcChild.getOperator(), RendererID, I, SrcChildName)) {
                        return Error;
                    }
                }
            }
        ...
        auto MaybeInsnOperand = OM.addPredicate<InstructionOperandMatcher>(InsnMatcher.getRuleMatcher(), SrcChild.getName());
        if (!MaybeInsnOperand) {
        ...
        auto InsnMatcherOrError = createAndImportSelDAGMatcher(Rule, InsnOperand.getInsnMatcher(), SrcChild, TempOpIdx);
        if (auto Error = InsnMatcherOrError.takeError()) {
            return Error;
        }
    ...
    if (auto* ChildDefInit = dyn_cast<DefInit>(SrcChild.getLeafValue())) {
        ...
        if (ChildRec->isSubClassOf("RegisterClass") || ChildRec->isSubClassOf("RegisterOperand")) {
            OM.addPredicate<RegisterBankOperandMatcher>(Target.getRegisterClass(getInitValueAsRegClass(ChildDefInit)));
            return Error::success();
        }
        ...
        if (ChildRec->isSubClassOf("Register")) {
            ...
            const CodeGenRegisterClass* RC = CGRegs.getMinimalPhysRegClass(ChildRec, &VT);
            if (!RC) {
                return failedImport("Could not determine physical register class of pattern source");
            }
        ...
        if (ChildRec->isSubClassOf("ImmLeaf")) {
            return failedImport("Src pattern child def is an unsupported tablegen class (ImmLeaf)");
        }
        ...
        if (ImmAllOnesV || ChildRec->getName() == "immAllZerosV") {
            auto MaybeInsnOperand = OM.addPredicate<InstructionOperandMatcher>(InsnMatcher.getRuleMatcher(), SrcChild.getName(), false);
            InstructionOperandMatcher& InsnOperand = **MaybeInsnOperand;

            ValueTypeByHwMode VTy = ChildTypes.front().getValueTypeByHwMode();

            const CodeGenInstruction& BuildVector = Target.getInstruction(RK.getDef("G_BUILD_VECTOR"));
            const CodeGenInstruction& BuildVectorTrunc = Target.getInstruction(RK.getDef("G_BUILD_VECTOR_TRUNC"));

            // Treat G_BUILD_VECTOR as the canonical opcode, and G_BUILD_VECTOR_TRUNC
            // as an alternative.
            InsnOperand.getInsnMatcher().addPredicate<InstructionOpcodeMatcher>(ArrayRef({&BuildVector, &BuildVectorTrunc}));
            ...
            // has to succeed.
            OperandMatcher& OM = InsnOperand.getInsnMatcher().addOperand(0, "", TempOpIdx);
            if (auto Error = OM.addTypeCheckPredicate(TypeSetByHwMode(VTy),
                    /*OperandIsAPointer=*/false)) {
                return failedImport(toString(std::move(Error)) + " for result of Src pattern operator");
            }

            InsnOperand.getInsnMatcher().addPredicate<VectorSplatImmPredicateMatcher>(ImmAllOnesV ? VectorSplatImmPredicateMatcher::AllOnes : VectorSplatImmPredicateMatcher::AllZeros);
            return Error::success();
        }

        return failedImport("Src pattern child def is an unsupported tablegen class");
    }
...
Error GlobalISelEmitter::importNamedNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N) const {
    ...
    if (!N.isLeaf()) {
        ...
        if (OperatorName == "bb" || OperatorName == "timm" || OperatorName == "tframeindex") {
    ...
    if (const auto* DI = dyn_cast<DefInit>(N.getLeafValue())) {
        ...
        if (R->isSubClassOf("ComplexPattern")) {
            ...
            if (I == ComplexPatternEquivs.end()) {
                return failedImport("ComplexPattern " + R->getName() + " does not have GISel equivalent");
            }

            const OperandMatcher& OM = M.getOperandMatcher(NodeName);
            MIBuilder.addRenderer<RenderComplexPatternOperand>(*I->second, NodeName, OM.getAllocatedTemporariesBaseID());
            return Error::success();
        }

        if (R->isSubClassOf("RegisterOperand") && !R->isValueUnset("GIZeroRegister")) {
            MIBuilder.addRenderer<CopyOrAddZeroRegRenderer>(NodeName, R->getValueAsDef("GIZeroRegister"));
            return Error::success();
        }

        // TODO: All special cases are handled above. Remove this check and add
        //   CopyRenderer unconditionally.
        if (R->isSubClassOf("RegisterClass") || R->isSubClassOf("RegisterOperand") || R->isSubClassOf("ValueType")) {
            MIBuilder.addRenderer<CopyRenderer>(NodeName);
            return Error::success();
        }
    }
    ...
    if (!M.hasOperand(NodeName)) {
        return failedImport("could not find node $" + NodeName + " in the source DAG");
    }
...
Error GlobalISelEmitter::importLeafNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator InsertPt) const {
    ...
    if (const auto* DI = dyn_cast<DefInit>(N.getLeafValue())) {
        ...
        if (R->getName() == "undef_tied_input") {
            ...
            auto I = M.insertAction<BuildMIAction>(InsertPt, M.allocateOutputInsnID(), &Target.getInstruction(RK.getDef("IMPLICIT_DEF")));
...
Error GlobalISelEmitter::importXFormNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N) const {
    const Record* XFormRec = N.getOperator();
    auto I = SDNodeXFormEquivs.find(XFormRec);
    if (I == SDNodeXFormEquivs.end()) {
        return failedImport("SDNodeXForm " + XFormRec->getName() + " does not have GISel equivalent");
    }
...
Error GlobalISelEmitter::importInstructionNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator& InsertPt) const {
    ...
    auto InsertPtOrError = createAndImportSubInstructionRenderer(++InsertPt, M, N, TempRegID);
    if (!InsertPtOrError) {
...
Error GlobalISelEmitter::importNodeRenderer(RuleMatcher& M, BuildMIAction& MIBuilder, const TreePatternNode& N, action_iterator& InsertPt) const {
    if (N.hasName()) {
...
Expected<BuildMIAction&> GlobalISelEmitter::createAndImportInstructionRenderer(RuleMatcher& M, InstructionMatcher& InsnMatcher, const TreePatternNode& Dst) const {
    ...
    for (auto PhysOp : M.physoperands()) {
        InsertPt = M.insertAction<BuildMIAction>(InsertPt, M.allocateOutputInsnID(), &Target.getInstruction(RK.getDef("COPY")));
        BuildMIAction& CopyToPhysRegMIBuilder = *static_cast<BuildMIAction*>(InsertPt->get());
        CopyToPhysRegMIBuilder.addRenderer<AddRegisterRenderer>(Target, PhysOp.first, true);
    ...
    if (auto Error = importExplicitUseRenderers(InsertPt, M, DstMIBuilder, Dst).takeError()) {
        return std::move(Error);
    }
...
Expected<action_iterator> GlobalISelEmitter::createAndImportSubInstructionRenderer(action_iterator InsertPt, RuleMatcher& M, const TreePatternNode& Dst, unsigned TempRegID) const {
    ...
    BuildMIAction& DstMIBuilder = *static_cast<BuildMIAction*>(InsertPtOrError.get()->get());

    // Assign the result to TempReg.
    DstMIBuilder.addRenderer<TempRegRenderer>(TempRegID, true);

    // Handle additional (ignored) results.
    InsertPtOrError = importExplicitDefRenderers(std::prev(*InsertPtOrError), M, DstMIBuilder, Dst, /*IsRoot=*/false);
    if (auto Error = InsertPtOrError.takeError()) {
        return std::move(Error);
    }

    InsertPtOrError = importExplicitUseRenderers(InsertPtOrError.get(), M, DstMIBuilder, Dst);
    if (auto Error = InsertPtOrError.takeError()) {
        return std::move(Error);
    }

    if (auto Error = constrainOperands(InsertPt, M, DstMIBuilder.getInsnID(), Dst)) {
        return std::move(Error);
    }
...
Expected<action_iterator> GlobalISelEmitter::createInstructionRenderer(action_iterator InsertPt, RuleMatcher& M, const TreePatternNode& Dst) const {
    const Record* DstOp = Dst.getOperator();
    if (!DstOp->isSubClassOf("Instruction")) {
        if (DstOp->isSubClassOf("ValueType")) {
            return failedImport("Pattern operator isn't an instruction (it's a ValueType)");
        }
    ...
    return M.insertAction<BuildMIAction>(InsertPt, M.allocateOutputInsnID(), DstI);
}

Expected<action_iterator> GlobalISelEmitter::importExplicitDefRenderers(action_iterator InsertPt, RuleMatcher& M, BuildMIAction& DstMIBuilder, const TreePatternNode& Dst, bool IsRoot) const {
    ...
    for (unsigned I = IsRoot ? 0 : 1, E = DstI->Operands.NumDefs; I != E; ++I) {
        ...
        if (OpInfo.Rec->isSubClassOf("OptionalDefOperand")) {
            for (const TreePatternNode& DefaultOp : make_pointee_range(CGP.getDefaultOperand(OpInfo.Rec).DefaultOps)) {
                ...
                DstMIBuilder.addRenderer<AddRegisterRenderer>(Target, Reg, /*IsDef=*/true, /*IsDead=*/true);
            }
            continue;
        }
        ...
        unsigned TempRegID = M.allocateTempRegID();
        InsertPt = M.insertAction<MakeTempRegisterAction>(InsertPt, *OpTy, TempRegID);
        DstMIBuilder.addRenderer<TempRegRenderer>(TempRegID, /*IsDef=*/true, /*SubReg=*/nullptr, /*IsDead=*/true);
    }
...
Expected<action_iterator> GlobalISelEmitter::importExplicitUseRenderers(action_iterator InsertPt, RuleMatcher& M, BuildMIAction& DstMIBuilder, const TreePatternNode& Dst) const {
    const CodeGenInstruction* DstI = DstMIBuilder.getCGI();
    ...
    if (Name == "EXTRACT_SUBREG") {
        ...
        const DefInit* SubRegInit = dyn_cast<DefInit>(Dst.getChild(1).getLeafValue());
        if (!SubRegInit) {
        ...
        if (!ValChild.isLeaf()) {
            ...
            unsigned TempRegID = M.allocateTempRegID();
            InsertPt = M.insertAction<MakeTempRegisterAction>(InsertPt, *ExtractSrcTy, TempRegID);

            auto InsertPtOrError = createAndImportSubInstructionRenderer(++InsertPt, M, ValChild, TempRegID);
            if (auto Error = InsertPtOrError.takeError()) {
                return std::move(Error);
            }
        ...
        const auto SrcRCDstRCPair = RC->getMatchingSubClassWithSubRegs(CGRegs, SubIdx);
        if (SrcRCDstRCPair) {
        ...
        if (const auto& SubOperand = M.getComplexSubOperand(RegOperandName)) {
            DstMIBuilder.addRenderer<RenderComplexPatternOperand>(*std::get<0>(*SubOperand), RegOperandName, std::get<1>(*SubOperand), std::get<2>(*SubOperand), SubIdx);
            return InsertPt;
        }
    ...
    if (Name == "REG_SEQUENCE") {
        ...
        const Record* RCDef = getInitValueAsRegClass(Dst.getChild(0).getLeafValue());
        if (!RCDef) {
        ...
        for (unsigned I = 1; I != ExpectedDstINumUses; I += 2) {
            ...
            if (const DefInit* SubRegInit = dyn_cast<DefInit>(SubRegChild.getLeafValue())) {
                CodeGenSubRegIndex* SubIdx = CGRegs.getSubRegIdx(SubRegInit->getDef());
    ...
    while (NonOverridableOperands > NumResults && CGP.operandHasDefault(DstI->Operands[NonOverridableOperands - 1].Rec)) {
    ...
    for (unsigned I = 0; I != DstINumUses; ++I) {
        ...
        if (CGP.operandHasDefault(OperandNode) && (InstOpNo < NonOverridableOperands || Child >= Dst.getNumChildren())) {
            // This is a predicate or optional def operand which the pattern has not
            // overridden, or which we aren't letting it override; emit the 'default
            // ops' operands.
            for (const TreePatternNode& OpNode : make_pointee_range(CGP.getDefaultOperand(OperandNode).DefaultOps)) {
        ...
        if (Error Err = importNodeRenderer(M, DstMIBuilder, Dst.getChild(Child), InsertPt)) {
            return Err;
        }
    ...
    if (NumDefaultOps + ExpectedDstINumUses != DstINumUses) {
        return failedImport(
            "Expected " + llvm::to_string(DstINumUses) + " used operands but found " + llvm::to_string(ExpectedDstINumUses) + " explicit ones and " + llvm::to_string(NumDefaultOps) + " default ones");
    }

    return InsertPt;
}

Error GlobalISelEmitter::importImplicitDefRenderers(BuildMIAction& DstMIBuilder, ArrayRef<const Record*> ImplicitDefs) const {
...
Error GlobalISelEmitter::constrainOperands(action_iterator InsertPt, RuleMatcher& M, unsigned InsnID, const TreePatternNode& Dst) const {
    const Record* DstOp = Dst.getOperator();
    ...
    if (DstIName == "COPY_TO_REGCLASS") {
        ...
        const Record* DstIOpRec = getInitValueAsRegClass(Dst.getChild(1).getLeafValue());

        if (DstIOpRec == nullptr) {
            return failedImport("COPY_TO_REGCLASS operand #1 isn't a register class");
        }

        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 0, Target.getRegisterClass(DstIOpRec));
    } else if (DstIName == "EXTRACT_SUBREG") {
        const CodeGenRegisterClass* SuperClass = inferRegClassFromPattern(Dst.getChild(0));
        if (!SuperClass) {
            return failedImport("Cannot infer register class from EXTRACT_SUBREG operand #0");
        }
        ...
        const auto SrcRCDstRCPair = SuperClass->getMatchingSubClassWithSubRegs(CGRegs, SubIdx);
        if (!SrcRCDstRCPair) {
        ...
        assert(SrcRCDstRCPair->second && "Couldn't find a matching subclass");
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 0, *SrcRCDstRCPair->second);
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 1, *SrcRCDstRCPair->first);
    } else if (DstIName == "INSERT_SUBREG") {
        // We need to constrain the destination, a super regsister source, and a
        // subregister source.
        const CodeGenRegisterClass* SubClass = inferRegClassFromPattern(Dst.getChild(1));
        if (!SubClass) {
            return failedImport("Cannot infer register class from INSERT_SUBREG operand #1");
        }
        const CodeGenRegisterClass* SuperClass = inferSuperRegisterClassForNode(Dst.getExtType(0), Dst.getChild(0), Dst.getChild(2));
        if (!SuperClass) {
            return failedImport("Cannot infer register class for INSERT_SUBREG operand #0");
        }
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 0, *SuperClass);
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 1, *SuperClass);
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 2, *SubClass);
        ...
        const CodeGenRegisterClass* SubClass = inferRegClassFromPattern(Dst.getChild(1));
        if (!SubClass) {
            return failedImport("Cannot infer register class from SUBREG_TO_REG child #1");
        }
        // We don't have a child to look at that might have a super register node.
        const CodeGenRegisterClass* SuperClass = inferSuperRegisterClass(Dst.getExtType(0), Dst.getChild(2));
        if (!SuperClass) {
            return failedImport("Cannot infer register class for SUBREG_TO_REG operand #0");
        }
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 0, *SuperClass);
        M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, 2, *SubClass);
        ...
        for (unsigned I = 1; I != Num; I += 2) {
            ...
            M.insertAction<ConstrainOperandToRegClassAction>(InsertPt, InsnID, I, *SrcRCDstRCPair->second);
        }
    } else {
...
const CodeGenRegisterClass* GlobalISelEmitter::getRegClassFromLeaf(const TreePatternNode& Leaf) const {
...
const CodeGenRegisterClass* GlobalISelEmitter::inferRegClassFromPattern(const TreePatternNode& N) const {
    if (N.isLeaf()) {
...
const CodeGenRegisterClass* GlobalISelEmitter::inferRegClassFromInstructionPattern(const TreePatternNode& N, unsigned ResIdx) const {
    const CodeGenInstruction& Inst = Target.getInstruction(N.getOperator());
    assert(ResIdx < Inst.Operands.NumDefs && "Can only infer register class for explicit defs");
    ...
    if (InstName == "INSERT_SUBREG") {
        ...
        return inferSuperRegisterClassForNode(N.getExtType(0), Child0, N.getChild(2));
    }
    ...
    if (InstName == "EXTRACT_SUBREG") {
        ...
        const CodeGenRegisterClass* SuperRC = inferRegClassFromPattern(N.getChild(0));
        if (!SuperRC) {
            return nullptr;
        }
        ...
        const auto SubRCAndSubRegRC = SuperRC->getMatchingSubClassWithSubRegs(CGRegs, SubIdx);
        if (!SubRCAndSubRegRC) {
            return nullptr;
        }
...
const CodeGenRegisterClass* GlobalISelEmitter::inferSuperRegisterClass(const TypeSetByHwMode& Ty, const TreePatternNode& SubRegIdxNode) const {
...
const CodeGenRegisterClass* GlobalISelEmitter::inferSuperRegisterClassForNode(const TypeSetByHwMode& Ty, const TreePatternNode& SuperRegNode, const TreePatternNode& SubRegIdxNode) const {
    ...
    if (const CodeGenRegisterClass* SuperRegisterClass = inferRegClassFromPattern(SuperRegNode)) {
        return SuperRegisterClass;
    }
    return inferSuperRegisterClass(Ty, SubRegIdxNode);
}

const CodeGenSubRegIndex* GlobalISelEmitter::inferSubRegIndexForNode(const TreePatternNode& SubRegIdxNode) const {
    if (!SubRegIdxNode.isLeaf()) {
        return nullptr;
    }
...
Expected<RuleMatcher> GlobalISelEmitter::runOnPattern(const PatternToMatch& P) {
    ...
    if (auto Err = isTrivialOperatorNode(Dst)) {
        return failedImport("Dst pattern root isn't a trivial operator (" + toString(std::move(Err)) + ")");
    }
    if (auto Err = isTrivialOperatorNode(Src)) {
        return failedImport("Src pattern root isn't a trivial operator (" + toString(std::move(Err)) + ")");
    }
    ...
    auto InsnMatcherOrError = createAndImportSelDAGMatcher(M, InsnMatcherTemp, Src, TempOpIdx);
    if (auto Error = InsnMatcherOrError.takeError()) {
        return std::move(Error);
    }
    ...
    if (Dst.isLeaf()) {
        if (const Record* RCDef = getInitValueAsRegClass(Dst.getLeafValue())) {
            ...
            auto& DstMIBuilder = M.addAction<BuildMIAction>(M.allocateOutputInsnID(), &DstI);
    ...
    unsigned DstExpDefs = DstI.Operands.NumDefs, DstNumDefs = DstI.ImplicitDefs.size() + DstExpDefs, SrcNumDefs = Src.getExtTypes().size();
    if (DstNumDefs < SrcNumDefs) {
        if (DstNumDefs != 0) {
            return failedImport("Src pattern result has more defs than dst MI (" + to_string(SrcNumDefs) + " def(s) vs " + to_string(DstNumDefs) + " def(s))");
        }
    ...
    for (unsigned I = 0; I < N; ++I) {
        ...
        const CodeGenRegisterClass* RC = inferRegClassFromInstructionPattern(Dst, I);
        if (!RC) {
            return failedImport("Could not infer register class for result #" + Twine(I) + " from pattern " + to_string(Dst));
        }
        OM.addPredicate<RegisterBankOperandMatcher>(*RC);
    }

    auto DstMIBuilderOrError = createAndImportInstructionRenderer(M, InsnMatcher, Dst);
    ...
    if (auto Error = constrainOperands(M.actions_end(), M, DstMIBuilder.getInsnID(), Dst)) {
        return std::move(Error);
    }
...
MatchTable GlobalISelEmitter::buildMatchTable(MutableArrayRef<RuleMatcher> Rules, bool Optimize, bool WithCoverage) {
    ...
    llvm::stable_sort(InputRules, [&OpcodeOrder](const Matcher* A, const Matcher* B) {
        auto* L = static_cast<const RuleMatcher*>(A);
        auto* R = static_cast<const RuleMatcher*>(B);
        return std::tuple(OpcodeOrder[L->getOpcode()], L->insnmatchers_front().getNumOperandMatchers()) < std::tuple(OpcodeOrder[R->getOpcode()], R->insnmatchers_front().getNumOperandMatchers());
    });
    ...
    std::vector<Matcher*> OptRules = optimizeRules<GroupMatcher>(InputRules, MatcherStorage);
...
void GlobalISelEmitter::emitAdditionalImpl(raw_ostream& OS) {
    OS << "bool " << getClassName() << "::selectImpl(MachineInstr &I, CodeGenCoverage &CoverageInfo) const {\n"
...
void GlobalISelEmitter::emitMIPredicateFns(raw_ostream& OS) {
    ...
    std::copy_if(AllPatFrags.begin(), AllPatFrags.end(), std::back_inserter(MatchedRecords), [](const Record* R) { return !R->getValueAsString("GISelPredicateCode").empty(); });
    emitMIPredicateFnsImpl<const Record*>(
        ...
        "  (void)Operands;\n"
        "  (void)MRI;",
        ArrayRef<const Record*>(MatchedRecords), &getPatFragPredicateEnumName, [](const Record* R) { return R->getValueAsString("GISelPredicateCode"); }, "PatFrag predicates.");
}

void GlobalISelEmitter::emitI64ImmPredicateFns(raw_ostream& OS) {
    std::vector<const Record*> MatchedRecords;
    std::copy_if(AllPatFrags.begin(), AllPatFrags.end(), std::back_inserter(MatchedRecords), [](const Record* R) {
        bool Unset;
        return !R->getValueAsString("ImmediateCode").empty() && !R->getValueAsBitOrUnset("IsAPFloat", Unset) && !R->getValueAsBit("IsAPInt");
    });
    emitImmPredicateFnsImpl<const Record*>(
        OS, "I64", "int64_t", ArrayRef<const Record*>(MatchedRecords), &getPatFragPredicateEnumName, [](const Record* R) { return R->getValueAsString("ImmediateCode"); }, "PatFrag predicates.");
}
...
void GlobalISelEmitter::emitAPFloatImmPredicateFns(raw_ostream& OS) {
    ...
    std::copy_if(AllPatFrags.begin(), AllPatFrags.end(), std::back_inserter(MatchedRecords), [](const Record* R) {
        bool Unset;
        return !R->getValueAsString("ImmediateCode").empty() && R->getValueAsBitOrUnset("IsAPFloat", Unset);
    });
    emitImmPredicateFnsImpl<const Record*>(
        OS, "APFloat", "const APFloat &", ArrayRef<const Record*>(MatchedRecords), &getPatFragPredicateEnumName, [](const Record* R) { return R->getValueAsString("ImmediateCode"); },
        "PatFrag predicates.");
}
...
void GlobalISelEmitter::emitAPIntImmPredicateFns(raw_ostream& OS) {
    std::vector<const Record*> MatchedRecords;
    std::copy_if(
        AllPatFrags.begin(), AllPatFrags.end(), std::back_inserter(MatchedRecords), [](const Record* R) { return !R->getValueAsString("ImmediateCode").empty() && R->getValueAsBit("IsAPInt"); });
    emitImmPredicateFnsImpl<const Record*>(
        OS, "APInt", "const APInt &", ArrayRef<const Record*>(MatchedRecords), &getPatFragPredicateEnumName, [](const Record* R) { return R->getValueAsString("ImmediateCode"); },
        "PatFrag predicates.");
}
...
bool hasBFloatType(const TreePatternNode& Node) {
    for (unsigned I = 0, E = Node.getNumTypes(); I < E; I++) {
        ...
        for (auto T : Ty) {
            if (T.second == MVT::bf16 || (T.second.isVector() && T.second.getScalarType() == MVT::bf16)) {
                return true;
            }
        }
    }
...
void GlobalISelEmitter::run(raw_ostream& OS) {
    ...
    emitSourceFileHeader(("Global Instruction Selector for the " + Target.getName() + " target").str(), OS);
    ...
    for (const PatternToMatch& Pat : CGP.ptms()) {
        ...
        if (auto Err = MatcherOrErr.takeError()) {
            if (WarnOnSkippedPatterns) {
                PrintWarning(Pat.getSrcRecord()->getLoc(), "Skipped pattern: " + toString(std::move(Err)));
            } else {
        ...
        if (RuleCoverage) {
            if (RuleCoverage->isCovered(MatcherOrErr->getRuleID())) {
                ...
                PrintWarning(Pat.getSrcRecord()->getLoc(), "Pattern is not covered by a test");
            }
        }
    ...
    auto OrderByName = [](const Record* A, const Record* B) { return A->getName() < B->getName(); };

    std::vector<const Record*> ComplexPredicates = RK.getAllDerivedDefinitions("GIComplexOperandMatcher");
    llvm::sort(ComplexPredicates, OrderByName);

    std::vector<StringRef> CustomRendererFns;
    transform(RK.getAllDerivedDefinitions("GICustomOperandRenderer"), std::back_inserter(CustomRendererFns), [](const auto& Record) { return Record->getValueAsString("RendererFn"); });
    ...
    CustomRendererFns.erase(llvm::unique(CustomRendererFns), CustomRendererFns.end());
    ...
    llvm::stable_sort(Rules, [&](const RuleMatcher& A, const RuleMatcher& B) {
        ...
        if (A.isHigherPriorityThan(B)) {
            assert(!B.isHigherPriorityThan(A)
    ...
    // Build match table
    const MatchTable Table = buildMatchTable(Rules, OptimizeMatchTable, GenerateCoverage);
    ...
    emitExecutorImpl(OS, Table, TypeObjects, Rules, ComplexPredicates, CustomRendererFns, "GET_GLOBALISEL_IMPL");
    emitPredicatesDecl(OS, "GET_GLOBALISEL_PREDICATES_DECL");
    emitPredicatesInit(OS, "GET_GLOBALISEL_PREDICATES_INIT");
}

void GlobalISelEmitter::declareSubtargetFeature(const Record* Predicate) { SubtargetFeatures.try_emplace(Predicate, Predicate, SubtargetFeatures.size()); }

unsigned GlobalISelEmitter::declareHwModeCheck(StringRef HwModeFeatures) { return HwModes.emplace(HwModeFeatures.str(), HwModes.size()).first->second; }

} // end anonymous namespace

//===----------------------------------------------------------------------===//

static TableGen::Emitter::OptClass<GlobalISelEmitter> X("gen-global-isel", "Generate GlobalISel selector");
