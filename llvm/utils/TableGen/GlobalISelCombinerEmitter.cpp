...
bool CombineRuleBuilder::emitCXXMatchApply(CodeExpansions& CE, RuleMatcher& M, ArrayRef<CXXPattern*> Matchers) {
    ...
    OS << "for (const auto& C : Observer.CreatedInstrsNico) { obs_created += MI2String(*C); obs_created += \" | \"; }\n";

    OS << "std::string obs_changed = \"\";\n";
    OS << "for (const auto& C : Observer.ChangedInstrsNico) { obs_changed += MI2String(*C); obs_changed += \" | \"; }\n";

    OS << "std::string obs_deleted = \"\";\n";
    OS << "for (const auto& C : Observer.DeletedInstrsNico) { obs_deleted += MI2String(*C); obs_deleted += \" | \"; }\n";
    ...
    OS << "Observer.DeletedInstrsNico.clear();\n";
    OS << "Observer.ChangedInstrsNico.clear();\n";
    OS << "Observer.CreatedInstrsNico.clear();\n";
