...
cl::list<std::string>
    SelectedCombiners("combiners", cl::desc("Emit the specified combiners"), cl::cat(GICombinerEmitterCat), cl::CommaSeparated);
...
void declareOperandExpansion(CodeExpansions& CE, const OperandMatcher& OM, StringRef Name) {
    if (OM.isVariadic()) {
        CE.declare(Name, "getRemainingOperands(*State.MIs[" + to_string(OM.getInsnVarID()) + "], " + to_string(OM.getOpIdx()) + ")");
    } else {
        CE.declare(Name, "State.MIs[" + to_string(OM.getInsnVarID()) + "]->getOperand(" + to_string(OM.getOpIdx()) + ")");
    }
}

void declareTempRegExpansion(CodeExpansions& CE, unsigned TempRegID, StringRef Name) {
...
bool CombineRuleOperandTypeChecker::processApplyPattern(InstructionPattern& P) {
    ...
    return check(P, /*CheckTypeOf*/ [&](const PatternType& Ty) {
        ...
        PrintError(RuleDef.getLoc(), "'" + OpName + "' ('" + Ty.str() + "') does not refer to a matched operand!");
        return false;
    });
}
    ...
    for (auto* Pat : ApplyPats) {
        for (unsigned K = 0; K < Pat->operands_size(); ++K) {
            ...
            if (Op.isDef() || Op.isNamedImmediate()) {
                ...
                if (PatternType Ty = inferNamedOperandType(*Pat, Op.getOperandName(), TECs)) {
                    if (DebugTypeInfer) {
        ...
        for (auto* Pat : ApplyPats) {
            if (!OTC.check(*Pat, [&](const auto&) { return true; })) {
                PrintFatalError(RuleDef.getLoc(),
                    "OperandTypeChecker unexpectedly failed on '" + Pat->getName() + "' during Type Inference");
            }
        }
        OTC.propagateTypes();
...
PatternType CombineRuleOperandTypeChecker::inferImmediateType(
    const InstructionPattern& IP, unsigned ImmOpIdx, const TypeEquivalenceClasses& TECs) const {
...
PatternType CombineRuleOperandTypeChecker::inferNamedOperandType(
    const InstructionPattern& IP, StringRef OpName, const TypeEquivalenceClasses& TECs, bool AllowSelf) const {
...
std::vector<std::string> CombineRuleOperandTypeChecker::getMCOIOperandTypes(
    ...
    const Record* VarArgsTy = CGI.TheDef->isSubClassOf("GenericInstruction")
        ? CGI.TheDef->getValueAsOptionalDef("variadicOpsType")
        : nullptr;
    std::string VarArgsTyName = VarArgsTy ? ("MCOI::" + VarArgsTy->getValueAsString("OperandType")).str()
...
private:
    ...
    void addCXXPredicate(RuleMatcher& M, const CodeExpansions& CE, const CXXPattern& P, const PatternAlternatives& Alts);
    ...
    bool emitMatchPattern(CodeExpansions& CE, const PatternAlternatives& Alts, const InstructionPattern& IP);
    bool emitMatchPattern(CodeExpansions& CE, const PatternAlternatives& Alts, const AnyOpcodePattern& AOP);
    ...
    bool emitCXXMatchApply(CodeExpansions& CE, RuleMatcher& M, ArrayRef<CXXPattern*> Matchers);

    // Recursively visits InstructionPatterns from P to build up the
    // RuleMatcher actions.
    bool emitInstructionApplyPattern(CodeExpansions& CE, RuleMatcher& M, const InstructionPattern& P, DenseSet<const Pattern*>& SeenPats, StringMap<unsigned>& OperandToTempRegID);
    ...
    bool emitBuiltinApplyPattern(CodeExpansions& CE, RuleMatcher& M, const BuiltinPattern& P, StringMap<unsigned>& OperandToTempRegID);
    ...
    // needed.
    using OperandMapperFnRef = function_ref<InstructionOperand(const InstructionOperand&)>;
    using OperandDefLookupFn = function_ref<const InstructionPattern*(StringRef)>;
    bool emitCodeGenInstructionMatchPattern(
        CodeExpansions& CE, const PatternAlternatives& Alts, RuleMatcher& M, InstructionMatcher& IM, const CodeGenInstructionPattern& P, DenseSet<const Pattern*>& SeenPats, OperandDefLookupFn LookupOperandDef, OperandMapperFnRef OperandMapper = [](const auto& O) { return O; });
...
bool CombineRuleBuilder::parseAll() {
    ...
    if (!buildRuleOperandsTable() || !typecheckPatterns() || !findRoots() || !checkSemantics() || !buildPermutationsToEmit()) {
        return false;
    }
    LLVM_DEBUG(verify());
    return true;
...
void CombineRuleBuilder::print(raw_ostream& OS) const {
    ...
    const auto DumpPats = [&](StringRef Name, const PatternMap& Pats) {
        ...
        for (const auto& [Name, Pat] : Pats) {
            ...
            if (isa<InstructionPattern>(Pat.get()) && ApplyRoots.contains(cast<InstructionPattern>(Pat.get()))) {
                OS << "<apply_root>";
            }
            OS << Name << ":";
...
void CombineRuleBuilder::verify() const {
    const auto VerifyPats = [&](const PatternMap& Pats) {
        for (const auto& [Name, Pat] : Pats) {
            ...
            if (Name != Pat->getName()) {
                Pat->dump();
                PrintFatalError("Pattern name mismatch! Map name: " + Name + ", Pat name: " + Pat->getName());
            }
...
CombineRuleBuilder::getLLTCodeGenOrTempType(const PatternType& PT,
    ...
    if (OM.isVariadic()) {
        PrintError("type '" + PT.str() + "' is ill-formed: '" + OM.getSymbolicName() + "' is a variadic pack operand");
        return std::nullopt;
    }
...
void CombineRuleBuilder::print(raw_ostream& OS,
    ...
    SmallVector<std::string, 1> Strings(
        map_range(Alts, [](const auto& PatAndPerm) {
            return PatAndPerm.first->getName().str() + "[" + to_string(PatAndPerm.second) + "]";
        }));
...
bool CombineRuleBuilder::addApplyPattern(std::unique_ptr<Pattern> Pat) {
    ...
    if (isa<AnyOpcodePattern>(Pat.get())) {
        PrintError("'" + Name + "': wip_match_opcode is not supported in apply patterns");
        return false;
    }

    if (isa<PatFragPattern>(Pat.get())) {
        PrintError("'" + Name + "': using " + PatFrag::ClassName + " is not supported in apply patterns");
        return false;
    }
...
bool CombineRuleBuilder::addMatchPattern(std::unique_ptr<Pattern> Pat) {
    ...
    if (const auto* BP = dyn_cast<BuiltinPattern>(Pat.get())) {
        PrintError("'" + BP->getInstName() + "' cannot be used in a 'match' pattern");
        return false;
    }
...
void CombineRuleBuilder::addCXXPredicate(RuleMatcher& M,
    ...
    const auto& ExpandedCode = DebugCXXPreds ? P.expandCode(CE, Loc, AddComment) : P.expandCode(CE, Loc);
        ...
        if (auto* IP = dyn_cast<InstructionPattern>(Pat.get())) {
            ...
            for (const auto& [Idx, Op] : enumerate(IP->operands())) {
                if (Op.getType().isTypeOf()) {
                    PrintError(PatternType::TypeOfClassName + " is not supported in 'match' patterns");
                    PrintNote("operand " + Twine(Idx) + " of '" + IP->getName() + "' has type '" + Op.getType().str() + "'");
                    HasDiag = true;
                }
            }
            if (HasDiag) {
...
bool CombineRuleBuilder::buildPermutationsToEmit() {
    ...
    if (int64_t MaxPerms = RuleDef.getValueAsInt("MaxPermutations");
        ...
        if ((int64_t)PermutationsToEmit.size() > MaxPerms) {
            PrintError("cannot emit rule '" + RuleDef.getName() + "'; " + Twine(PermutationsToEmit.size()) + " permutations would be emitted, but the max is " + Twine(MaxPerms));
            return false;
        }
    }
...
bool CombineRuleBuilder::checkSemantics() {
    ...
    const auto CheckVariadicOperands = [&](const InstructionPattern& IP,
        ...
        for (auto& Op : IP.operands()) {
            ...
            if (IsMatch && &Op != &IP.operands_back()) {
                PrintError("'" + IP.getInstName() + "': " + PatternType::VariadicClassName + " can only be used on the last operand");
                return false;
            }

            if (Op.isDef()) {
                PrintError("'" + IP.getInstName() + "': " + PatternType::VariadicClassName + " cannot be used on defs");
                return false;
            }
        }

        if (HasVariadic && !IP.isVariadic()) {
            PrintError("cannot use a " + PatternType::VariadicClassName + " operand on non-variadic instruction '" + IP.getInstName() + "'");
            return false;
        }
                ...
                if (auto* FI = CGP->getMIFlagsInfo()) {
                    if (!FI->copy_flags().empty()) {
                        PrintError("'match' patterns cannot refer to flags from other "
                                   "instructions");
                        PrintNote("MIFlags in '" + CGP->getName() + "' refer to: " + join(FI->copy_flags(), ", "));
                        return false;
                    }
                }
            }
        ...
        if (const auto* CGP = dyn_cast<CodeGenInstructionPattern>(IP)) {
            if (auto* FI = CGP->getMIFlagsInfo()) {
                for (auto InstName : FI->copy_flags()) {
                    ...
                    if (It == MatchPats.end()) {
                        PrintError("unknown instruction '$" + InstName + "' referenced in MIFlags of '" + CGP->getName() + "'");
                        return false;
                    }

                    if (!isa<CodeGenInstructionPattern>(It->second.get())) {
                        PrintError(
                            "'$" + InstName + "' does not refer to a CodeGenInstruction in MIFlags of '" + CGP->getName() + "'");
                        return false;
                    }
                }
            }
        ...
        switch (BIP->getBuiltinKind()) {
            case BI_EraseRoot: {
                ...
                if (IRoot->getNumInstDefs() != 0) {
                    ...
                    PrintNote("'" + IRoot->getInstName() + "' has " + Twine(IRoot->getNumInstDefs()) + " output operands");
                    return false;
                }
                break;
            }
            case BI_ReplaceReg: {
                ...
                if (!Def) {
                    PrintError(Name + " cannot find a matched pattern that defines '" + OldRegName + "'");
                    return false;
                }
                if (MatchOpTable.getDef(OldRegName) != MatchRoot) {
                    PrintError(Name + " cannot replace '" + OldRegName + "': this builtin can only replace a register defined by the "
                                                                         "match root");
                    return false;
                }
                break;
    ...
    if (!hasOnlyCXXApplyPatterns() && !MatchDatas.empty()) {
        PrintError(MatchDataClassName + " can only be used if 'apply' in entirely written in C++");
        return false;
    }
...
bool CombineRuleBuilder::findRoots() {
    const auto Finish = [&]() {
        ...
        if (IPRoot->getNumInstDefs() == 0) {
            ...
            if (!ApplyRoot) {
                PrintError("apply pattern root '" + RootName + "' must be an instruction pattern");
                return false;
            }
        ...
        if (auto It = ApplyPats.find(RootName); It != ApplyPats.end()) {
            if (find(ApplyRoots, It->second.get()) == ApplyRoots.end()) {
                PrintError("apply pattern '" + RootName + "' is supposed to be a root but it does not redefine any of "
    ...
    if (!MatchRoot) {
        PrintError("Cannot use live-in operand '" + RootName + "' as match pattern root!");
        return false;
    }
...
bool CombineRuleBuilder::buildRuleOperandsTable() {
    const auto DiagnoseRedefMatch = [&](StringRef OpName) {
        PrintError("Operand '" + OpName + "' is defined multiple times in the 'match' patterns");
    };

    const auto DiagnoseRedefApply = [&](StringRef OpName) {
        PrintError("Operand '" + OpName + "' is defined multiple times in the 'apply' patterns");
    };
...
bool CombineRuleBuilder::parseDefs(const DagInit& Def) {
    ...
    for (unsigned I = 0, E = Def.getNumArgs(); I < E; ++I) {
        ...
        if (const Record* MatchDataRec = getDefOfSubClass(*Def.getArg(I), MatchDataClassName)) {
...
bool CombineRuleBuilder::emitPatFragMatchPattern(
    CodeExpansions& CE, const PatternAlternatives& Alts, RuleMatcher& RM, InstructionMatcher* IM, const PatFragPattern& PFP, DenseSet<const Pattern*>& SeenPats) {
    ...
    const auto OperandMapper =
        ...
        if (ArgOp.getType() && O.getType() && ArgOp.getType() != O.getType()) {
            ...
            PrintWarning("impossible type constraints: operand " + Twine(PIdx) + " of '" + PFP.getName() + "' has type '" + ArgOp.getType().str() + "', but '" + PFName + "' constrains it to '" + O.getType().str() + "'");
            if (ArgOp.isNamedOperand()) {
                PrintNote("operand " + Twine(PIdx) + " of '" + PFP.getName() + "' is '" + ArgOp.getOperandName() + "'");
            }
            if (O.isNamedOperand()) {
                PrintNote("argument " + Twine(PIdx) + " of '" + PFName + "' is '" + ParamName + "'");
            }
        }
    ...
    for (const auto& [Idx, InOp] : enumerate(PF.out_params())) {
        ...
        assert(isa<CodeGenInstructionPattern>(Def) && "Nested PatFrags not supported yet");
...
bool CombineRuleBuilder::emitApplyPatterns(CodeExpansions& CE, RuleMatcher& M) {
    ...
    for (auto* ApplyRoot : ApplyRoots) {
        assert(isa<InstructionPattern>(ApplyRoot) && "Root can only be a InstructionPattern!");
    ...
    // Erase the root.
    unsigned RootInsnID = M.getInsnVarID(M.getInstructionMatcher(MatchRoot->getName()));
    M.addAction<EraseInstAction>(RootInsnID);

    return true;
}

bool CombineRuleBuilder::emitCXXMatchApply(CodeExpansions& CE, RuleMatcher& M, ArrayRef<CXXPattern*> Matchers) {
    ...
    OS << "for (const auto& C : Helper.Observer.CreatedInstrsNico) { obs_created += MI2String(*C); obs_created += \" | \"; }\n";

    OS << "std::string obs_changed = \"\";\n";
    OS << "for (const auto& C : Helper.Observer.ChangedInstrsNico) { obs_changed += MI2String(*C); obs_changed += \" | \"; }\n";

    OS << "std::string obs_deleted = \"\";\n";
    OS << "for (const auto& C : Helper.Observer.DeletedInstrsNico) { obs_deleted += MI2String(*C); obs_deleted += \" | \"; }\n";
    ...
    OS << "Helper.Observer.DeletedInstrsNico.clear();\n";
    OS << "Helper.Observer.ChangedInstrsNico.clear();\n";
    OS << "Helper.Observer.CreatedInstrsNico.clear();\n";
...
bool CombineRuleBuilder::emitInstructionApplyPattern(
    CodeExpansions& CE, RuleMatcher& M, const InstructionPattern& P, DenseSet<const Pattern*>& SeenPats, StringMap<unsigned>& OperandToTempRegID) {
    ...
    for (auto& Op : P.named_operands()) {
        ...
        if (const auto* DefPat = ApplyOpTable.getDef(OpName)) {
            ...
            if (!Op.isNamedImmediate() && !MatchOpTable.lookup(OpName).Found) {
                PrintError("invalid output operand '" + OpName + "': operand is not a live-in of the match pattern, and it "
                                                                 "has no definition");
                return false;
            }
        }
    ...
    auto& DstMI = M.addAction<BuildMIAction>(M.allocateOutputInsnID(), &CGIP.getInst());
    ...
    for (auto& Op : P.operands()) {
        ...
        if (Op.isNamedImmediate()) {
            PrintError("invalid output operand '" + Op.getOperandName() + "': output immediates cannot be named");
            PrintNote("while emitting pattern '" + P.getName() + "' (" + P.getInstName() + ")");
            return false;
        }
        ...
        if (!Op.isDef()) {
            if (auto It = OperandToTempRegID.find(OpName);
                ...
                assert(!MatchOpTable.lookup(OpName).Found && "Temp reg is also from match pattern?");
                ...
                assert(MatchOpTable.lookup(OpName).Found && !ApplyOpTable.getDef(OpName) && "Temp reg not emitted yet!");
        ...
        if (OpLookupRes.Found) {
            if (OpLookupRes.isLiveIn()) {
                ...
                PrintError("Cannot define live-in operand '" + OpName + "' in the 'apply' pattern");
                return false;
            }
        ...
        if (auto It = OperandToTempRegID.find(OpName);
            ...
            if (!Ty) {
                PrintError("def of a new register '" + OpName + "' in the apply patterns must have a type");
                return false;
            }
...
bool CombineRuleBuilder::emitCodeGenInstructionApplyImmOperand(
    RuleMatcher& M, BuildMIAction& DstMI, const CodeGenInstructionPattern& P, const InstructionOperand& O) {
...
bool CombineRuleBuilder::emitBuiltinApplyPattern(
    CodeExpansions& CE, RuleMatcher& M, const BuiltinPattern& P, StringMap<unsigned>& OperandToTempRegID) {
...
bool isLiteralImm(const InstructionPattern& P, unsigned OpIdx) {
    if (const auto* CGP = dyn_cast<CodeGenInstructionPattern>(&P)) {
        ...
        return (InstName == "G_CONSTANT" || InstName == "G_FCONSTANT") && OpIdx == 1;
    }
...
bool CombineRuleBuilder::emitCodeGenInstructionMatchPattern(
    CodeExpansions& CE, const PatternAlternatives& Alts, RuleMatcher& M, InstructionMatcher& IM, const CodeGenInstructionPattern& P, DenseSet<const Pattern*>& SeenPats, OperandDefLookupFn LookupOperandDef, OperandMapperFnRef OperandMapper) {
    auto StackTrace = PrettyStackTraceEmit(RuleDef, &P);
    ...
    for (auto [Idx, OriginalO] : enumerate(P.operands())) {
        ...
        assert(RemappedO.isNamedOperand() == OriginalO.isNamedOperand() && "Cannot remap an unnamed operand to a named one!");

        const auto Ty = RemappedO.getType();

        const auto OpName = RemappedO.isNamedOperand() ? RemappedO.getOperandName().str() : "";
        ...
        const unsigned RealIdx = (P.isIntrinsic() && !OriginalO.isDef()) ? (Idx + 1) : Idx;
        ...
        if (Ty.isVariadicPack() && M.hasOperand(OpName)) {
            ...
            PrintError("each instance of a " + PatternType::VariadicClassName + " operand must have a unique name within the match patterns");
            PrintNote("'" + OpName + "' is used multiple times");
            return false;
        }

        OperandMatcher& OM = IM.addOperand(RealIdx, OpName, AllocatedTemporariesBaseID++,
        ...
        if (Ty.isVariadicPack()) {
            ...
            assert((Idx + 1) == P.operands_size() && "VariadicPack isn't last operand!");
        ...
        if (Ty && (OpName.empty() || !M.getOperandMatcher(OpName).contains<LLTOperandMatcher>())) {
        ...
        if (OriginalO.hasImmValue()) {
            ...
            //    (G_SEXT $y, (i32 0))
            //    (COPY $x, 42:$y)
            PrintError("'" + OpName + "' is a named immediate, it cannot be defined by another "
                                      "instruction");
        ...
        // emit it.
        auto InstOpM = OM.addPredicate<InstructionOperandMatcher>(M, DefPat->getName());
        if (!InstOpM) {
            // TODO: copy-pasted from GlobalISelEmitter.cpp. Is it still relevant
            // here?
            PrintError("Nested instruction '" + DefPat->getName() + "' cannot be the same as another operand '" + OriginalO.getOperandName() + "'");
            return false;
        }
    ...
    for (const Record* Rec : RulesAndGroups) {
        ...
        if (!RulesSeen.insert(RuleName).second) {
            PrintWarning(Rec->getLoc(),
                "skipping rule '" + Rec->getName() + "' because it has already been processed");
            continue;
        }
...
static TableGen::Emitter::Opt X("gen-global-isel-combiner", EmitGICombiner, "Generate GlobalISel Combiner");
